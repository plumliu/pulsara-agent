...
当本 turn 存在 active skill injection，且模型调用 `terminal` 或 `terminal_process` 时，runtime 必须在 `capability_gate_decision.value.capability_context` 中写入轻量归因：
...
所有最终 gate 结果必须写入 `CustomEvent(name="capability_gate_decision")`，用于 inspector/replay 解释历史行为。
...
每个工具调用最终 gate 判断必须产生 `capability_gate_decision` 事件，value 至少包含：
