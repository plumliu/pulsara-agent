...
classification；terminal、terminal_process 与 terminal_monitor 必须按真实 action/arguments 通过共享 classifier 解析，不能仅凭工具名把 search、write、
...
## 16. Terminal process monitor

公开terminal能力固定拆成三个工具：`terminal`只启动命令，`terminal_process`只公开`list/log/poll/wait/write/submit/close_stdin/kill`，`terminal_monitor`只公开`register/list/cancel`。不存在`watch`、`wake_when`、旧monitor action或兼容alias。`terminal_monitor.register`建立persistent repeated observation，不是后台版`wait`：注册ToolResult立即返回，后续progress/heartbeat/completion/expiry通过Host ingress或合法`PRE_MODEL_STEP`交付。`terminal_process.wait/poll/log`仍只返回当前ToolResult，并携带typed output cursor receipt。

三个工具的model-visible description与executor description必须由`terminal_public_api.py`中的同一组静态常量拥有，descriptor和tool class不得复制第二份文案。Descriptions必须冻结以下决策与停止条件：`terminal`返回非`running`状态时只表示该次调用没有live process可管理，不得因此调用process/monitor工具，但不阻止artifact读取、错误修复或后续任务；所有后续ID逐字复制前一个ToolResult，不得构造；`terminal_process.list`不需要process ID，其余process action使用`terminal`返回的exact ID；`poll`与`log`是二选一的即时观察，不能默认连续调用；`wait`最多调用一次且不超过30秒，仍为`running`时不得循环；长任务默认省略conditions以禁用progress和heartbeat，completion持续受监控直至bounded expiry，只有用户或任务本身确实要求进度/周期报告时才启用output condition或heartbeat；`submit`添加换行、`write`不添加换行，`close_stdin`只在程序需要EOF时调用；`terminal_monitor.cancel`与`terminal_process.kill`分别表示“停止通知”和“终止进程”，是互斥意图而不是推荐的连续步骤。Few-shot只演示这些高风险决策，不需要穷举所有action，也不依赖额外skill读取才能正确使用核心工具。
