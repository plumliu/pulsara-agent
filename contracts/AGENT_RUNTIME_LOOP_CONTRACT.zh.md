...
7. store exposure in `state.scratchpad["capability_exposure"]` for this run。
8. emit `CustomEvent(name="capability_exposure_resolved")`。
9. 进入 model/tool loop。
...
- pending MCP elicitation：`pending_interaction_kind="mcp_elicitation"`。
...
MCP elicitation 使用通用 tool suspension seam。

`ToolExecutionSuspended(interaction_kind="mcp_elicitation")` 必须：

- 停止当前 tool batch；
- 取消尚未完成的兄弟 tool task；
- 写 `tool_execution_suspended` custom event；
- 设置 `pending_interaction_kind="mcp_elicitation"`；
- 由 HostSession 暴露 `PendingMcpElicitation`。

用户 resolution 后，runtime 必须调用 adapter 的 resume path，将 answer 路由回 manager/server/request id。
...
AgentRuntime active loop 不得在 pending approval / pending plan / pending MCP elicitation 时 compact。
...
- 不允许把 MCP elicitation 格式化成普通 tool error 后继续。
...
- MCP elicitation suspend/resume 路径完整。
