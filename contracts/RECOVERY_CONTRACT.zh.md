...
## 14. Typed MCP lifecycle recovery

Reopen先用 `McpInputRequiredLifecycleStore` 从 typed suspension、resolution-submitted、
resume-failed、terminal disposition/ToolResult与 closure facts重建 lifecycle。它与 live
runtime及 Inspector共用同一个 reducer；不得解释自由 payload或按 call ID/sequence猜测。

固定顺序为：

```text
model stream/control recovery
→ parent/child MCP active lifecycle recovery
→ closure + ToolResult + settlement
→ window/account close
→ RunEnd
→ generic dangling-run/child projection repair
```

SESSION_REOPEN 不恢复 process-local lease或 opaque request state。Active suspension在 exact
lease owner不存在时必须写 recovered lease-unavailable closure；generic dangling repair和
child repair遇到 active MCP lifecycle时 delegate/skip。Parent graph terminal reference只可在
child closure、ToolResult与 RunEnd FULL 后产生。

Critical publication failure使用 RuntimeSession publication latch与 exact
terminal-maintenance lease。Terminal disposition已关闭 interaction时不写第二个 closure；
closure publication failure只补 RunEnd；RunEnd publication failure不再生成 event。任何
NONE retry复用 stable candidates和原 absolute recovery deadline。

Host SESSION_REOPEN在任何model/control、PostgreSQL query、MCP closure、parent/child
dangling repair、window/account close、RunEnd与projection repair开始前冻结唯一absolute
deadline，并把同一个monotonic timestamp贯穿整个调用图。不得按dangling run、child或
repair stage重新生成`now + timeout`，否则合法长session会退化为
`O(owner_count × timeout)` open延迟。
