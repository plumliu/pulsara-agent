...
- **拒绝**（`is_read_only=False`）：`write_file` / `edit_file`（工作区写）、`terminal` / `terminal_process` / `terminal_monitor`（终端执行、控制或调度）、`remember_*`（durable memory 写）、以及未来任何副作用工具。
...
| terminal_monitor.list | DENY | ALLOW | ALLOW | ALLOW |
| terminal_monitor.register/cancel | DENY | WAIT | WAIT | ALLOW |
...
- `terminal_monitor.list`遵循相同观察豁免；`register/cancel`始终按调度副作用处理。
...
- 不允许新增 terminal / terminal_process 执行入口绕过 hardline 判定；`terminal_monitor`不得获得命令或stdin执行能力。
...
- terminal_monitor.list在ask/accept/bypass下不触发审批，register/cancel按terminal调度副作用处理。
...
- PRE_INTERACTION_RESUME 必须从原 `RunGenesisAuthority`与已提交authority revision重建permission snapshot；当前Host default的变化不能改写
  suspended run，continuation boundary/exposure与前驱snapshot不一致必须fail closed。
...
## 12. Terminal monitor scheduling permission
