...
lease、retiring slot 和 close attempt 的唯一 lifecycle owner。
...
- pending payload 必须包含 interaction id、wrapper tool call id、wrapper tool name、server id、source_method、protocol version、request_state、input_requests、round count、deadline、audit-only `mcp_installation_id`，以及含slot/snapshot/generation的exact `mcp_binding_identity`。
