...
- Episode:有点像 run timeline summary / working_context,但 Pulsara 不把它作为 canonical memory。
- Atomic Facts:类似 canonical `statement` / typed memory nodes。
- Foresight:当前没有一等公民。`stale_after` / `expires_at` 只是过期/衰减,不是「未来一段时间内应被考虑的前瞻」。
- Metadata:Pulsara 更强,因为 event log / evidence / governance batch / scope 都可追。

可学点:

- 为 recall 准备一段经过规范化的 `embedded_text`,不必只 embed `statement`。
- 对「未来有效的临时状态」建模,但这应走 Pulsara ontology/governance,不能直接把 LLM foresight 写 canonical。
...
但 Pulsara 不该照搬「在线 LLM 改写 profile」:
...
Pulsara 应学习 MemScene 的 **检索结构价值**,而不是学习它的无治理写法。
...
CREATE TABLE memory_vector_index (
    ...
    PRIMARY KEY (graph_id, memory_id, embedding_model),
...
- embedding 模型可版本化。
- 支持迁移期 old/new model dual-read。
- 不把 vector 生命周期绑死到 FTS index。
...
- 对刚写入 memory 的 governance relatedness,可用「候选即时 embedding + existing vector index」查询已有节点,但候选本身不必先入库。
...
- provider 层是显式依赖:
  ...
  - `EmbeddingProvider`
  - `RerankProvider`
- service 层由 runtime wiring 负责构造。
- memory recall / governance relatedness / future search tools 通过依赖注入复用这些 provider。

不建议做 `get_embedding_provider()` / `get_reranker()` 这类进程级 singleton accessor,原因很具体:

- Pulsara 现在的可测性依赖 wiring 注入 deterministic fake,而不是 monkey-patch 全局单例。
- real LLM / durable / test-double 路径已经大量依赖 `build_runtime_wiring(...)` 的构造边界。
- provider 的 timeout、模型版本、batch policy、shadow rollout 最终都要进 settings / wiring,而不是藏在模块级缓存里。
- recall 只是 retrieval stack 的第一个消费者;如果未来 governance relatedness 复用 dense retrieval,显式注入比“全局拿一个 provider”更容易追 trace 和控预算。
...
- local sentence-transformers / Qwen embedding。
- remote OpenAI-compatible embedding。
- test deterministic fake。

约束:

- provider id 必须进入 `embedding_model`。
- dim 必须和 DB vector dim 一致。
...
- 读 `memory_write_outbox` 或显式 `sync_memory(memory_id)`。
- 生成 `embedded_text`。
...
它应与 `MemorySearchIndexSync` 并列,而不是把所有逻辑塞进现有 FTS sync。
...
class SemanticReranker(Protocol):
    ...
    def rerank(
        self,
        query: str,
        candidates: Sequence[MemoryRerankCandidate],
        *,
        limit: int,
    ) -> list[MemoryRerankScore]: ...
```

reranker 不存储任何东西。它只在 query time 计算 `(query, candidate_text)` 的相关性。
...
实现时建议把 wiring 形状直接写死成:

```python
tokenizer = build_tokenizer(settings.retrieval.tokenizer)
embedding_provider = build_embedding_provider(settings.retrieval.embedding)
rerank_provider = build_rerank_provider(settings.retrieval.rerank)

sparse_candidates = SparseCandidateService(...)
dense_candidates = DenseCandidateService(...)
recall_rerank = RecallRerankService(...)
...
- `HybridMemoryRecallService` 仍然是 runtime wiring 暴露给 host/tool 的唯一 recall entrypoint。
- `DenseCandidateService` 和 `RecallRerankService` 可以在某些部署中为 `None` / disabled,这样 recall 自动降级回 sparse-only。
- settings 只负责 provider/backends 与 policy 开关,不直接表达 recall orchestration 细节。
- 未来若 governance relatedness 复用 dense retrieval,应复用 `DenseCandidateService`,不要旁路再造一条 embedding 查询链。
...
new candidate statement
  -> embed immediately
  -> vector ANN over active same-scope same-type memories
  -> optional rerank
  -> related_existing_memories
```

这个路径不需要 candidate 入库,也不需要 scene summary。它先解决 `egg tart / dan tat` 这种 subject recall hole。
...
### PR A:Schema 与 test substrate 准备

目标:让 Postgres 成为 semantic recall 的唯一运行 substrate。

改动:

- 启用/验证 pgvector extension。
- 新增 `memory_vector_index` schema。
- 新增 schema tests。
- 保留 in-memory 仅作纯逻辑 test double,不得用于 recall / governance relatedness。

测试:

- schema 包含 vector table / FK / model key。
- 删除 memory node 后 vector row cascade。
- 不把 embedding 字段写进 `graph_documents.payload`。

### PR B:provider 层与 vector index sync

目标:能对 canonical memory nodes 建向量索引。

改动:

- `Tokenizer` / `EmbeddingProvider` / `RerankProvider` 维持为 retrieval-side provider 层。
- `EmbeddingProvider` protocol。
- deterministic fake provider for tests。
- `MemoryVectorIndexSync.sync_memory/rebuild/consume_outbox`。
- `embedded_text` builder。
- hash skip。
...
- sync single memory。
- rebuild graph。
- outbox consume 后 vector row 存在。
- statement 不变时 hash skip。
- model_id 不同可并存。

### PR C:按路线 B 拆 recall orchestration

目标:不把 vector 通道直接塞回旧单类,而是把 recall core 拆成四层 service。

改动:

- `SparseCandidateService`。
- `DenseCandidateService`。
- `RecallRerankService` 骨架(本 PR 可先不启用真实 rerank)。
- `HybridMemoryRecallService` 作为新的外层 orchestrator。
- 旧 `LexicalMemoryRecallService` 可短暂保留为 compatibility shim,但应尽快删除,不要长期双栈。
- RRF channels 加 `("vector", vector)`。
- `why` / trace 标明 channel 与 model。

测试:

- 同义查询仅 vector 命中时返回 memory。
- lexical/FTS/vector 多路融合排序稳定。
- vector backend unavailable 时降级为 lexical+FTS,并写 structured warning。
- CHEAP_AUTO 仍有 recent suppression。
- EXPLICIT_SEARCH 不做近期抑制或按现有策略。

### PR D:Reranker 接入显式搜索

目标:提高 `memory_search` top-k 精度。

改动:

- `RecallRerankService` 从骨架变成真实 provider-backed service。
- fake reranker tests。
- EXPLICIT_SEARCH 默认对 fused top-M rerank。
- CHEAP_AUTO 默认不开,或只在很小 top-M 开。

测试:

- reranker 能重排 fused candidates。
- reranker failure 不让 recall 整体失败,而是 structured warning + fallback fused order。
- trace 记录 reranker model / reranked ids。

### PR E:Governance relatedness 替换 token overlap

目标:删除 §6.6 stopgap 的核心债务。

改动:

- governance engine 注入 semantic relatedness service。
- 对新 candidate statement 即时 embed。
- 查 active same-scope same-type existing memories。
- 返回 `related_existing_memories` advisory。
- 保留 executor target validation。

测试:

- "egg tart" / "dan tat" 能成为 related。
- false positive 不能机械 supersede;仍需 Flash 选择 target + executor 复核。
- Postgres-only test;in-memory governance tests 只测推理逻辑,不测 relatedness substrate。

### PR F:Scene index projection
...
### PR G:Agentic search ergonomics
...
- UI/worker 提供 index lag diagnostics。
...
1. `memory_vector_index` schema。
2. `EmbeddingProvider` + `MemoryVectorIndexSync`。
3. 按路线 B 拆出 `SparseCandidateService / DenseCandidateService / HybridMemoryRecallService`,并先接入 vector channel。

这三步能直接补 Pulsara 最大短板:语义召回。
...
原因是它们都依赖 semantic recall 地基。先把向量召回做稳,再看真实 dogfood 中多跳/subject drift 是否仍咬人,再引入 scene 层。
