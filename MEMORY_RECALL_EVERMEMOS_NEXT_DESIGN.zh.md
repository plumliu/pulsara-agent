...
### PR B:Embedding provider 与 vector index sync
...
### PR C:Vector candidates 接入 recall

目标:recall 从 lexical+FTS 扩展到 lexical+FTS+vector。

改动:

- 新增 vector query channel。
- `LexicalMemoryRecallService` 可重命名为 `HybridMemoryRecallService`,或先保持类名但内部加入 vector。
- RRF channels 加 `("vector", vector)`.
- `why` 标 vector model。
- trace 记录各 channel ids。
...
3. recall 加 vector channel,保留 lexical/FTS。
