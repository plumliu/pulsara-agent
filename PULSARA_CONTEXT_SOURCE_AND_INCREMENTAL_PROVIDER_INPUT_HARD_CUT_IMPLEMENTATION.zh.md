...
>
> **ROAC supersession（2026-07-20）：** runtime-owned non-root source的最终wire carrier、replacement semantic no-op/exact head、clock append-once、source lifecycle与
> observation Long-Horizon rewrite由`PULSARA_RUNTIME_OBSERVATION_AND_AUXILIARY_CONTEXT_PREFIX_CONTINUITY_HARD_CUT_IMPLEMENTATION.zh.md`收紧。尤其：
> `working_context|mixed`不进入memory source；runtime request与observation分离；standalone auxiliary rebase已删除。
...
| Runtime environment | durable environment fact | generation root | semantic变化 typed root rollover |
| Runtime clock proposal | frozen compile clock | generation-neutral observation | Provider planner按committed clock head决定是否追加 |
| Memory projection | canonical recalled-memory projection | replacement snapshot | 相同no-op；变化完整replacement；空集合explicit empty |
| Capability prose catalog | exposure projection | replacement snapshot | prose相同no-op、变化完整replacement；tool schema变化仍rollover |
| Active skill | 完整 capability active-skill snapshot | replacement snapshot | 相同集合no-op；变化追加完整snapshot；空集合显式empty replacement |
| Plan status | durable plan workflow revision/terminal | replacement snapshot | active状态更新；退出时显式terminal snapshot |
| Plan guidance | durable plan revision | causal append once | 每个新guidance事实只追加一次 |
| Recovery | recovery event/fact | append once | 每个恢复事实一次 |
| Rollout status | durable rollout account | replacement snapshot | status update；exhausted时typed terminal |
...
| MCP diagnostic | installed snapshot diagnostic | replacement snapshot | diagnostic update/typed terminal；tool schema变化另触发rollover |
| Workspace skill | durable active-skill projection | replacement snapshot | 相同no-op；变化或empty追加replacement |

如果某 source 的旧值与新值共存会产生安全风险或无法用 typed supersession表达，该 source必须使用 generation root/rollover，不得自行删除旧值。

### 9.1 Historical source disposition

Candidate collection、compiler allocation与既有provider prefix之间不得以“本轮candidate是否存在”
传递删除语义。Compiler必须在allocation完成后，为generation内每个historical replacement
source head及本轮相关candidate冻结一个typed disposition：

```text
retain | replace | explicit_empty | terminal | rewrite_required
```

- `projection_failed` 只能retain已存在的head；首次失败不创建head；
- source exit必须由typed empty/terminal candidate表达，不能靠absence；
- semantic-identical candidate是retain/no-op，不追加新unit；
- changed optional candidate若被budget allocation省略，而旧head已不能代表当前truth，则产生
  `rewrite_required/allocation_omitted`；
- `rewrite_required`只允许通过typed source-disposition rollover删除旧head，普通append禁止删除；
- fresh、从未提交过且被省略的candidate只留下allocation audit，不产生phantom rollover。

Provider planner必须消费这张表并拒绝任何没有disposition的historical head；不得忽略本轮
selected candidates后无条件复制全部旧head。Rollover authority与durable reducer分别在commit
前后验证exact disposition set和predecessor head fingerprints。
...
class ProviderVisibleInputCompatibilityFact(FrozenFactBase):
    ...
    provider_user_carrier_protocol_fingerprint: str
    semantic_fingerprint: str
...
Global source registry与historical decoder/source/lowering bindings都不进入generation compatibility。它们属于replay attribution，而且只覆盖该generation实际引用的schemas/contracts。例外是**真实改变provider wire grammar**的closed user-carrier protocol：human/runtime-request/runtime-observation kind、producer、typed payload、codec与root interpretation的组合fingerprint必须进入上述provider-visible compatibility，协议变化必须rollover：
...
`unknown`与泛化budget rollover不得作为production正常reason。无法分类的compatibility drift为contract mismatch。`SOURCE_DISPOSITION_REWRITE_REQUIRED`只接受第9.1节完整disposition set及exact predecessor-head authority；它不能替代Long-Horizon transcript rewrite。
