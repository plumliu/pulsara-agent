...
### 7.4 Round 9已激活；Round 9.1仍为规划边界
...
该能力族按两轮处理；Round 9已经ACTIVATED，Round 9.1仍为DRAFT/NOT ACTIVATED：
...
- [Round 9.1 Agent Skills Standard](ROUND_9_1_AGENT_SKILLS_STANDARD_IMPLEMENTATION_SPEC.zh.md)：在Round 7.1与Round 9激活后，以Agent Skills开放标准替换Pulsara legacy Skill manifest；只保留Round 9已冻结的四个physical roots（workspace/user各自的`.pulsara/skills`与`.agents/skills`），由Skill owner执行一次global precedence scan，并向registry提交一个聚合`LOCAL_SKILL_CATALOG` snapshot。该轮负责metadata-first progressive disclosure与ordinary `read_file`契约；真实Builtin/MCP/Terminal调用继续由各自owner判断。Round 5B只消费其已安装normal source，不反向建立loaded-state。
