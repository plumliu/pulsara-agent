...
Plugin不进入上述两轮。[Round 9.2 Agent Plugin bundle与Hook lifecycle](ROUND_9_2_AGENT_PLUGIN_BUNDLE_AND_HOOK_LIFECYCLE_IMPLEMENTATION_SPEC.zh.md)把Plugin建模为Agent Plugins 1.0 portable core与Codex-first/Claude-overlap宿主适配层：启用后贡献MCP definitions、Skill roots与non-managed command Hooks，不增加第三套tool executor；同时只保留一个`DEFERRED_PENDING_PHC10`的Subagent-spec package inventory，不提前实现层次化/批量调度。Hook使用独立11项`HookEventType`（内部名称带`Event`后缀），不复用Committed/Live event，不拥有repository append或event-replay authority。Round 5B继续延期，并只消费Round 9/9.1/9.2已冻结事实做cold rebase，不再拥有正常epoch的MCP meta gateway或第二套Hook engine。
