...
2026-09-12 队列引导 UX 冻结：PR04 规定运行中的 composer 输入先成为 canonical NEW_TURN 队列项，再由用户对 exact 项选择发送为当前轮引导、编辑回 composer 或删除。发送必须由 kernel 单事务把 source 取消并创建 replacement steer，与 ROOT settlement 形成唯一赢家；服务端接纳后 UI 可立即显示“引导”，但不宣称模型已读。未操作项继续由既有 FIFO owner 在 loop 结束后逐条处理。该规格吸收 Codex 同 turn/multi-round 与 PendingSteer 的优点，同时明确不复制其 accepted steer 可能落成 leftover input 的窄竞态。2026-09-12 已按 Phase A–D 完成实施与 activation：单事务 queue action、严格协议、composer 三项操作、即时蓝色引导和旧直达路径删除均完成。真实浏览器还复现并修复 reload 后 queue action 错用旧 live ROOT 的问题，目标唯一取当前 canonical RUNNING ROOT。最终 Python full 1758、focused 293、前端 full 214；final wheel/launcher、41 次真实 OpenRouter 调用、wait/final/settlement race、edit/delete/FIFO、lost ACK/reload/observer/窄屏验收完成。详见 [PR04 activation evidence](output/playwright/pr04-dogfood/activation-evidence.md)。不改变 PR01、PR03 或其他批次的独立状态。

2026-09-12 审查后修正：当前工作树已修复 PR04 的三个 UI/runtime finding，以及 Subagent-R1 的 canonical ROOT 顺序在任务清单合并时丢失的问题。新增七项回归，focused 154、前端 full 221、lint/typecheck/build、协议生成和 diff check 通过；不修改 kernel、数据库、provider prefix 或保存配置。新入口为 `index-DkOQlHP2.js` / `index-Cryz142G.css`。原 PR04/Subagent-R1 activation 记录仍作历史保留，本轮未重验 Python full、continuity、isolated wheel 与真实 provider/browser，故当前状态以上表和各规格的最新修订节为准，没有 stage/commit。
