...
- `DurableMemoryHooks` 与 `ReflectiveMemoryHooks` **都不 override `project()`**（[durable_hooks.py:33,84](src/pulsara_agent/memory/durable_hooks.py:33)），所以无论哪种 wiring，projection 每轮空转。
...
- `ExecutionEvidenceLedger`（[ledger.py:27](src/pulsara_agent/memory/ledger.py:27)）的 `submit_*` 全是**创建**：`gate.evaluate_*` → `_require_existing_nodes(evidence)` → `graph.put_jsonld(node)` → 逐条 evidence `_add_relation(evidence_id, memory.SUPPORTS, node_id)`。**没有任何更新已存在节点 / 状态转换的方法。**
- `_add_relation`（[ledger.py:427](src/pulsara_agent/memory/ledger.py:427)）= `get_jsonld` + 改 dict + `put_jsonld`，是**独立于节点写的第二次 HTTP mutation**。
- gate 产出三种创建期状态：`ACTIVE` / `NEEDS_REVIEW` / `REJECTED`（[write_gate.py:45](src/pulsara_agent/memory/write_gate.py:45)，`WriteDecision(accepted, status, reason, confidence_level)`）。
- **REJECTED 节点仍然落图**（gate-reject ≠ 不写）：`submit_preference` 无条件 `put_jsonld(... status=decision.status ...)`（[ledger.py:245](src/pulsara_agent/memory/ledger.py:245)），测试钉死 [test_memory_write_service.py:120](tests/test_memory_write_service.py:120)（`graph.has_jsonld(result.memory_id)` 为真）与 [test_durable_memory_contract.py:284](tests/test_durable_memory_contract.py:284)。**只有** `ValidationError` / 缺 evidence 抛异常时才 `record=None` 且不写节点（[write_service.py:56](src/pulsara_agent/memory/write_service.py:56)，[test_memory_write_service.py:140](tests/test_memory_write_service.py:140)）。
...
- **`supersedes` / `contradicts` 边无生产者**：ledger 只写 `PROVIDES`（[ledger.py:179](src/pulsara_agent/memory/ledger.py:179)）和 `SUPPORTS`（[ledger.py:217,262,306,344,386](src/pulsara_agent/memory/ledger.py:217)）。`SUPERSEDES`/`CONTRADICTS` 仅存在于 ontology/codec。
- `governance.apply_decision`（[governance.py:46](src/pulsara_agent/memory/governance.py:46)）是**非原子四写**：`memory_write_service.submit`（写 graph）→ `event_log.extend`（写事件库）→ `_append_governance_candidate_if_needed`（写候选池）→ `_append_decision`（独立 `psycopg.connect` 写 `memory_governance_decisions`，[candidate_pool.py:296](src/pulsara_agent/memory/candidate_pool.py:296)）。无 2PC / outbox / 幂等键去重；只有 `governance_batch_id` 作关联键。
- `dedupe.already_exists`（[dedupe.py:56](src/pulsara_agent/memory/dedupe.py:56)）= `find_by_type` 扫描 + 比对 statement/scope + status 白名单 `{ACTIVE, NEEDS_REVIEW}`。
...
- Postgres 现有表：`sessions / runs / turns / agent_events / tool_execution_records / artifacts`（[postgres_schema.py:11](src/pulsara_agent/storage/postgres_schema.py:11)）+ 候选池的 `memory_candidates / memory_governance_decisions`（[candidate_pool.py:409](src/pulsara_agent/memory/candidate_pool.py:409)，FK 到 `sessions/runs/turns`）。**无** canonical memory 表。DSN：`settings.storage.postgres_dsn`，env `PULSARA_POSTGRES_DSN`（[settings.py:19](src/pulsara_agent/settings.py:19)）。
...
| 类别 | 过滤器 | 今天是否做实活 | 依据 |
|---|---|---|---|
| **承重（load-bearing）** | `status NOT IN (REJECTED)`、`status IN (ACTIVE[, NEEDS_REVIEW diagnostic])`、scope 可达、type 允许、非 projection-echo | **是**。REJECTED 行真实物化（§1.2），漏过会把被拒记忆当事实注入 | [dedupe.py:66](src/pulsara_agent/memory/dedupe.py:66) 的白名单正为此 |
...
**关键设计（修正）**：`GraphStore.put_jsonld` 是**通用 JSON-LD 存储接口**，`ExecutionEvidenceLedger` 用它写的不止 memory conclusion 节点，还有 ToolResult / Artifact / Evidence / Turn 等运行时节点（[ledger.py:35](src/pulsara_agent/memory/ledger.py:35)）。一张 memory-specific 的表（带 `memory_type/status/statement`）装不下这些。因此分两层：
...
**关系维护（修正）**：现有 `GraphStore` Protocol **没有** `add_relation`；`ExecutionEvidenceLedger._add_relation` 的真实做法是 `get_jsonld(source)` → 改 dict → `put_jsonld(document)`（[ledger.py:427](src/pulsara_agent/memory/ledger.py:427)），即**关系以谓词内联在 JSON-LD 文档里**回写。因此 `PostgresGraphStore.put_jsonld` **必须从文档解析出 relation 谓词，同步进 `memory_relations`**——否则 SUPPORTS/PROVIDES 永远不会进投影表，`CanonicalNodeView.evidence_ids/outgoing/incoming` 会失真。我们**不**新增 `add_relation` 到协议（保持协议不变、不改 ledger）。
...
2. **relation-sync 覆盖所有 `graph_documents` 文档，不只 5 类 memory 节点**。关键：`SUPPORTS` 的 source 是 evidence/runtime 节点（ledger 写的是 `evidence_id → supports → memory_id`，[ledger.py:216](src/pulsara_agent/memory/ledger.py:216)），而 evidence 文档不在 `memory_nodes` 投影里。若只对 5 类 memory 文档做 relation-sync，`CanonicalNodeView.evidence_ids` 会**永远为空**。因此 `_sync_relations_from_document` 对每个被 `put_jsonld` 的文档都执行，无论其 `@type` 是否属于 5 类。
...
【新增】`src/pulsara_agent/memory/query.py`。这是 R1 裁判源的程序化入口，**取代** FEASIBILITY 文档批评的"逐候选 SPARQL 扩展"。
...
| 0.2 | `PostgresGraphStore`（通用 JSON-LD 存储 + 投影同步）+ `MemoryQuery` | `graph/postgres.py`【新增】、`memory/query.py`【新增】 | InMemory 不能查 / N+1 / 关系失真 |
...
| 0.5 | `MemoryWriteUnitOfWork`：canonical 写 + decision + outbox 同事务 + 幂等键 | `memory/unit_of_work.py`【新增】、`memory/governance.py`【改造】、`memory/candidate_pool.py`【改造】、`storage/memory_schema.py` | split-brain |
...
【改造】[governance.py:46](src/pulsara_agent/memory/governance.py:46) 当前四个独立写无事务，且 `PostgresCandidatePool.append_decision` 自己 `psycopg.connect`（[candidate_pool.py:296](src/pulsara_agent/memory/candidate_pool.py:296)）——只改 `governance.py` 不够。
...
**核心：`MemoryWriteUnitOfWork`（共享 connection/transaction）**。【新增】`src/pulsara_agent/memory/unit_of_work.py`。问题根因是三个写入方各自开连接；解法是让它们绑定到同一 connection：
...
- 【关键改造】**`apply_decision` 必须调用 `uow.memory_write_service.submit(...)`，而不是 `self.memory_write_service.submit(...)`**。当前 `MemoryGovernanceExecutor` 持有 wiring 期预造好的 `self.memory_write_service`（其 ledger 绑的是旧 graph/旧连接，[governance.py:83](src/pulsara_agent/memory/governance.py:83)）；若沿用它，`submit()` 仍走旧连接，"同事务写"落空。`MemoryWriteService`/`ExecutionEvidenceLedger` 已是数据类（[ledger.py:27](src/pulsara_agent/memory/ledger.py:27)：`graph` 是字段），UoW 在 `__enter__` 时用 `self.graph` 现造一个等价 service，让 ledger 的 `put_jsonld`/`_add_relation` 全部走 UoW 的同一 connection。
- 【关键改造】**dedupe / `already_exists` 也必须使用 `uow.graph`**。当前 `apply_decision` 在写入前会跑 dedupe 检查（[governance.py:71](src/pulsara_agent/memory/governance.py:71)），`already_exists` 接受一个 `graph` 参数做 `find_by_type` 扫描（[dedupe.py:56](src/pulsara_agent/memory/dedupe.py:56)）。若它仍走 wiring 期的 `self.graph`，会出现"在旧连接上判重、在 UoW 新连接上写入"的读写错位——同一 batch 内刚写的节点判不到、或判重读到事务外的旧快照。因此 dedupe 必须 `already_exists(candidate, uow.graph, graph_id=...)`，与写入共用同一 connection/事务视图。
- 【改造】[governance.py:46](src/pulsara_agent/memory/governance.py:46) `apply_decision` 用 `with MemoryWriteUnitOfWork(dsn) as uow:` 包裹整段写入，内部一律用 `uow.*`（`uow.graph` 跑 dedupe / `uow.memory_write_service` / `uow.decisions` / `uow.outbox` / `uow.lifecycle`），不再触碰 `self.memory_write_service` 或 `self.graph`。
...
3. **reconciliation job**【新增】`memory/reconcile.py`：扫 `memory_write_outbox WHERE status='pending' AND created_at < now()-interval` 重放/修复；扫"有节点但无 decision 行"的损坏态。
...
| # | 任务 | 目标文件 | 类型 |
|---|---|---|---|
| 1.1 | `MemoryRecallService` 协议 + 默认实现 | `memory/recall.py`【新增】 | 核心 |
| 1.2 | lexical exact channel | `memory/recall.py` | 核心 |
| 1.3 | BM25/FTS channel（v1 朴素 lexical 起步） | `memory/recall.py` | 核心 |
| 1.4 | bounded union + RRF 融合 | `memory/recall.py` | 核心 |
| 1.5 | canonical filter（承重类，§2.3） | `memory/recall.py` | 核心 |
| 1.6 | `project()` 真正实现 | `memory/durable_hooks.py`【改造】 | 接线 |
| 1.7 | projection builder（fenced + 结构化） | `memory/projection.py`【新增】、`runtime/context.py`【改造】 | 接线 |
| 1.8 | `memory_search` / `memory_get` 工具 | `tools/builtins/memory_query.py`【新增】、`registry.py`【改造】 | 工具 |
| 1.9 | `do_not_write_back` 代码护栏 | `memory/projection_ledger.py`【新增】、`durable_hooks.py`【改造】 | 护栏 |

### 5.1 `MemoryRecallService`（任务 1.1）

【新增】`src/pulsara_agent/memory/recall.py`：
...
【改造】[durable_hooks.py:33](src/pulsara_agent/memory/durable_hooks.py:33)：`DurableMemoryHooks` 增加 `recall: MemoryRecallService | None` 和 `projector: ProjectionBuilder` 依赖，override：
...
【新增】`src/pulsara_agent/memory/projection.py`，产出 §1.1 fence 兼容的 dict（保留 `summary`/`items` 键以兼容 [context.py:177](src/pulsara_agent/runtime/context.py:177)，但 `items` 升级为结构化）：
...
【新增】`src/pulsara_agent/memory/projection_ledger.py`。prompt 提示不够，必须有代码校验。
...
【改造】[durable_hooks.py](src/pulsara_agent/memory/durable_hooks.py) 的 `_drain_to_pool` / `_finalize_invalid_to_pool`：候选入池前检查 `projection_ledger.is_echo(candidate.statement, state)`，命中则拒绝或标 `skip_reason="projection_echo"`，不入池。**v1 必做**——否则系统会把召回内容当新证据自我增殖。
...
| 1.5.2 | index 写后增量同步（outbox 驱动） | `memory/index_sync.py`【新增】 | 同步 |
| 1.5.3 | rebuild job（全量从 canonical 重建） | `memory/index_sync.py` | 重建 |
| 1.5.4 | `recall_traces` / `recall_usages` 表 + 写入 | `storage/memory_schema.py`、`memory/recall.py`【改造】 | 追踪 |
| 1.5.5 | recent-recall suppression | `memory/recall.py`【改造】 | 去重 |
| 1.5.6 | structured unavailable + cooldown | `memory/recall.py`【改造】 | 降级 |
...
| 2.2 | **lifecycle producer**：状态转换 + 双向边 + 事件 | `memory/lifecycle.py`【新增】、`governance.py`【改造】 | 前置·核心 |
| 2.3 | structured ActionBoundary trigger 字段 | `entities/memory/action_boundary.py`【改造】、`write_gate.py`【改造】 | 本体 |
| 2.4 | direct-relation reranker（进程内） | `memory/rerank.py`【新增】 | 差异化 |
| 2.5 | grounded explainer（结构化 claim） | `memory/explain.py`【新增】 | 差异化 |
...
【新增】`src/pulsara_agent/memory/lifecycle.py`。这是把"边 + 状态"同时落地、且**多节点原子**的唯一入口。
...
- **接入 governance（W1b 入口）**：【改造】[governance.py:46](src/pulsara_agent/memory/governance.py:46)，当治理判定一个新节点替代旧节点时，在 UoW 内调 `lifecycle.supersede(...)`，而非只写新节点。
...
【新增】`src/pulsara_agent/memory/rerank.py`。在 §3.4 `CanonicalNodeView`（已含直接 in/out 边）上做，**进程内、零额外 HTTP**（借鉴 scope-recall-hermes 的 in-memory hop 权重）：
...
【新增】`src/pulsara_agent/memory/explain.py`。**解释必须机器可校验**，否则 confab=0 进不了 CI。
...
| hybrid RRF（lexical+BM25+vector） | `memory/recall.py`【改造】 | 三通道 RRF 在 eval 上不跌破 floor、不增无关召回 |
| optional MMR 多样性 | `memory/rerank.py`【改造】 | 默认关闭；开启需证明对复杂 query 有可感知收益 |
...
### 10.1 召回链路类型（`memory/recall.py`、`memory/query.py`）
...
### 10.2 解释契约（`memory/explain.py`，守 §2.4）
...
### 10.3 projection dict（`memory/projection.py` → `runtime/context.py`）
...
原子性：MemoryWriteUnitOfWork（memory/unit_of_work.py）绑定单 connection，暴露：
...
src/pulsara_agent/memory/query.py               MemoryQuery + CanonicalNodeView（fetch_nodes/lexical/fts，均带 graph_id）
src/pulsara_agent/memory/unit_of_work.py        MemoryWriteUnitOfWork + CandidateDecisionRepository + OutboxRepository（共享 connection；暴露绑定 graph 的 memory_write_service/lifecycle）
src/pulsara_agent/memory/recall.py              MemoryRecallService + LexicalMemoryRecallService + RecallTrigger/RecallStatus
src/pulsara_agent/memory/projection.py          ProjectionBuilder（结构化 + fenced）
src/pulsara_agent/memory/projection_ledger.py   ProjectionLedger（do_not_write_back 护栏）
src/pulsara_agent/memory/index_sync.py          增量同步 + rebuild（Phase 1.5）
src/pulsara_agent/memory/reconcile.py           outbox 重放 + damaged-node 检测（Phase 0/1.5）
src/pulsara_agent/memory/lifecycle.py           MemoryLifecycle（supersede/contradict/stale，Phase 2，走 W1b；关系只经投影 helper）
src/pulsara_agent/memory/rerank.py              direct-relation reranker（Phase 2）
src/pulsara_agent/memory/explain.py             grounded explainer + ClaimKind + 校验器（Phase 2）
...
src/pulsara_agent/memory/durable_hooks.py   project() 真正实现；_drain_to_pool 加 echo 护栏（:33,:57）
src/pulsara_agent/memory/governance.py      apply_decision 用 MemoryWriteUnitOfWork 单事务；接 lifecycle（:46）
src/pulsara_agent/memory/candidate_pool.py  append_decision 逻辑抽到 CandidateDecisionRepository（可绑定外部 connection）（:296）
...
src/pulsara_agent/memory/write_gate.py      ActionBoundary trigger 校验（Phase 2）
