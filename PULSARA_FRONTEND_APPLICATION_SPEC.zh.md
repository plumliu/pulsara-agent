...
已连接、可编辑且尚无消息的新会话使用居中的首屏：简短标题与圆角输入框，不展示品牌标识或框下提示。
首屏默认只显示输入区、发送按钮和小型“输入选项”入口；模型、推理、Skills、规划与权限
沿用已有控件，按需展开。模型未配置时仍默认收起，以原有颜色的轻微呼吸效果提示选项入口；
展开后改为提示模型选择，打开模型菜单或选好模型后停止提示。直接发送未配置模型的草稿时，
展开选项与模型菜单，保留草稿、不提交、不离开首屏；选择模型后由用户再次发送。
reduced-motion 下不播放呼吸效果。已有历史、继承摘要、运行中或
中断的会话使用正常工作台，不重新播放首屏过渡。

首次有效提交时，标题淡出，同一个输入框平滑移到底部，控制栏从框内向下展开。
动画不创建第二份编辑器或提交路径；草稿清除仍以提交接纳为准，发送失败保留草稿，
Enter 提交保留输入焦点。过渡状态仅属于当前页面；reduced-motion 下直接切换布局。

空会话的“压缩上下文”按钮置灰，未发送草稿或待接纳输入不算可压缩内容；已有消息、继承摘要、
压缩上下文或运行中的轮次仍按既有流程压缩。离线时不可发起压缩。Kernel 沿用现有入口判断：
无活动轮次且无已结束轮次时返回 `NOT_NEEDED / NO_TERMINAL_TURN`，不接纳压缩命令、不调用摘要模型。
...
- composer 使用单一 Tiptap/ProseMirror 图文文档，图片只从实际文件拖入或 paste 事件中的
  ...
  文本处理，不自动抓取；
- 空闲且没有待处理输入时，本页新提交的第一条输入立即显示为用户消息；发送中、等待开始及失败状态
  显示在气泡底部，不经过排队卡片。此位置在提交时确定，后续开始运行也不退回队列；正式用户
  消息按同一 command identity 替换临时展示，不重复显示。仅是页面临时表现，底层仍走既有提交与排队路径；
- 运行中或已有等待输入时的新提交继续显示 pending queue，在 current turn 完成前保持可见，
  不混入 active assistant draft；
- 普通用户输入以右侧独立说话者块呈现，使用用户图标与“你”标识；运行中的显式引导则以
  紧凑的“引导”批注嵌入当前执行流，不伪装成一轮新的普通对话；
  引导沿用界面的纸色、暖灰文字与低饱和暖色细边，不使用蓝色大色块。
  展开与收起共用同一外观及正文排版；收起过程时仅平滑缩小引导卡片的纵向内外边距，
  不截断用户原文，也不隐藏图片入口。明暗主题使用既有颜色变量，并遵循减少动态效果偏好；
...
- 中间正文、可见思考、工具与子任务活动按原顺序显示；运行时过程区展开，完整最终回复被确认后
  自动收起本轮中间过程。最终回复始终留在外面，与展开后的过程使用连续的正文布局，不新增
  气泡、分隔卡或重复说话者标题。仅有最终正文、没有中间内容时不显示空折叠入口；
- 历史过程默认折叠，用户可点击“处理过程”展开，手动选择不被普通刷新数据覆盖；重新打开会话
  则恢复默认。失败或中断不得伪装成最终回答，重新加载后仍可展开已有过程。用户的 steer 与
  其他输入，以及上下文压缩分界，始终可见。定位历史消息或子任务时先展开包含目标的过程；
- 自动折叠只消费已有完成事实：ROOT 的最终回复沿用 `fork_eligible` 派生资格，不能把每条
  `ASSISTANT_MESSAGE`、非空流式正文或一次工具完成当作整轮结束；子任务对话沿用已有结果投影。
  最终回复自己的可见思考也归入过程，正文、复制与分叉操作留在外面。折叠状态只存在前端，
  不写数据库、不改变消息顺序或模型输入，也不根据只有时分的展示时间虚构耗时；
...
- ordinary `read_file` 的 exact path 若与当前 effective Skill catalog 中某个 `SKILL.md` 的 canonical
  path 或 catalog location 完全相同，tool trace 将“读取文件”改写为“使用 `<name>` 技能”。
  ...
  任一可见语义边界都必须断开轨道；
- 内置工具默认摘要使用自然语言操作名与简短的对象、结果描述；不直接展示函数名、内部标识、
  JSON、stdout 或工具正文。路径、搜索词、命令等用户可理解的操作对象可简短展示。
  只根据工具实际返回的状态和计数描述结果；调用返回不等于后台命令或子任务已结束，
  命令卡片记录当次动作：启动返回显示“已执行命令”，读取输出显示“已读取输出”，
  等待调用返回但进程未结束显示“已等待命令”，确认正常结束才显示“命令执行完成”；
  不把历史卡片写成持续更新的“命令仍在运行”。失败、超时和停止仍明确展示。
  消息排队不等于对方已读取，记忆提交审核不等于已保存，配置保存不等于已全部生效。
  正常工具摘要优先呈现动作记录，例如“已等待子任务”“已发送补充信息”“已提交记忆”，
  不附加“后续可查看”“仍可继续”“尚未确认保存”等要求用户理解内部流程的说明。
  这些文案只表达等待、发送或提交动作，不宣称子任务已完成、消息已读或记忆已永久保存。
  缺少结果的历史操作标为“未记录到操作结果”，不得写成已完成或确定已取消。
  工具失败说明发生了什么，不把重读文件、调整搜索范围等模型可自行处理的步骤交给用户。
  真正需要用户选择或批准的提示继续保留。
  常见失败用自然语言说明；未识别的失败保留失败状态，精确诊断仍可通过原始结果开关查看。
  主对话、内联子任务和任务详情共用同一摘要；MCP direct 与 MCP meta 工具保持既有展示。
- 设置 → 通用提供“显示内置工具原始结果”开关，默认关闭，选择只保存在当前浏览器。
  关闭时，主对话、内联子任务及任务详情均不展示内置工具的 raw result、正文型 result summary、
  artifact 预览或分页入口；保留工具名称、执行状态、命令、图片、文件差异及 MCP 结构化服务信息。
  没有其他可展示内容的工具卡片不提供空展开入口。打开后恢复精确原文、复制及完整输出分页；
  超出 inline projection 的完整输出仍通过 `ReadContentRequest` / artifact path 按需读取。
  外部 MCP 工具（`mcp__<server>__<tool>` 及 `use_new_mcp_tool` 返回的外部结果）保持原展示行为。
  该开关不改变工具执行、结果保存、artifact 生成、模型输入或 provider 前缀。
