...
本文面向 `PULSARA_NEXT_MAINLINES_ROADMAP.zh.md` 中的「主线三：Unified Capability Surface」。这里的 capability / capacity 指同一类问题：模型可发现、可调用、可授权、可观测的一切能力面，包括内置工具、workflow 工具、memory 工具、本地 skill、skill-guided CLI/terminal usage、未来 MCP server tool，以及更远期的 plugin / bundle。
...
- provider adapter：builtin / workflow / memory / local skill / MCP 各自接入统一 descriptor；CLI v1 先作为 skill-guided terminal usage 的可观测能力，而不是默认 typed tool provider。MCP 与 CLI 不同：MCP server 本身会产出真实 typed tools，但必须经 server config、client lifecycle、tool discovery、exposure/gate 统一管理。
...
- built-in tools
- skills
- CLI skills / terminal-backed workflows
- MCP tools
- workflow tools
...
- MCP tools 一多，直接广告会污染 schema 和上下文；CLI recipes 一多则会污染 skill prompt，因此两者都需要 exposure/summary 策略，但不能混成同一种 typed provider。
...
### 3.5 MCP 配置：stdio 本地服务与 streamable HTTP 远端服务

Codex 的 MCP 配置不是「skill 里告诉模型怎么敲命令」这一层，而是 runtime 直接管理 MCP server：

- 本地 stdio server：`command / args / env / env_vars / cwd`。
- 远端 streamable HTTP server：`url / bearer_token_env_var / http_headers / env_http_headers`。
- 通用控制：`enabled / required / startup_timeout_sec / tool_timeout_sec / supports_parallel_tool_calls`。
- 暴露控制：`enabled_tools / disabled_tools`。
- 授权控制：`default_tools_approval_mode` 和 per-tool `tools` approval。
- OAuth：`scopes / oauth / oauth_resource`。
- 环境归属：`environment_id`，并且 remote stdio 要求绝对 `cwd`，说明「server 在哪里运行」是配置契约的一部分。

Codex 的连接管理也证明 MCP 是有生命周期的 runtime resource：

- `McpConnectionManager` 持有 server clients、startup 状态、metadata、tool provenance、elicitation manager。
- stdio launcher 明确区分 local child process 与 executor process placement，并负责进程组终止。
- streamable HTTP 初始化有 retry / timeout / auth failure 分类。

可借鉴点：

- Pulsara MCP provider 未来必须先读配置/manifest，再启动或连接 server；不能把 MCP tool 当成裸 Python function。
- MCP server config 是 capability provider 的输入；MCP tool descriptor 是 discovery 结果；两者都要进 inspector。
- 本地 stdio MCP 与远端 HTTP MCP 都是真实 MCP tool surface，但生命周期、认证、失败语义不同，应在 descriptor/provenance/health 里可见。
- `required=true` 类语义要有明确启动失败行为：是降级隐藏，还是 host/session fail-fast。

要避免：

- 不要把 MCP 当成 CLI skill：MCP 已经提供 typed schema 和 protocol result，不应让模型通过 terminal 手写 JSON-RPC。
- 也不要把 CLI 当成 MCP：普通 CLI 没有统一 schema / elicitation / tool discovery，仍应走 skill-guided terminal。
- 不要每轮用户消息都重新连接所有 MCP server；health/tool snapshot 需要 TTL / generation / cached diagnostics。
...
### 4.5 MCP：配置作用域 + 多 transport + MCPTool 代理

Claude Code 的 MCP 配置更接近一个完整产品面：

- 配置 scope：`local / user / project / dynamic / enterprise / claudeai / managed`。
- 本地 stdio：`command / args / env`。
- 远端：`sse / http / ws`，支持 headers / headersHelper / OAuth。
- 内部 transport：IDE SSE/WS、SDK、claude.ai proxy。
- server 状态：connected / failed / needs-auth / pending / disabled。
- tool serialization：MCP tool 保留 `isMcp`、`originalToolName`、normalized name mapping。
- CLI 配置入口：`claude mcp add --transport http|sse|stdio ...`，写入 `.mcp.json` 或对应 scope config。

执行上，Claude Code 使用稳定 `MCPTool` 代理：基础 tool schema 是 passthrough，真正的 name、description、prompt、call、permission 由 MCP client 层按 server tool 动态覆盖。MCP tool 名称也会做脱敏/归一化处理，例如 telemetry 中把 `mcp__server__tool` 视作可能包含用户配置的 PII。

可借鉴点：

- MCP provider 要提供配置管理、连接状态、tool discovery 和 tool adapter 四个面；缺一不可。
- MCP tool name 需要 namespace / normalization / collision 策略，不能直接塞全局 flat name。
- MCP server/tool 名称可能泄露用户私有配置，event / telemetry / inspector 应分清「本地调试可见」和「可上传遥测」。
- 远端 MCP 的 auth / needs-auth 是 availability 状态，不是普通 tool execution error。

要避免：

- 不要把 MCP adapter 做成一个巨大 Tool object，把 UI、permission、client lifecycle、schema mapping 全塞进去。Pulsara 应保持 descriptor / provider owner / execution adapter / inspector projection 分层。
...
这和 Pulsara 记忆召回/async provider 生命周期评审是同一个坑：MCP/async tool adapter 不能随便在 worker thread 里新建 loop 共享 async client；CLI 侧则应继续通过 terminal process 边界承载，不把 CLI 子命令伪装成 async client tool。
...
- MCP/plugin adapter 引入后，必须在文档中把 trust boundary 说清楚；CLI skill-guided terminal usage 也必须继续继承 terminal 的 OS/process 边界。不要暗示 descriptor 能提供进程级隔离。
...
- Pulsara 的未来 MCP/plugin provider 应优先读静态 descriptor / manifest，再决定是否加载代码或连接 server；CLI 侧优先读取 skill metadata / docs，再由 terminal 执行。
...
| CLI | 多数通过 shell/exec + skill 指导 | BashTool 是主路径，SkillTool 加载说明 | skill 文档常直接写 terminal 序列 | exec 是核心，plugin tool 是补充 | 做 skill/docs/prompt guidance + terminal/exec/shell tool 的 capability 化，不做 typed CLI provider |
...
Pulsara 未来 MCP / plugin provider 一定会遇到工具数量膨胀；CLI recipes 如果被错误地逐条 typed 化，也会遇到同样问题。应该提前设计：
...
这能让 MCP provider 不必每轮重新连接/探测，也能让 CLI binary health / skill diagnostics 不必每轮重复探测，并让 inspector 解释 stale/degraded 状态。
...
工具名 allowlist 是最后 fallback，不应是主机制。否则 MCP/plugin 一进来，重名、别名、shadowing、namespace 都会让 permission 失真；CLI 侧若把 subcommand 错误 typed 化，也会制造同类漂移。
...
### 9.5 CLI v1 应先是 skill-guided terminal usage

本地 agent 产品的主流形态不是把全局 CLI 大量封装成 typed tools，而是：
...
也就是模型读说明书，然后在 terminal 中调用用户已安装的 `hf`、`gh`、`docker`、`npm`、`uv` 等 CLI。Pulsara v1 应优先把这种路径纳入 capability surface：

- skill 可以声明 suggested / required terminal usage。
- active skill 与 terminal tool call 应能在 inspector 中关联。
- permission gate 仍以 terminal/exec 为硬边界。
- terminal output 的 artifact、timeout、cwd/env、retained process 语义仍由 terminal tool 统一处理。

typed CLI provider / adapter 不属于当前路线。CLI 的默认形态就是开放世界 shell，硬封成大量 tool schema 会损失灵活性并复制 terminal 风险面。

skill-guided terminal 的 CLI 操作最容易产生：
...
因此 CLI 相关 capability 至少必须让系统解释：

- 哪个 active skill / recipe 指导了 terminal 调用。
- terminal 调用的 cwd/env/timeout/artifact policy。
- 是否需要网络、登录、外部 binary。
- read-only / mutating / open-world 风险提示。
- max output
- artifact policy
- interactive policy
...
class CapabilityDescriptor:
    ...
    name: str
    namespace: str | None
    provider_kind: Literal["builtin", "workflow", "memory", "skill", "mcp"]
    provider_id: str
    version: str | None
    description: str
...
- CLI recipe / skill catalog：读取 skill metadata，产生 suggested terminal usage / required binary diagnostics；默认不产生命令 tool descriptor。
...
- 某 tool call 为什么被拒。
- 某输出为什么被 artifact。
- 某 MCP capability 或 CLI recipe / required binary 为什么 degraded。
- name 是否被 shadowed。
...
### 11.2 CLI：skill + terminal 是 v1 主路径

CLI v1 不应默认把每个全局 CLI 或 subcommand 注册成 typed tool。更符合 Codex / Claude Code / Hermes / OpenClaw 的路径是：

- skill 负责说明 CLI 的安装、认证、常用命令、验证方式和常见坑。
- terminal/exec 负责真实执行。
- capability surface 记录 skill catalog、active skill、suggested tools / risk hints、terminal gate decision、artifact 与 retained process。
- inspector 能解释「这次 terminal 调用是在某个 active skill 指导下发生的」。

typed CLI provider / adapter 不属于当前设计。若未来确有需求，应另起独立设计并证明它不会绕过 terminal hardline；本路线只支持 skill/docs/prompt guidance + terminal/exec/shell tool。
...
- MCP：由配置的 server 通过 protocol 暴露 typed tools，Pulsara runtime 负责连接、发现、授权和调用。
- CLI：由 skill/docs 指导模型通过 terminal 执行命令，Pulsara runtime 不为每个 subcommand 创造 typed tool。

Firecrawl 是很好的对照：当前本地 Firecrawl skill（如 `firecrawl-search`、`firecrawl-scrape`、`firecrawl-crawl`、`firecrawl-browser`）声明的是 `Bash(firecrawl *)` / `Bash(npx firecrawl *)`，也就是说明书 + terminal 路线；若同时安装 Firecrawl MCP server，那是另一条 MCP capability 路线。两者可以共存，但必须在 capability provenance 上分清楚。
...
MCP tool 的 read-only 默认 false，除非 server annotations / descriptor 显式证明；CLI terminal usage 仍按 terminal policy 判定，不按 CLI 名称推断只读。
...
1. CLI skill-guided terminal observability（active skill ↔ terminal call ↔ artifact / permission reason）。
2. MCP adapter。
