...
删除字段：`background`、`timeout_seconds`、`session_id`（别名）、`max_lifetime_seconds`。`additionalProperties:false` 保持。

> **2026-06-20 real-provider 修正**：`max_lifetime_seconds` 保留在 runtime `TerminalRequest` / `ProcessRegistry.exec_with_yield`，但不暴露给 LLM-facing `terminal` schema。两轮 real-LLM numeric materialization probe（`gpt-5.4-mini` 与 `gpt-5.4`）显示当前 OpenAI-compatible provider 会完整物化 closed schema 的 optional properties：有 default 的字段填 default；无 default 的 integer/string/bool 分别填 `0`/`""`/`false`（`workdir` 有时会被语义化为 `"."`）。因此 LLM-facing schema 里的每个 property 都必须假设会被传入。若暴露无 default 的 `max_lifetime_seconds`，它会稳定变成 `0`，重新制造隐式 server 铡刀。模型侧生命周期控制先用 `terminal_process.kill`；宿主/内部代码仍可直接走 runtime watchdog。
...
max_output = max_output_chars_arg(call.arguments)  # <=0 -> default; tiny positive -> sane floor
...
result = session.execute(TerminalRequest(
    ...
    tty=tty, max_output_chars=max_output,
...
### 5.3.1 runtime-only kill cap 错误文案

`max_lifetime_seconds` 不是旧字段，也不是曾经支持后废弃的模型参数。它是 runtime/host-only 能力。若模型或调用方直接把它塞进 `terminal` tool arguments，tool 层应单独返回：

```text
max_lifetime_seconds is runtime-only and is not model-facing; use terminal_process.kill to stop a yielded process
```

不要把它混进 `background` / `timeout_seconds` / `session_id` / `notify_on_complete` 的 "no longer supported" 错误里。这样能把「硬切旧协议」和「runtime-only 保护能力」区分开。

### 5.3.2 numeric materialization 防御

real provider 会把有 default 的 numeric optional 字段填成 default；当前 `yield_time_ms=10000`、`max_output_chars=20000` 因此是安全的。仍需对 `max_output_chars` 做 executor 侧保险：`<=0` fallback 到默认值，过小正数 floor 到合理下限（例如 512），避免未来 provider / model 变体把输出窗口物化成 `0` 或 `5` 后造成 terminal 全面 blocked 或几乎无输出。`yield_time_ms=0` 是合法「立即 yield」语义，不能简单 clamp。

同一条规则也适用于 `workdir`：real probe 显示无 default 的 `workdir` 会被物化成 `""` 或 `"."`。这两个值在语义上都应表示「模型没有显式指定 workdir，沿用 session current cwd」。因此 resolver 必须把 `"."` / `"./"` 和空字符串放进同一分支，不能把 `"."` 解释成 workspace root，否则会随机击穿 cwd persistence。

这条规则不是 terminal 独有。所有 LLM-facing numeric 参数都不能把 schema default 当成唯一安全网；executor 消费处应使用共享 bounded normalizer：`<=0` 回到业务默认值，过小正数按字段语义 floor，过大值 cap。`memory_search.limit=0` 必须回到默认 5，而不是被 `max(1, ...)` 压成 1；`terminal_process.max_output_chars` 与 `terminal.max_output_chars` 使用同一输出窗口规则；`terminal_process.timeout_seconds=0` 回到有限默认等待窗口，而不是 block 或无限等待。
...
改动：
1. [terminal.py:114](src/pulsara_agent/tools/builtins/terminal.py:114) 的 `if background and "timeout_seconds" in args` 从 `_blocked_result` 改为：忽略 `timeout_seconds`、照常走 background spawn、payload metadata 挂 `{"ignored_argument": "timeout_seconds", "reason": "background is unbounded; use terminal_process.wait or terminal_process.kill"}`。
...
改动：
1. `TerminalRequest` 改字段（§4.2）；`TerminalSession.execute` 删 background 分叉、走 `exec_with_yield`（§4.1）；删 `LocalTerminalBackend` / `TerminalBackend` 旁路 + `_start_background`。
2. `terminal` schema 改 yield 形态（§5.1）；`_execute` 删全部 mode 守卫（§5.2）；payload 加 `yielded_to_background`（§5.3）。
3. hard-cut 删 `background` / `timeout_seconds` / `session_id` 别名 / `tty-and-not-background` 守卫 / `notify_on_complete` / model-facing `max_lifetime_seconds` 残留。
4. 改写依赖旧字段的现有测试为 yield 等价形态。

退出条件：
1. schema 无 `background`/`timeout_seconds`/`session_id`/`max_lifetime_seconds`；有 `yield_time_ms`/`tty`。
...
| 测试 | 断言 |
|---|---|
| schema 形状 | `terminal` 无 `background`/`timeout_seconds`/`session_id`/`max_lifetime_seconds`；有 `yield_time_ms`/`tty`；`additionalProperties:false` |
...
| model-facing lifetime 硬切 | 直接给 `terminal` 塞 `max_lifetime_seconds` → blocked，错误说明 runtime-only；runtime watchdog 只供宿主/内部 `TerminalRequest` 使用 |
| numeric/materialization 防御 | `terminal.max_output_chars=0` fallback 到默认；过小正数 floor 到合理下限；`terminal_process.max_output_chars` 使用同一规则；`terminal_process.timeout_seconds=0` 回到有限默认；`yield_time_ms=0` 保持立即 yield；`workdir="."`/`"./"` 与空字符串一样沿用 session cwd；`memory_search.limit=0` 回到默认 5 |
...
2. runtime 直接调用显式 `max_lifetime_seconds=2` 时**才** ~2s 被杀——杀只来自宿主/内部显式 watchdog，LLM-facing `terminal` schema 不暴露该字段。
