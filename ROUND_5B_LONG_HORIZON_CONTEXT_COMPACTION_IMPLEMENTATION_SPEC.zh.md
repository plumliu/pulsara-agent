...
> 激活日期：2026-08-21
>
> 机器证据：[round5b_long_horizon_context_compaction_activation.json](benchmarks/suites/core/v1/round5b_long_horizon_context_compaction_activation.json)
>
> 记录日期：2026-08-17
>
> 当前代码基线：5077144b139aa5f3377e59215b696d7e16a58ce3（feat: activate Agent Skills standard）
>
...
>
> 前置实现：[Round 3 compiler](ROUND_3_STRUCTURED_MODEL_INPUT_COMPILER_IMPLEMENTATION_SPEC.zh.md)、[Round 3.1 prefix continuity](ROUND_3_1_PROVIDER_INPUT_PREFIX_CONTINUITY_IMPLEMENTATION_SPEC.zh.md)、[Round 5A execution envelope](ROUND_5_LONG_HORIZON_EXECUTION_ENVELOPE_IMPLEMENTATION_SPEC.zh.md)、[Round 5A.1 provider-neutral output termination](archived_docs/ROUND_5A_1_PROVIDER_NEUTRAL_MODEL_OUTPUT_TERMINATION_IMPLEMENTATION_SPEC.zh.md)、[Round 5A.2 durable provider replay/cross-restart continuation](archived_docs/ROUND_5A_2_DURABLE_PROVIDER_REPLAY_AND_CROSS_RESTART_THREAD_CONTINUATION_IMPLEMENTATION_SPEC.zh.md)、[Round 7 model-visible outcome/timing](ROUND_7_MODEL_VISIBLE_FAILURE_AND_TOOL_OBSERVATION_IMPLEMENTATION_SPEC.zh.md)、[Round 7.1 provider-visible ToolResult projection](ROUND_7_1_PROVIDER_VISIBLE_TOOL_RESULT_PROJECTION_IMPLEMENTATION_SPEC.zh.md)、[Round 8 advisory memory](archived_docs/ROUND_8_ADVISORY_MEMORY_SUBSYSTEM_IMPLEMENTATION_SPEC.zh.md)、[Round 9 unified capability semantics](ROUND_9_UNIFIED_CAPABILITY_SEMANTICS_IMPLEMENTATION_SPEC.zh.md)、[Round 9.1 Agent Skills](ROUND_9_1_AGENT_SKILLS_STANDARD_IMPLEMENTATION_SPEC.zh.md)、[Lightweight TODO refinement](archived_docs/PULSARA_LIGHTWEIGHT_TODO_TOOL_REFINEMENT_IMPLEMENTATION_SPEC.zh.md)
>
> hard-cut前参考基线：5b7ad9f7ffc8565bc572180b2bde0c81ab64473a
>
> 规范归属修订（2026-08-17）：正常epoch MCP direct/meta、`MCP_CATALOG`、`list_mcp_servers`、`inspect_new_mcp_tool`、`use_new_mcp_tool`与direct unavailable gate全部由Round 9唯一拥有；普通ToolResult的40,000-byte logical FULL、8-KiB COMPACT、artifact与conditional guidance全部由Round 7.1唯一拥有。Round 5B只在合法rebase boundary重新消费Round 9/9.1的current owner snapshots、registry、dispatch cut/views与普通compiler结果，并复用Round 7.1既有ToolResult variants；它不重新发现能力、不实现meta gateway、不定义第二套MCP exposure DTO，也不定义compaction专用ToolResult阈值。
>
> Skill收口修订（2026-08-25）：Round 9.1不再定义`read_file` activation intent/lookup。Round 5B只为同一真实user run中已经ordinary COMPLETE + actual FULL + continuity-installed的Skill纯派生bounded `RETAINED_SKILL_CONTEXT`。资格证明只join该assistant request之前最新的installed historical `SKILL_CATALOG` observation、canonical request/result与old epoch actual FULL placement；不读compaction-time filesystem，不用current manifest重新批准或否决历史已交付正文，也不建立loaded-state、receipt或跨turn activation history。
>
...
>
> TODO完整交接hard-cut（2026-08-23）：`COMPACTION_RUNTIME_HANDOFF`升级为V2 complete-TODO contract。只要current TODO snapshot存在，FULL与COMPACT都携带相同的完整ordered rows与exact counts，`omitted.todos = 0`；删除actionable-only DTO和TODO prefix降级。TODO本身收紧为16项、192 UTF-8 bytes/item与4 KiB整表，既有runtime handoff仍保持32 KiB。
>
...
>
> Round 9.2/9.3 downstream同步（2026-08-26）：独立[Hook subsystem](ROUND_9_2_HOOK_SUBSYSTEM_IMPLEMENTATION_SPEC.zh.md)让generic `KernelHookDispatcher`在existing compaction lifecycle接收Pre/PostCompact与ROOT SessionStart(compact)；[Plugin implementation](ROUND_9_3_AGENT_PLUGIN_BUNDLE_AND_HOOK_ADAPTER_IMPLEMENTATION_SPEC.zh.md)只向该dispatcher贡献fixed-path definitions，并把portable Skills/MCP按本文批准的cold successor capability cut接入。两者均已ACTIVATED；没有compaction-private Hook engine/context owner、Plugin agent-definition source或第二cold assembler。
>
> Round 9.3 successor同步：same-epoch `reload_capabilities`只追加compatible source observations，保持SYSTEM/tools byte-identical与messages suffix-only。Explicitly adopted compaction successor是合法root rebuild boundary：从current complete Plugin view重新冻结Plugin Skill third batch与native Plugin MCP configs；old installed epoch/attempt通过generic physical lifetime anchors持旧immutable roots直到drain。`PreCompact`、`PostCompact`、`SessionStart(compact)`仍完全沿本文唯一coordinator/cold-assembler path运行。
>
...
Round 5B只增加一个合法的**successor boundary**：active compaction明确关闭旧epoch并建立新epoch，因此使用Round 9的current owner snapshots、registry、target/native preflight、parent dispatch cut与sibling views重新执行标准cold selection。Successor固定使用`EmptyCapabilityEpochPredecessor`：完整current MCP cohort能与Builtin一起fit时全部DIRECT，否则全部META_ONLY；不保留一套compaction-private hybrid规则。Round 9.1的current effective `SKILL_CATALOG`从统一catalog inspection生成，而同一activation内的`ACTIVE_SKILL`精确继承old installed observation，不因文件变化静默改写。对同一真实user run中已经通过ordinary `read_file`完整FULL交付的Skill，本轮另以低authority `RETAINED_SKILL_CONTEXT`有界重注入；它不是activation lookup或loaded-state，也不代表current catalog。
...
-> append one lifecycle-neutral synthetic summarization user message whose
...
-> freeze current Runtime facts and standard Round 9 cold capability inputs
-> pass every candidate through the same CompactionContinuationSeed,
   KernelColdEpochInputAssembler, reclaim validation and tail-shrink search
-> atomically insert snapshot/revision/event and advance exact binding pointer
-> close old continuity epoch
-> after FULL only: active exact-reads/reassembles and installs the new cold epoch;
   idle closes the proven candidate without opening a provider call
-> active: HANDOFF COMPLETE / RESUME NOW and continue the same canonical turn;
   idle: durably AWAIT NEXT USER
~~~

任何summary、planning或共享dry candidate proof失败都保留旧binding与旧epoch。压缩在canonical FULL之前不按active/idle选择不同算法、prompt、validator或resource path。只有FULL之后，active Runtime才允许successor epoch成为当前model-input authority；idle FULL只使snapshot成为下一turn的canonical cold base。
...
- private analysis stripping、共享completed-output安全边界与source-stale检查；
...
grok-build还提供了固定meta-dispatch这一有价值的窄机制：动态MCP descriptor不必全部进入provider `tools`，因此新MCP发现不必破坏KV prefix。该正常epoch产品语义已经完整抽取到Round 9；Round 5B不再拥有search/inspect/use实现，只在合法rebase boundary把current完整MCP cohort按Round 9 EMPTY cold规则重新规划为全部DIRECT或全部META_ONLY。
...
- Web/Desktop compaction timeline、history diff或snapshot inspector。
...
- `SealedBuiltinCapabilitySnapshot`、`PreparedMcpCapabilitySourceSnapshotSet`、`PreparedSkillCatalogSourceSnapshot`与其`EffectiveSkillCatalogInspection`；
- `FrozenCapabilityRegistrySnapshot`；
- `FrozenCapabilityDispatchCut`及其`FrozenToolCapabilityDispatchView` / `FrozenSkillCapabilityDispatchView`；
- `EmptyCapabilityEpochPredecessor`、`FrozenToolCapabilityExposureSelection`与`FrozenToolCapabilityExposurePlan`；
- `FrozenMcpRouteProjection`与ordinary `MCP_CATALOG` renderer；
- `FrozenSkillProjectionInput`、`SkillProjectionOutput`与compiler最终effective Skill source head；
- normal `PreparedKernelToolSurface`、`ProcessLocalToolSurfaceBorrow`、`PreparedToolExecutionBinding`与`PreparedUnavailableDirectMcpGate`。
...
本文删除并禁止实现`McpEpochExposureKind`、`FrozenEpochMcpExposure`、`McpEpochExposureBorrow`及任何同义compaction-private DTO。DIRECT/NEW/UNAVAILABLE route只能来自Round 9的Tool sibling view、`FrozenMcpRouteProjection`与standard planner；native surface只能来自Round 9的`FrozenToolCapabilityExposurePlan.direct_tool_surface`。

Current MCP supervisor仍是唯一physical/catalog owner；Round 5B的process-local settlement resources只组合一个Round 9 successor exposure plan与一次normal `ProcessLocalToolSurfaceBorrow`，不保存第二份descriptor tuple、catalog generation、pending/latest generation或slot identity。

### 2.6 Candidate proof与active epoch boundary只复用Round 9 EMPTY cold selection

所有compaction candidate的pre-adoption dry proof都与ordinary fresh/restart cold-open使用相同的Round 9选择；若FULL后的生命周期要求继续，active successor才成为真实new epoch，而不是old epoch的增量tool-surface mutation：

1. 在summary成功后冻结current exact-scope三类owner snapshots并构造complete registry；
2. resolve exact model target/profile，完成native-wire轻量preflight；
3. 以`EmptyCapabilityEpochPredecessor`构造唯一`FrozenCapabilityDispatchCut`及其Tool/Skill sibling views；
4. Tool planner先输出`FrozenToolCapabilityExposureSelection`；adapter只为最终选中的DIRECT集合物化native wire，形成`FrozenToolCapabilityExposurePlan`；
5. fixed Builtin与完整current eligible MCP cohort全部fit时，MCP cohort整体DIRECT；否则fixed Builtin保持DIRECT、完整MCP cohort全部META_ONLY；
6. Skill composer独立消费同一个parent cut的Skill view；它不参与Tool route或native eligibility。

Round 5B不得为了“尽量保留old DIRECT”实现第三种hybrid predecessor、partial winner、ranking或compaction-private planner。old epoch的DIRECT/META分类只服务old prefix；进入successor时可以合法重分。若完整MCP cohort越界，旧DIRECT也会在new epoch成为META_ONLY，这是标准cold fallback，不是功能丢失。若未来需要“保留old DIRECT并只晋升部分NEW”，必须先在Round 9正式扩展统一planner，而不能藏在compaction中。
...
Schema、ref resolution、dirty/permission/admission/attempt/invoke顺序、one-attempt/result与MCP origin以Round 9 §6.7–§6.8为唯一真源。本轮只验证summary期间工具绝对不可执行、successor epoch中仍为META_ONLY的tool继续使用该普通路径，以及已经进入successor native surface的tool只能直接调用。
...
### 2.12 Compaction successor是标准cold Capability边界

summary call必须继续使用old epoch的exact SYSTEM、direct tools与固定meta tools。summary期间完成的MCP discovery不能插入该request，也不能改变其prefix。

summary成功后，每个compaction入口都调用§2.6的标准EMPTY cold流程完成同一个dry cold-base proof：

~~~text
current owner-issued Builtin/MCP/Skill snapshots
+ current complete FrozenCapabilityRegistrySnapshot
+ exact resolved target / native-wire eligibility
+ EmptyCapabilityEpochPredecessor
-> successor FrozenCapabilityDispatchCut
-> Tool / Skill sibling views
-> FrozenToolCapabilityExposureSelection
-> selected DIRECT native-wire materialization
-> FrozenToolCapabilityExposurePlan
+ SkillProjectionOutput / compaction inherited ACTIVE_SKILL candidate
-> FrozenNonTriggerContextSources
~~~

Host在每个pre-adoption settlement attempt中持有一个uninstalled no-Hook dry dispatch，用于证明standard EMPTY cold base可行；它不是post-FULL installation authority。只有FULL后的active continuation才从rotated exact current cut重新取得parent cut/views、final `FrozenToolCapabilityExposurePlan`、一个`FrozenNonTriggerContextSources`与normal physical borrow，并只把最终no-Hook/Hook sibling之一交给continuity owner。Compiler/continuity candidate唯一拥有最终effective source heads；process-local attempt不得复制`MCP_CATALOG`/`SKILL_CATALOG` head mirror。Physical层使用Round 9已有seam：
...
current permission、Plan、skill、memory和live state同样只进入process-local assembly。任何capability planning/physical字段都不进入`PreparedCompactionCanonicalAdoption`或其fingerprint。borrow/callback/slot对象绝不序列化。Pre-adoption dry borrow在FULL后统一关闭；active的最终borrow从post-adoption current facts重新取得并只服务唯一final sibling：
...
- canonical write前borrow丢失、owner close或没有compatible binding时discard attempt；canonical FULL后则保留snapshot winner，installation按frozen semantic plan尝试exact rebind，失败时不回滚canonical rows并留给下一次cold read；
- successor安装完成前，old epoch仍按其旧direct/meta分类结算已经开始的tool batch。

active adoption FULL后：

- 完整current eligible MCP cohort fit时全部成为下一epochDIRECT；不fit时全部仍为META_ONLY；
- old meta refs全部按old epoch失效；
- Round 9 `list_mcp_servers`按新epoch重新报告DIRECT/NEW；
- Round 9 `use_new_mcp_tool`继续存在，并只接受new epoch route projection中确为`NEW_MCP_META_ONLY`的工具；
- handoff无需重述route：native tools与`MCP_CATALOG`已经分别表达DIRECT/META_ONLY。

若normal borrow冻结后某个selected DIRECT descriptor被server移除或semantic replacement，frozen successor plan仍按new epoch保留该descriptor但以Round 9 unavailable gate阻止dispatch；supervisor中的replacement等待后续cold adoption，不能在同一epoch以meta绕过。不得为了追逐current generation反复废弃已经合法的summary，也不得让旧descriptor调用新schema executor。

DIRECT selection进入provider `tools` channel，不进入BASE_SYSTEM。MCP server instructions、status、failure与META_ONLY tool names仍属于untrusted runtime observation。本文所称“把动态内容放回新context root”是按其真实authority重新placement，不是把所有内容字面拼到system message。

idle manual compaction也执行本节完全相同的cold selection、assembler与reclaim proof，但该process-local dispatch cut、prepared surface、borrow或slot lease永不安装，并在FULL settlement中释放。它不声称这些facts到下一条prompt时仍current；下一条same-scope turn仍以届时owner snapshots走Round 9普通EMPTY planning。
...
- successor `FrozenToolCapabilityExposurePlan`必须满足Round 9 native tool count/schema/provider target bound；
...
| process-local attempt | safe-point fence、summary stream、tail plan、共享dry cold-base proof/缩尾搜索、adoption settlement | 否 |
...
current owner-issued inventories + complete registry
    -> exact target/native preflight
    -> FrozenCapabilityDispatchCut + sibling views
    -> FrozenToolCapabilityExposureSelection
    -> selected native materialization
    -> FrozenToolCapabilityExposurePlan
    -> provider direct tools + NEW catalog observation/meta refs
~~~

不新增“direct selection row”“new MCP table”或session-level exposure receipt，也不把Round 9 pure DTO复制进compaction package。Host close/takeover后旧meta ref自然失效；replacement Host重新连接、完整discovery并在cold epoch按当前READY_CLEAN truth冻结direct surface。canonical transcript只保留模型实际发出的`inspect_new_mcp_tool`/`use_new_mcp_tool` request及其普通ToolResult，不保留process-local catalog generation。
...
ResolvedCompactionPolicy
    ...
    maximum_retained_tool_groups: int      default 3
    maximum_retained_tail_utf8_bytes: int  default 2097152
    maximum_recent_human_messages: int     default 3
...
- 0 < target < trigger < 1；production默认仍为0.55/0.85，但不以任意区间阻止显式测试或未来operator policy；
- recent human count与retained tool group count必须为正；默认均为3，实际读取仍受canonical item/byte与planning deadline约束；
- handoff byte bound至少8 KiB且至多256 KiB；
- planning deadline必须为正；summary stream不设独立total deadline，复用transport的connect/write/read-idle owner；
- configuration不能改变canonical candidate semantic identity；它只影响新attempt planning。

环境变量或未来config入口可以覆盖model slot context window与ratio；prepared target给出的resolved effective input budget是本次trigger、summary与successor唯一模型窗口真值，不再叠加独立128K或262K cap。Round 3 target estimator仍拥有最终effective input budget。
...
reserve按当前scope允许的admission kinds取最大closed quote；不能由调用点猜一个平均消息大小，也不能每个子步骤重新计量。canonical/epoch byte reserve至少机械覆盖同一个central admission limits中4 MiB assistant message hard bound，并与prompt、ToolResult hard bound取max；不得另写1 MiB经验值。metadata-first quote在正文hydrate前按effective floor进行有界range scan并exact绑定prepared cut。该fact只用于提前触发，既不写库也不改变canonical candidate identity。这样正常路径至少保留一次最大合法admission的headroom，compaction planner仍能在第4,097项或16/64 MiB hard stop之前物化source view。
...
planner必须在summary前证明summary request本身能在旧exact prefix上打开，并冻结：

- current SYSTEM与tools；
- selected exact summary prefix及其native replay hydration；
- selected protected tail与recent-human carrier identities；
- provider input budget与wire envelope。

Runtime不按一个虚构的“maximum summary正文”在provider open前拒绝attempt。summary call没有canonical mutation、tool authority或effect；summary完成后，所有入口都使用实际summary、实际runtime handoff与current sources执行§12.2同一exact dry cold-base assembly、reclaim验证与缩尾搜索。任何actual candidate不fit都会在adoption前失败。automatic/mid-turn adoption要求实际new input <= post target。manual force允许超过soft target，但仍必须：

- 比旧source input至少减少minimum_reclaim_tokens，或旧input已经越过hard provider boundary；
...
同一个provider dispatch最多采用一次compaction。new epoch dry assembly仍超过target时，不允许在同一dispatch递归summary summary。
...
manual command可以force低于threshold的compact，但仍必须有可压缩prefix和正reclaim。没有可压缩内容，或所有candidate均无法取得当前policy要求的reclaim，统一返回typed NOT_NEEDED，不写snapshot/event。active与idle入口必须共用同一candidate选择、soft-target/reclaim验证及失败收缩搜索：实际candidate过大或回收不足时，都逐次减少保留tool group直到成功或到0；不得让idle提前失败或提前返回NOT_NEEDED。两者只在adoption后的生命周期上分流。
...
Terminal process、Terminal monitor和flat subagent可以继续physical运行；它们不能在fence期间修改被summary覆盖的exact target canonical prefix。所有入口都在summary之后重新采集最新current Runtime handoff并进入同一dry proof。idle的该次采集只证明当前candidate，不约束未来turn；下一turn cold compile仍重新读取届时current state。
...
- `projection.final_estimate`与`logical_utf8_bytes`由central factory对materialized SYSTEM、唯一tool surface与materialized messages计算，不是调用者参数；estimate允许超过effective input budget，summary prefix、共享pre-adoption dry proof与active post-FULL final assembly仍各自必须满足其真实budget。

`source_view_fingerprint`只组合compatibility discriminator及上述existing carrier/projection的canonical fingerprints，再覆盖physical working-set report；其中replay部分只组合5A.2 manifest-cut fingerprint，不组合尚未hydrate的private body。Round 9 predecessor若后续需要，只能由`predecessor_epoch_view + normal_compile_binding.tool_surface`通过现有factory临时派生，不能作为source view中的第二份状态。不得把这些carrier的内部标量重新序列化一遍。`ModelInputCompileBinding`中的estimator object/transport capability不进入fingerprint，只使用其既有binding/target/estimator/tool-surface fingerprints。
...
all matching result/closure items for every exact
...
ProtectedTailSelectionFact
    ...
    ordered retained CompleteToolGroup identities
        ...
        ordered (assistant_entry_id, tool_call_id) identities
...
summary request一旦open便不可被中途改写。summary结束后，所有入口都按Round 9冻结current owner snapshots/registry、resolve target/native preflight、构造parent cut/views与final `FrozenToolCapabilityExposurePlan`，再冻结其余Runtime facts并做同一exact dry assembly。facts冻结前发生的MCP/Skill变化采用当时最新complete snapshot；冻结后发生的变化继续由各自owner唯一拥有。active FULL后按Round 9/9.1 normal compatible-append规则重读并安装最终successor；idle FULL后关闭未安装的proof，未来turn再从届时current facts普通cold-open。permission、Plan、memory等其他current facts按各自既有freeze规则处理。只有canonical source head漂移、source proof失效或共享dry facts使assembly失败时才丢弃attempt。
...
### 8.4 Summary prompt guidance contract
...
在适用时覆盖以下内容；这些只是内容指导，不是固定标题或输出模板：
- 用户当前目标、最新纠正、明确约束、scope边界、偏好与不可违反的决定；
- 已完成工作、关键技术/架构决定、原因及真正通过的验证；
- 继续工作必要的文件路径、symbol、命令、配置、数据、示例或opaque handle；
- 错误、失败方案、根因、修复和仍在评估的假设；
- 待办、当前工作、精确诊断和直接下一步。

特别重视后续用户反馈；当前canonical内容优先。继承旧summary中仍相关的语义；区分已完成、已验证、仅尝试、失败、待确认与待执行，不发明工作。优先精确信息，避免大段dump、重复消息和无关历史，但不为短而遗漏关键约束。

Runtime会另行提供最近用户原话、protected tool group和current Runtime facts，因此不要逐条复述消息，不枚举动态catalog，不复制Skill正文，也不声称动态状态在交接后仍current。

CONTINUATION OWNERSHIP：summary prompt对所有入口逐字相同，不接收active/idle参数。Runtime另行机械拥有“立即继续当前turn”或“等待下一条用户消息”的生命周期；summary不得推断、指示或编码任一分支，也只能在conversation本身确实留下工作时把它描述为pending。
...
## 9. Summary output normalization与snapshot carrier

### 9.1 Bounded normalization

raw response在process-local owner内完成：

1. provider stream必须正常terminal；
2. 只接受assistant textual content；
3. 规范化BOM、CRLF、外围whitespace和单层Markdown fence；
4. 删除完整的leading top-level analysis scratchpad；
5. 若存在完整`<summary>...</summary>` wrapper则将其视为presentation hint并取出正文；没有wrapper时直接接受完整normalized text；
6. neutralize正文中残留的compaction control token；
7. UTF-8正文必须非空，并服从普通completed assistant output的共享4 MiB安全边界；Round 5B不再另设64 KiB summary cap；
8. 不要求标题、编号、段落、XML、Markdown或固定section，不做semantic fact-check或自然语言schema parser。

provider whole-response atomicity负责拒绝真正的truncated/incomplete response。normalizer不能用字符裁剪制造成功，也不能把prompt中的内容建议升级为canonical prose schema。summary正文进入closed JSON string；可选wrapper不进入context_snapshots.content或后续provider input。
...
### 10.1.1 Shared `KernelColdEpochInputAssembler`

Round 5B把“新epoch第一次provider open的完整输入构造”抽成一个位于neutral Runtime模块、无provider可见descriptor的pure internal class/function：

~~~text
KernelColdEpochInputAssembler
~~~

它不是summarizer、compiler替代品、ContextSource owner、Capability owner或continuity owner；它只是把调用方已经冻结的semantic seed与current authority inputs送入**既有唯一**SYSTEM renderer、Round 9 Tool/Skill consumers、Round 3 compiler、Round 5A.2 wire materializer和Round 3.1 cold/reset candidate factory。Round 10 subagent first-open必须复用同一个production implementation，禁止另写child prompt builder。
...
FrozenColdConversationSeed
  CanonicalColdContinuationSeed
    exact scope / turn / current context-base identity
    ordered canonical provider-neutral items through exact cut

  CompactionContinuationSeed
    exact scope / anchor turn / binding rewrite identity
    validated CONTEXT_SNAPSHOT item
...
~~~text
exact continuity predecessor / existing reset reason
exact resolved model target and provider profile
existing frozen BASE_SYSTEM construction input / contract
one Round 9 FrozenCapabilityDispatchCut
its FrozenToolCapabilityDispatchView / FrozenSkillCapabilityDispatchView
one finalized FrozenToolCapabilityExposurePlan
one FrozenNonTriggerContextSources exact-joining that Tool plan and Skill view
trigger/current-turn source candidates required by the existing compiler
Round 5A.2 metadata-only replay manifest cut
one absolute provider-dispatch planning deadline
existing compiler/lowering/wire contract identities
~~~
...
|---|---|
| stable BASE_SYSTEM与合法scope/profile supplement | SYSTEM |
| exact DIRECT Builtin/MCP与fixed meta descriptors | provider `tools[]` |
| conversation seed | ordered messages |
| permission、Plan、MCP/Skill catalog、memory、clock、handoff等 | 各自runtime-observation source |

Assembler不得接受调用方传入的raw拼接SYSTEM字符串、任意tool JSON或无source identity的message suffix；也不得把动态observation“方便地”提升进SYSTEM。Seed中的opaque user/tool body只经既有provider-neutral renderer处理，不递归清洗或重解释。

为保持Round 5A.2“先selection、后只hydrate selected body”的内存边界，assembler是一个无I/O的两阶段pure API，而不是一次偷偷读取数据库的helper：

~~~text
prepare_semantic(...)
  -> FrozenCompiledModelInput
  -> existing SelectedDurableReplayHydrationRequest | NONE

caller performs the existing bounded repository hydration
  under the same absolute planning deadline

finalize_wire(compiled, selected_hydration | NONE, ...)
  -> ColdEpochInputAssemblyResult
~~~

`prepare_semantic()`只能从final selected assistant placements和metadata-only manifest cut派生现有Round 5A.2 hydration request；不能水合所有候选body。Repository hydration仍由Host/caller拥有，assembler不接收repository callback。`finalize_wire()`必须exact验证session/scope/cut/target/placements与hydration fingerprint，再调用existing wire materializer和continuity candidate factory。

最终输出carrier只组合现有process-local artifacts，不拥有独立authority或第二个aggregate fingerprint：

~~~text
ColdEpochInputAssemblyResult
  compiled_input: FrozenCompiledModelInput
  wire_input_plan: FrozenProviderWireInputPlan
  continuity_candidate_inputs: existing closed cold/reset inputs
~~~

selected durable-replay hydration已经由`FrozenProviderWireInputPlan`exact引用，不在result中重复保存。Result及所有private-body字段必须`repr=False`；它只提供typed return shape，不能成为新的compatibility或installation identity。若本次selection无需native replay，request、hydration和wire-plan proof三者都必须为NONE，不构造empty carrier。

不得为这次抽象新增`ColdEpochState`、aggregate durable fingerprint、generation、relation、event、receipt、job、checkpoint或recovery owner。Assembler不读取PostgreSQL、Host、MCP supervisor、Skill filesystem、Terminal/TODO owner，不取得physical tool borrow/install authority，不执行provider open，也不做continuity CAS。调用方继续负责safe point、owner snapshot时机、physical exact join、CAS、open与settlement。

Normal fresh/restart cold-open使用`CanonicalColdContinuationSeed`；所有compaction candidate都使用`CompactionContinuationSeed`完成同一pre-adoption dry proof；Round 10 child first-open使用`SubagentInitialSeed`。三者必须调用同一`conversation_kernel/cold_epoch.py` production implementation、同一参数validator与同一two-stage wire path，不能只有相似接口却各自复制renderer。Summary call本身必须继续走独立`PreparedCompactionSummaryCall`，不得伪装cold epoch。`IDLE_BASE_ONLY`只是不在FULL后安装已证明的candidate；下一条真实turn到来时仍由normal cold-open以当时current authorities构造`CanonicalColdContinuationSeed`。这样shared assembler抽走的是重复的input assembly，而不是把summary、task admission或domain lifecycle合并成一个god object。
...
Compaction不是Round 9.1定义的“下一真实activation boundary”。Successor Skill输入因此固定拆成两部分：

~~~text
SKILL_CATALOG
    <- current EffectiveSkillCatalogInspection
    <- current FrozenSkillCapabilityDispatchView
    <- existing Round 9.1 catalog renderer

ACTIVE_SKILL
    <- old effective installed ACTIVE_SKILL observation
    <- old ProcessLocalSourceHead.installed_observation_fingerprint exact join
    <- no filesystem reread and no current-body substitution
~~~

Round 5B复用同一个`KernelSkillProjectionComposer`产出current effective catalog，不得复制Agent Skills parser/renderer。Compaction context collector把current catalog candidate、inherited active candidate、`RETAINED_SKILL_CONTEXT`、runtime handoff与其余non-trigger candidates组装进一个普通`FrozenNonTriggerContextSources`，并继续让该carrier exact引用same parent Skill view与owner snapshot。`ACTIVE_SKILL`的`VALUE/CLEARED/UNAVAILABLE` effective state从old epoch最终source head和对应installed observation机械恢复：`VALUE`逐字继承exact name/location/reason/body，`CLEARED/UNAVAILABLE`继承其终止语义，`NOT_APPLICABLE`只表示本call没有successor，必须解析到仍有效的old installed head。无法exact join old head/body时active adoption fail closed，不能从current file猜测。

Current producer observation变化仍可以改变successor `SKILL_CATALOG`，但不能改写same-activation `ACTIVE_SKILL`。下一条真实ROOT user activation boundary或Round 10新child first-open才由Round 9.1 normal composer从当时current effective inspection生成新的active snapshot。Compiler最终对catalog/active source选择出的effective heads只由continuity candidate拥有；post-adoption final assembly消费一个`FrozenNonTriggerContextSources`，不再镜像逐source candidate或最终heads。
...
它只表达当前Host可机械证明的live/control **结构、identity、status与currentness**，不表达历史输出。但`command_preview`、`cwd`、TODO text与subagent objective仍可来自用户、工具或模型，不能因为Runtime完成了bounded freeze就被提升为可信指令。当前source只有一个整体trust字段，因此V2将整个`COMPACTION_RUNTIME_HANDOFF`冻结为`UNTRUSTED_OBSERVATION`，不分裂第二个source；V2 hard-cut把TODO branch改为完整snapshot并删除旧lossy表示。
...
~~~json
{
  ...
  "todos": [
    ...
    {"ordinal": 1, "status": "completed", "text": "..."}
  ],
  "todo_counts": {
    "pending": 0,
    "in_progress": 1,
...
- TODO投影完整current ordered snapshot，包括pending/in_progress/completed；最多16项、每项text最多192 UTF-8 bytes、整表canonical JSON最多4 KiB；
- flat subagent只投影当前Host ACTIVE，最多当前existing capacity；
- FULL aggregate <=32 KiB；
- COMPACT对Terminal process、monitor保留其actionable public identity；只允许从subagent task board尾部删除PENDING/WAITING whole rows并保留exact omitted count，ACTIVE rows不得删除；
- TODO没有item ID：FULL与COMPACT必须逐项携带同一张完整ordered表，每项包含`ordinal + status + text`，`omitted.todos`恒为0；不得截断text、删除尾部、只交接actionable子集、把completed降成count-only表示，或把ordinal描述为稳定ID；
- TODO的`todo_counts`在FULL/COMPACT中都逐项匹配完整rows，表达exact pending/in_progress/completed/total；
...
- 若完整TODO与其余mandatory runtime facts无法进入COMPACT或successor input，则typed resource boundary、provider open=0；不得发送部分TODO后声称current table已交接。

所有入口的live state都在summary完成后、共享dry assembly前冻结。active在adoption FULL后再次读取并要求exact join，或重新assemble；idle关闭该次proof，未来turn重新冻结届时live state。不得在summary开始前抓取一次然后盲用几分钟后的状态，也不得把idle的pre-adoption proof提升为未来turn authority。

### 10.5 TODO与Terminal owner修改面

`TodoRunStateOwner`提供exact-run、只读bounded snapshot方法并直接交付既有immutable `FrozenTodoSnapshot`，不再复制一个lossy compaction DTO；不得把current items持久化或复制进repository。TODO subshape不携带durable item ID；`ordinal`只是当次projection中的ordered position。Terminal manager/monitor增加Host-scoped只读snapshot方法，必须在各自lock内freeze，不读raw output。Subagent manager只提供现有flat task的bounded只读view；hierarchical graph后续另行扩展同一source。
...
1. canonical assistant request与canonical ToolResult通过exact `tool_call_id`与`tool_request_entry_id`唯一join，且request item在old epoch installed messages/placements中定位到exact one message ordinal；duplicate call ID或ambiguous placement直接ineligible；
2. 只考虑该assistant request ordinal之前的installed `SKILL_CATALOG` observations，选择ordinal最高的一项；它必须是canonical `SNAPSHOT + VALUE`，body必须解码为exact `{"skills": [...]}` closed shape，name/location均唯一；`CLEARED`、`UNAVAILABLE`、duplicate或unparseable均ineligible，不得回退到更早VALUE；
3. request必须是ordinary `read_file`，`offset=1`，其`path`上一步historical catalog中exact one matching row的model-visible `location`一致；该location必须以`.../<logical-name>/SKILL.md`结尾；
4. ToolResult public `path`必须等于该historical location的确定性public-path projection：workspace-relative保持relative，`${PULSARA_HOME}`与`~/.agents`使用该Host为epoch冻结的home bindings展开，bundled absolute location保持absolute；该projection不访问current filesystem；
5. ordinary result从line 1明确到EOF，`truncated=false`，`limit >= total_lines`，每行严格为连续`N|text`，canonical ToolResult preview为`COMPLETE`；
6. 旧epoch实际compiler为该result选择`FULL`，并且包含该FULL result的provider input已经由continuity CAS成功安装；只提交row、HEAD_TAIL、COMPACT、REF_ONLY、OMITTED或取消中的attempt均不算；
7. 从ToolResult反解出provider实际收到的logical document text，复用唯一root-neutral parser，并用historical location中`SKILL.md`的parent basename执行placement validation；parsed name/description必须与historical row exact join，retained body定义为该delivered document的parsed body；
8. 它属于当前同一真实user run和exact ROOT/child scope，不是更早turn、foreign child或另一Host猜测出的历史read。
...
首次rebase时，Runtime只从old `FrozenProviderInputEpochView`的actual installed messages/placements、canonical call/result与该request之前最新historical catalog observation重建上述事实。它不得建立跨call mutable“曾加载Skill”表，不访问current filesystem，也不用compaction-time effective catalog替换、批准或否决历史FULL read。`_warning`、dedup telemetry等非content字段不得成为Skill identity。随后另以old epoch actual selected representation证明该exact result为FULL，不能从artifact preview、tool name或人类可读文本启发式推断。Loose override已删除、bundled/loose winner已变化时，历史上已交付的body仍可retained；current catalog独立呈现新winner，supporting resource未来是否仍可读由ordinary tool决定。Host/reopen若已失去证明old actual FULL installation所需的process-local epoch view，则保守视为ineligible；不得从row存在反推FULL。

Repeated active compaction还允许一种递归但仍纯派生的输入：immediate predecessor epoch中**最终effective且continuity-installed**的`RETAINED_SKILL_CONTEXT VALUE`。Runtime必须从predecessor `ProcessLocalSourceHead.installed_observation_fingerprint`定位exact observation，逐项解析其closed `name + catalog_location + body` shape并验证existing retained bounds；不重新join current manifest或filesystem。`CLEARED/UNAVAILABLE`、foreign scope/run或无法exact join的observation都不继承；same-run证明必须逐值确认该head的existing `last_emitted_turn_id == canonical_read.scope.turn_id`，仅ROOT/child scope bucket相同不构成run identity。该分支只携带模型上一epoch实际已看到的低authority正文，不携带read/result/epoch ID，也不是durable loaded-state。
...
Repeated compaction合并顺序固定为：当前predecessor base之后新发生的eligible FULL reads按delivery recentness在前；上一版installed retained items在后并保持其既有relative order。相同Skill以最新eligible FULL read为winner；没有新read时，继承项的选择与render顺序保持稳定。这样第二次compaction不需要回扫已经落到current source floor以下的原始ToolResult，也不需要持久化delivery ledger。
...
active rebase后同一run继续时，successor第一次compile把它与current catalog、inherited active snapshot及其他runtime sources一起安装。同一turn的normal compatible append必须保留该head；下一真实ROOT user message的首次normal append则追加exact `CLEARED` suffix，若该turn在provider open前先compaction，则通过上述turn-ID mismatch直接构造无retained的cold successor。两条路径都从空retained set重新开始且不改写旧prefix；child集合在exact child run终结时消失。Repeated mid-turn compaction只从post-base新FULL reads与immediate predecessor installed retained observation合并候选，不复制上一snapshot的内部identity，也不从genesis回扫ordinary reads。
...
CompactionSettlementResources
    ...
    CompactionSettlementAttempt
        CompactionAttemptToken
        old continuity epoch identity
        pre-adoption no-Hook dry dispatch (same structural proof for every target)
        no continuity permit / no installed ToolResult delivery / no provider open
        post_adoption_action = RESUME_ACTIVE_TURN | AWAIT_NEXT_USER

        after FULL and only for RESUME_ACTIVE_TURN:
            exact rotated canonical cut
            current no-Hook base + optional Hook sibling
            one final normal ProcessLocalToolSurfaceBorrow
            one final continuity installation authority / provider open

        after FULL and for AWAIT_NEXT_USER:
            release the shared dry resources
            clear old continuity identity | EMPTY
~~~
...
每个compaction入口都取得§2.12的current Round 9 parent cut/views、final Tool plan与normal `ProcessLocalToolSurfaceBorrow`，并使用process-local synthetic canonical snapshot构造同一种candidate：
...
把上述内容封装为`CompactionContinuationSeed`，调用§10.1.1唯一`KernelColdEpochInputAssembler`。protected tool tail按最长合法suffix逐组缩减；Retained Skill selection按§10.6的recent prefix从8逐步收窄到0。每次候选都经同一个assembler调用normal compiler、target estimator与wire materializer，再经过同一soft-target/reclaim validator，直到无Hook cold base满足aggregate bound与整体post target，不得截断单个Skill正文。active/idle没有不同prompt、quote、validator、retry或fallback。只有该uninstalled dry proof成功且`PreparedCompactionCanonicalAdoption`已经冻结，才允许repository transaction；dry proof不签发continuity permit、不安装ToolResult delivery且不physical open。

事务FULL后先运行PostCompact，至此才按生命周期结算。若目标需要继续且turn仍RUNNING，reader旋转并读取真实snapshot/revision的exact current cut，重新冻结current Tool surface、runtime/non-Hook facts并构造final no-Hook base。仅ROOT在coordinator fence内调用runner提供的`SessionStartCompactPort`；child端口必须是`None`。SessionStart context如存在，再从相同post-adoption exact facts构造optional Hook sibling。No-Hook/Hook siblings的SYSTEM/tools必须逐字相同，Hook按placement 68成为唯一新增semantic source并重新过bounds；Hook sibling invalid、不fit或omitted时回退valid no-Hook sibling。Pre-adoption dry messages/source heads/wire/candidate不要求与任一final sibling相等，因为它从未installed。最终只选择一个sibling，经existing continuity owner执行至多一次CAS/install/open；不得先安装base再augmentation、revoke/supersede或第二次CAS。

若目标是等待下一条用户消息，FULL后立即关闭同一pre-adoption dry dispatch、Tool plan、retained selection与physical borrow，清除旧continuity，并且绝不创建PreparedProviderInputAppendCandidate、continuity install candidate、permit或provider open。该proof只说明canonical candidate在write前以当时current facts通过同一真实装配与reclaim条件；它不承诺未来prompt或未来current sources。下一条same-scope turn才按当时current capability catalog、permission、Plan、skill与memory执行普通cold compile。
...
repository API接收`PreparedCompactionCanonicalAdoption`与窄的write-precondition facts，但只把前者作为winner identity。它不读取process-local capability owner、source view、prefix proof、dry assembly、retained-Skill proof或tool-surface borrow。所有入口在write前由Host暂时持有同一种Round 9/9.1 semantic plan、retained source selection与normal physical borrow。FULL后，需要继续的生命周期由同一个settlement task从current facts exact重建并安装；等待下一消息的生命周期则释放全部proof资源，不产生installation side branch。physical identity冲突时provider open=0，但不能回滚、改写candidate或另写已经FULL的canonical snapshot。不得分成“先存summary、再更新binding”两个事务；不得产生orphan snapshot或CompactionStarted row。
...
source view、prefix proof、old epoch、MCP borrow/execution identity、dry assembly及active/idle process branch不参与FULL比较，因为数据库不持有它们。UNKNOWN不允许直接生成新summary、snapshot ID或boundary。FULL再按当前process-local resource branch结算：active最多安装一次new epoch；idle只确认canonical base winner并清除旧continuity。NONE重写same canonical candidate前必须重新验证同一resource/preconditions仍可用；CONFLICT丢弃，不继续provider。

### 12.6 Host-owned settlement task

summary完成并形成`PreparedCompactionCanonicalAdoption + CompactionSettlementResources`后，Host安装唯一process-local settlement task，拥有write、confirmation与binding pointer settlement。只有canonical FULL之后，settlement才读取post-adoption action并选择continuity install或clear。request waiter cancellation只detach waiter；Host close必须drain settlement。
...
- `AWAIT_NEXT_USER`：释放与active同形的pre-adoption dry资源及summary资源，清除旧scope continuity；永远不调用`HostProviderInputContinuityOwner.install()`；
...
当前所有new-turn producer必须走repository内唯一共享的context-base selection与transaction verification helper；不额外冻结一层无消费者的inheritance DTO：
...
这些producer完成canonical turn admission后，第一次provider dispatch统一从已接纳的binding构造`CanonicalColdContinuationSeed`并调用§10.1.1 assembler。该共享路径同样服务无snapshot的ordinary fresh/restart cold-open；不得让snapshot继承另走compaction-private renderer。
...
| manual no compactable prefix / all candidates non-reclaiming | none | active与idle经过相同缩尾搜索后均返回typed NOT_NEEDED；当前turn照常继续或保持idle |
...
| MCP late-ready/semantic replacement while summary streams、candidate freeze前 | none yet | summary继续old tools；所有入口完成后freeze最新READY_CLEAN exposure并走同一dry assembly；只有active在FULL后重读并安装 |
...
| other current source/non-MCP tool surface changes during summary | none yet | refreeze and dry assemble current facts |
| canonical source head drifts despite fence | none | conflict, discard summary |
| dry assembly over target | none | try fewer retained groups if summary not yet called; after output, discard |
| snapshot blob publication fails | none | old epoch remains |
| commit ACK unknown | possible FULL | exact-confirm only canonical row drafts/event；不查询prefix/MCP/dry-assembly resources |
...
Round 5B只要求headless Host API与Protocol能够发出manual compact、重建bounded compaction状态，并对fence期间的目标scope command给出typed拒绝。仓库不随本轮提供Web或Desktop实现，也不把client-local draft/deferred lane计入activation。
...
process-local Terminal/TODO/MCP/memory不假装属于该RR transaction；所有入口在summary后另行freeze并进入同一uninstalled dry proof。FULL后，active再从current facts构造final assembly；idle释放proof，下一turn再从届时current facts cold assemble。
...
src/pulsara_agent/conversation_kernel/cold_epoch.py
    KernelColdEpochInputAssembler
    FrozenColdConversationSeed closed union
    ColdEpochInputAssemblyResult
...
src/pulsara_agent/conversation_kernel/compaction/
    ...
    prompt.py          summary guidance/output normalization/provider carrier
    model_call.py      PreparedCompactionSummaryCall preflight/open_once
    runtime_handoff.py Host-local live-state projection
    retained_skills.py pure historical-catalog/ordinary-read/FULL eligibility + rendering
...
Round 9 capability owners/planner
    -> FrozenCapabilityDispatchCut + Tool/Skill sibling views
    -> FrozenToolCapabilityExposureSelection / FrozenToolCapabilityExposurePlan
    -> ordinary PreparedKernelToolSurface / ProcessLocalToolSurfaceBorrow

Round 9.1 owner snapshot/discovery + old installed epoch view
    -> current SKILL_CATALOG + inherited ACTIVE_SKILL source candidates
    -> retained_skills pure fresh-read/inherited-source eligibility and selection
    -> RETAINED_SKILL_CONTEXT compiler source

Round 3 compiler + Round 5A.2 wire planner + Round 9 dispatch cut
    -> neutral KernelColdEpochInputAssembler
    -> consumed by compaction service and Round 10 subagent coordinator
~~~
...
- `cold_epoch.py`读取repository/Host/current owner、取得physical borrow/install authority、执行provider open或continuity CAS；
- compaction或subagent package复制SYSTEM/tools/runtime-source placement、wire lowering或cold candidate construction；
...
### R5B-A0：Shared cold-epoch input assembly

- 在neutral `conversation_kernel/cold_epoch.py`落唯一`KernelColdEpochInputAssembler`；
- frozen seed只支持closed `CanonicalColdContinuationSeed | CompactionContinuationSeed | SubagentInitialSeed`，不接受raw prompt list或open-world purpose branch；
- exact join scope、resolved target、frozen BASE_SYSTEM input、Round 9 `FrozenCapabilityDispatchCut`及两个sibling views、final `FrozenToolCapabilityExposurePlan`、exact `FrozenNonTriggerContextSources`、trigger/current-turn source candidates、metadata-only replay manifest cut与唯一planning deadline；
- 两阶段pure API先返回existing selected-hydration request，由caller按同一deadline完成bounded repository read，再finalize既有wire/candidate artifacts；assembler内部I/O为0；
- 复用现有SYSTEM renderer、compiler、wire planner与continuity candidate factory，返回无独立fingerprint的`ColdEpochInputAssemblyResult`；
...
- current `SKILL_CATALOG`从owner-issued effective inspection生成；`ACTIVE_SKILL`从old effective installed observation精确继承，不把compaction误当activation boundary；
- `RETAINED_SKILL_CONTEXT`从old installed historical catalog + FULL ordinary reads与immediate predecessor installed retained source纯派生，不join current catalog/filesystem，最多8项；
...
- summary request始终使用old `FrozenProviderInputEpochView` / `ModelInputCompileBinding`的exact native tools与old catalog message prefix；source view不另存Capability predecessor mirror；
- active successor使用current complete registry、exact target/native preflight形成标准`FrozenCapabilityDispatchCut`、两个sibling views、selection与final `FrozenToolCapabilityExposurePlan`；
- fixed Builtin + 完整current eligible MCP cohort fit时MCP全部DIRECT；overbound时Builtin保持DIRECT、全部MCP为META_ONLY，不partial ranking且compaction可成功；old epoch route不作为successor selection输入；
- old DIRECT schema replacement只在successor boundary采用，不在old epoch以meta双版本绕过；
- successor compile从current owner-issued effective inspection生成`SKILL_CATALOG`、从old effective installed observation继承`ACTIVE_SKILL`，并把它们与historically proven `RETAINED_SKILL_CONTEXT`一起纳入continuity CAS；
...
### 20.1.1 Shared cold-epoch assembler

- 现有ordinary EMPTY cold-open在抽取前后，Chat Completions与Responses的SYSTEM、ordered tools、semantic messages、wire plan、quote与continuity candidate inputs逐项相等；
- `CanonicalColdContinuationSeed`、`CompactionContinuationSeed`与Round 10 `SubagentInitialSeed`只能进入同一个sealed production assembler；architecture scan中不存在normal/compaction/child-specific prompt builder；
- wrong scope、wrong target、mixed Round 9 parent/view/Tool plan/source collection、stale Skill owner、stale source candidate、foreign replay hydration或deadline mismatch在任何continuity registration/provider open之前拒绝；
...
- `FrozenModelToolSurface`是old provider tool surface唯一owner；source view只组合existing compile binding/optional epoch view，不保存Round 9 predecessor或复制MCP exposure/spec tuple；需要predecessor时由现有factory临时派生；
...
- active/idle使用同一份lifecycle-neutral summary prompt；它明确summary-only限制随response结束，且不得把“本次不能执行”写成“用户任务因compaction排队/延期”；
...
- process在summary期间完成，dry assembly看到最终状态；
- new Host无physical owner时追加CLEARED；
- TODO完整保留pending/in_progress/completed ordered rows；
- flat subagent bounded；
- MCP/skill/permission/Plan/memory不重复进handoff；
- selected DIRECT MCP进入successor native tools而非SYSTEM/handoff正文；META_ONLY工具只由MCP_CATALOG说明；
- FULL到COMPACT deterministic；TODO rows在两种variant中byte-equivalent且`omitted.todos = 0`，只允许subagent PENDING/WAITING尾部whole-row降级；
- 32 KiB hard bound。

### 20.8 Retained Skill context

- ordinary `read_file`从offset 1一次到EOF、canonical COMPLETE、actual FULL并已continuity install时，同run active compaction可重注入provider历史上实际收到的exact parsed body；
...
- historical read后current filesystem修改、删除、invalid或same-name winner变化不否决已交付body；successor current catalog独立显示当前winner；
- next real ROOT user message清空；exact child run与ROOT/other child严格隔离；
- idle compaction不生成retained source；
- repeated mid-turn compaction合并post-base新FULL reads与immediate predecessor exact installed retained observation；原始read位于source floor以下时不回扫genesis，也不建立loaded ledger；
...
- compaction successor按EMPTY cold规则把fit的完整current eligible MCP cohort整体选为DIRECT，old refs失效；
- 完整MCP cohort越native bound时不partial选取，全部MCP改走meta且compaction可成功；old DIRECT没有特殊保留权；
- successor Tool plan/normal physical borrow冻结后完成的新discovery不作废adoption；各source owner仍唯一拥有current truth，安装后按current-minus-frozen差集成为NEW append；
...
Activation必须在ephemeral clean-v0 PostgreSQL完成至少四条。固定fixture probe必须把`HOME`与`PULSARA_HOME`绑定到call-local临时空目录，只保留package内bundled producer与该probe显式创建的workspace definitions；不得让operator真实user roots改变阈值、catalog或轨迹。该隔离只约束Round 5B固定fixture，user-root产品能力由Round 9.1/统一Skill dogfood独立证明：

### 21.1 Standard cold MCP selection
...
1. 从一个已安装epoch开始，其中至少一个MCP tool为DIRECT，另一个current READY_CLEAN tool为META_ONLY；
2. 在compaction前证明META_ONLY tool仍通过Round 9 inspect/use route可用，并冻结该事实的bounded sentinel；
...
1. provider根据已安装的`SKILL_CATALOG`用ordinary `read_file(offset=1, limit=2000)`完整读取一个Skill，并保留该assistant request之前最新historical catalog observation；
2. 证明canonical preview为COMPLETE、old epoch actual representation为FULL且continuity已安装，request/result/path/parsed name/description与historical row逐项join；
...
6. 改变或删除loose winner后重复场景，旧body仍依historical FULL proof retained，current catalog独立展示新winner或bundled fallback；
7. 执行第二次mid-turn compaction，证明可从immediate predecessor installed retained observation继续携带正文，不回扫source floor以下的原始read，也不重新join current filesystem；
8. 下一真实ROOT user message不继承model-driven retained body。

证据必须足以重放诊断判断：除既有aggregate外，保存每次真实user prompt与最终模型正文、normalized provider blocks、tool arguments/results、每次compaction trigger/disposition/public code及`INITIAL | REPAIR`完整顺序；每个provider terminal还保存reported input/cached/output/reasoning/total tokens，并用exact cached/input分子分母分别汇总foreground、summary与全轨迹cache hit，missing usage保持显式且cache仍不是correctness gate。该trace是显式activation/dogfood evidence，不是production operational log、conversation recovery authority或durable execution history；它不创建receipt/replay/retry机制。只对`PULSARA_API_KEY`的exact value做精确scrub，不广泛遮蔽summary、ToolResult、schema、临时workspace path或其他诊断正文；不得持久化整份environment、transport headers或requestState。
...
22. successor MCP selection偏离Round 9 EMPTY cold all-or-none规则、按ranking/discovery timing/偶然completion order做partial selection，或把server instructions/catalog写入SYSTEM；
23. repository transaction读取process-local Capability owner/physical borrow，或把source view、prefix proof、dry assembly、execution binding、slot lease、retained-Skill proof或catalog callback写入`PreparedCompactionCanonicalAdoption`；
...
25. idle adoption调用continuity install、签发未被provider消费的permit、在settlement后遗留pre-adoption planning/access/retained-Skill/borrow资源或启动runner；
...
28. active successor完整MCP cohort越native bound导致compaction失败、继续私自保留old DIRECT、或产生partial DIRECT winner，而不是按Round 9统一退为META_ONLY；
...
31. canonical candidate fingerprint包含source view、prefix proof、epoch nonce、Capability semantic/physical identity、retained-Skill/tool-tail proof或dry assembly等数据库无法逐字段确认的字段；
...
39. normal `ProcessLocalToolSurfaceBorrow`复制完整successor exposure/catalog，active resources同时保存第二份相同Round 9/9.1 semantic fact/effective source head，或Round 5B直接管理`McpSlotLease`；
...
43. `RETAINED_SKILL_CONTEXT`因physical read、canonical row存在、HEAD_TAIL/COMPACT/REF_ONLY或partial page而接纳Skill；跨真实ROOT user message继承；重注入已修改/删除manifest；重复compaction从genesis回扫旧read；或建立durable loaded-skill ledger。
44. TODO COMPACT删除任何current row、只保留actionable子集、把completed降成count、只保留ordinal/status、伪造稳定item ID、截断单项text、让counts与rows不一致，或发送部分/空正文声称current TODO已完整交接。
...
- active/idle逐字共用唯一lifecycle-neutral summary prompt，明确response-local no-tool restriction不会变成任务状态；closed snapshot carrier由Runtime机械持久化`RESUME_ACTIVE_TURN | AWAIT_NEXT_USER`、完整handoff instruction与active-request location，active successor立即resume，idle断电重启后仍等待未来真实user message；
...
- successor `SKILL_CATALOG`来自current owner-issued effective inspection；same-activation `ACTIVE_SKILL`来自old effective installed observation并逐字保持，不因current文件变化重写；下一真实activation boundary才读取current body；
- `RETAINED_SKILL_CONTEXT`只接纳同run、exact scope、一次从offset 1到EOF、canonical COMPLETE、actual FULL且continuity已安装，并与该assistant request之前highest-ordinal canonical `SNAPSHOT + VALUE` historical catalog row逐项join的ordinary Skill read，或immediate predecessor exact installed retained observation中符合closed shape/bounds的item；不重新join current manifest/filesystem。最多8项、40,000-token aggregate，超界按recent prefix整项缩减，不截断body；下一真实ROOT user message或idle compaction不继承；
- active/idle在FULL前逐字共用summary prompt，并共用candidate selection、`CompactionContinuationSeed` dry assembly、soft-target/reclaim validator与逐组缩尾搜索；snapshot adoption原子且ACK unknown闭合；只有FULL后active才install successor epoch，idle释放同类proof资源、清除旧continuity并让下一turn cold-open；
- `PreparedCompactionCanonicalAdoption`只含可由snapshot/revision/pointer/predecessor/event确认的row drafts；所有planning/epoch/MCP/execution/dry-assembly事实只存在于process-local resources；
- current binding唯一决定active summary；
- new ROOT producers全部继承latest exact ROOT snapshot；
- current Terminal/monitor/TODO/flat subagent由bounded `UNTRUSTED_OBSERVATION` handoff重建；Runtime只证明其结构与currentness，TODO不携带item ID，FULL/COMPACT完整保留相同的ordered `ordinal/status/text` rows与exact counts，`omitted.todos = 0`；
...
- source view、prefix proof、protected-tail selection、canonical adoption与normal tool-surface physical borrow均遵守single-owner DTO contract：existing frozen carriers被组合而不重新摊平，source view不重复保存Capability predecessor，数据库重复列只由central factory派生为只读row mirrors；
...
- targeted、PostgreSQL、retained、full pytest、static与real-provider dogfood通过；
- Gap Index同步记录Round 7.1 normal ToolResult投影、Round 9/9.1与Lightweight TODO前置、PHC-07B恢复，不扩大Web/Desktop UI、memory extraction或Round 10层次化subagent范围。
...
2. `KernelColdEpochInputAssembler`如何从Round 5B抽出、保持ordinary cold-open wire不变，并让compaction/child共享而不拥有任何I/O或authority；
3. 三种compaction入口及token/resource-headroom OR trigger；
4. `PreparedCompactionSummaryCall`如何取得active/idle target、证明source view exact、禁止execution authority，以及COMPATIBLE_APPEND时old SYSTEM/tools/prefix不变；
5. successor如何以EMPTY predecessor复用Round 9 exact target/native preflight、parent/views与两阶段Tool selection/materialization，并按完整MCP cohort all-or-none DIRECT/META规则使old refs失效；
6. current `SKILL_CATALOG`、inherited `ACTIVE_SKILL`、post-compile effective source heads与`RETAINED_SKILL_CONTEXT`如何随continuity CAS安装；
7. protected tool group、Round 7.1 normal result variants与recent human selection；
8. FULL_HISTORY floor、shared canonical range fingerprint、bounded lineage digest，以及canonical row-draft candidate与process resources分层后的atomic adoption/ACK unknown；
9. current Runtime sources、`RETAINED_SKILL_CONTEXT`与`UNTRUSTED_OBSERVATION` COMPACTION_RUNTIME_HANDOFF的重建，以及TODO无ID、完整有序的subshape；
10. FULL前唯一压缩路径、FULL后的active install/idle clear settlement、lane/fence顺序、cross-turn及repeated compaction；
11. durable job machinery删除后的最终oracle；
12. exact测试、PostgreSQL、static与四条dogfood证据；
13. 明确normal MCP/Skill/ToolResult contract不是本轮修改面，并列出其余non-goals。
