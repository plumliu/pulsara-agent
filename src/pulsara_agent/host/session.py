...
from pulsara_agent.runtime.session import EventReconciliationRequired
from pulsara_agent.event_log import EventIdConflict
...
class HostSession:
    ...
    def _plan_workflow_state_fact(self) -> PlanWorkflowStateFact:
        events = self.wiring.runtime_wiring.event_log.iter()
        plan_events = [
            event
            for event in events
        ...
        if self.plan_state.active:
            ...
            if entered is None or entered.sequence is None:
                raise ValueError(
            ...
            return PlanWorkflowStateFact(
                workflow_id=f"plan_workflow:{entered.id}",
                active=True,
                revision=len(plan_events),
                entered_event_id=entered.id,
                entered_event_sequence=entered.sequence,
                entry_run_id=entered.run_id,
                entry_turn_id=entered.turn_id,
                entry_reply_id=entered.reply_id,
    ...
    def _transcript_snapshot_fact(
        ...
        *,
        ...
        if isinstance(
            ...
            compacted_window_id = (
                preflight_terminal.window_id
                if isinstance(preflight_terminal, ContextCompactionCompletedEvent)
                else None
            )
        else:
            compaction_id = None
    ...
    async def _commit_new_run_entry(
        ...
        *,
        ...
        candidates = (draft.run_start_event, *pending_audits)
        ...
        try:
            ...
            if isinstance(exc, EventPublicationAfterCommitError):
                ...
                confirmation = runtime_session.confirm_event_batch(candidates)
                if confirmation.missing_event_ids:
                    self._set_boundary_commit_state("commit_outcome_unknown")
                    raise
    ...
    async def _exit_pending_plan_workflow_pipeline(
        ...
        *,
        ...
        async with self._run_lock:
            ...
            self._install_run_termination_intent(state, AbortKind.USER_STOP)
            result = await self.wiring.agent_runtime.abort_run(
            ...
            if self.pending_interaction is not None and (task is None or task.done()):
                ...
                async with self._run_lock:
                    ...
                    if pending is None:
                        return None
                    state = self._require_suspended_state(pending)
                    self._install_run_termination_intent(state, reason)
    ...
    def _install_run_termination_intent(
        self,
        state: LoopState,
        reason: AbortKind,
    ...
    def _live_boundary_projection(self) -> dict[str, object]:
        ...
        compaction_events = self.wiring.runtime_wiring.event_log.iter()
        started = {
            event.id
    ...
    def _boundary_batch_confirmation(
        ...
        candidates = attempt.candidate_events
        candidate_ids = attempt.candidate_event_ids
        if not candidates or not candidate_ids:
            return None
        runtime_session = self.wiring.runtime_wiring.runtime_session
        try:
            confirmation = runtime_session.confirm_event_batch(candidates)
            ...
            return BoundaryBatchConfirmation(
                status=BoundaryBatchCommitStatus.CONFLICT,
            ...
            # RuntimeSession has already latched the structural partial batch.
            stored = tuple(
                event
                for event_id in candidate_ids
                if (event := runtime_session.event_log.get_by_id(event_id)) is not None
            )
            return BoundaryBatchConfirmation(
                status=BoundaryBatchCommitStatus.PARTIAL,
                candidate_event_ids=candidate_ids,
                committed_event_ids=tuple(event.id for event in stored),
                committed_sequences=tuple(
                    event.sequence for event in stored if event.sequence is not None
                ),
                actual_last_sequence=runtime_session.event_log.next_sequence() - 1,
            )
        except BaseException:
            runtime_session.latch_event_commit_outcome_unknown()
            return BoundaryBatchConfirmation(
        ...
        return BoundaryBatchConfirmation(
            status=status,
            candidate_event_ids=candidate_ids,
            committed_event_ids=tuple(event.id for event in committed),
            committed_sequences=sequences,
            actual_last_sequence=confirmation.actual_last_sequence,
        )
    ...
    def _original_run_start_for_resume(self, state: LoopState) -> RunStartEvent:
        ...
        if len(starts) != 1 or starts[0].sequence is None:
            raise RuntimeError(
    ...
    async def _prepare_and_commit_resume_boundary(
        ...
        *,
        ...
        if not isinstance(source_plan, CapabilityExposurePlan) or not isinstance(
            source_fact, CapabilityExposureSnapshotFact
        ):
        ...
        try:
            ...
            if isinstance(exc, EventPublicationAfterCommitError):
                ...
                if confirmation.missing_event_ids:
                    ...
                    raise
                self._set_boundary_commit_state("committed")
                committed_events = tuple(confirmation.committed_events)
