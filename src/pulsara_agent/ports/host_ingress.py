...
@dataclass(frozen=True, slots=True)
class ActiveRunPromptSteerSafePointLease:
    """Exact queue reservation borrowed for one next ModelStart commit."""
    ...
    queue_item_head_event_reference: Any
    queue_item_head_candidate_payload_fingerprint: str
    queue_item_revision: int
...
def build_active_run_prompt_steer_guard(
    *,
    lease: ActiveRunPromptSteerSafePointLease,
    provider_input_start_bundle: Any,
    expected_user_steer_event_id: str,
) -> ActiveRunPromptSteerCommitGuardFact:
    """Bind one queue reservation to the exact prepared provider append."""
    ...
    if isinstance(provider_guard, ExistingAppendCommitGuardFact):
        ...
        raise HostIngressAdmissionStale(
            "active-run prompt steer requires an existing provider generation"
        )
    return build_frozen_fact(
        ActiveRunPromptSteerCommitGuardFact,
        schema_version="active_run_prompt_steer_commit_guard.v1",
        runtime_session_id=lease.runtime_session_id,
        run_start_event_reference=lease.run_start_event_reference,
        active_segment_id=lease.active_segment_id,
        active_segment_generation=lease.active_segment_generation,
        ...
        queue_reservation_fingerprint=lease.reservation_fingerprint,
        queue_item_head_event_reference=lease.queue_item_head_event_reference,
        queue_item_head_candidate_payload_fingerprint=(
            lease.queue_item_head_candidate_payload_fingerprint
        ),
        expected_queue_item_revision=lease.queue_item_revision,
        expected_queue_account_revision=lease.queue_account_revision,
