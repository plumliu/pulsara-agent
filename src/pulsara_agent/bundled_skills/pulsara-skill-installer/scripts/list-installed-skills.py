...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="List installed workspace Pulsara skills."
    )
    parser.add_argument(
        "--workspace",
        default=".",
    ...
    parser.add_argument(
        "--dest-root", help="Skills root. Defaults to <workspace>/.pulsara/skills."
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        if args.json:
            print(
                json.dumps(
                    {"skills_root": str(dest_root), "skills": skills},
                    ensure_ascii=False,
                    indent=2,
                )
            )
            ...
            for skill in skills:
                status = (
                    "valid" if skill.get("valid") else f"invalid: {skill.get('error')}"
                )
