"""Pure terminal command risk classification."""
...
def is_risky_terminal_command(command: str) -> bool:
    ...
    return _matches_any(
        _RISKY_COMMAND_PATTERNS, stripped
    ) or is_sensitive_terminal_command(stripped)
...
def _matches_any(patterns: tuple[re.Pattern[str], ...], command: str) -> bool:
