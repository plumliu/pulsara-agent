...
记忆页继续提供“全局记忆”／“项目记忆”范围、类别、生命周期筛选、分页、详情、允许展示的来源、关系及删除预览。项目选择列表由本 memory domain 中**当前仍存在**的项目 context 正式 fact 派生，ACTIVE 和 SUPERSEDED 均计入；只有会话、没有项目记忆的目录不出现，删除最后一条项目记忆后目录退出列表。按后续项目／会话／记忆解耦规格，项目元数据来自独立的 canonical workspace。列表与详情读取均不接受会话页当前会话作为筛选依据；删除后刷新列表，若所选项目已无记忆则清空选择。会话归档或永久删除不删除已保存记忆，记忆项目不必是普通会话列表可见项目的子集。CLI 退出仅关闭运行时，普通列表仅展示 OPEN；已归档会话由设置页管理，取消归档后恢复 OPEN。记忆页保留 ARCHIVED 来源说明，提示先取消归档；DELETED 来源无法恢复。首版用户 GUI 不新增手动关系标定；会话页不加记忆管理控件。可按正式 fact 时间显示最近保存项，不添加 durable 通知。用户删除不能撤回先前发给模型的记忆内容。
