...
def test_message_reducer_replays_text_thinking_tool_events() -> None:
    ...
    assert msg.content[2].type == "tool_call"
    assert msg.content[2].input == '{"q":"x"}'
    assert msg.content[3].type == "tool_result"
    assert msg.content[3].output[0].text == "found"
    assert msg.content[3].state is ToolResultState.SUCCESS
