...
def test_postgres_set_status_keeps_graph_document_and_memory_projection_in_sync() -> None:
    ...
    try:
        store.put_jsonld(_preference("preference:status", "The user prefers concise summaries."), graph_id=graph_id)
...
def test_postgres_lifecycle_superseded_node_is_not_recalled_and_edge_is_materialized() -> None:
    dsn = StorageConfig.from_env().postgres_dsn
    _connect_or_skip(dsn).close()
    graph_id = f"graph:test/{uuid4().hex}"
    store = PostgresGraphStore(dsn=dsn)
    ...
    try:
        ...
        MemorySearchIndexSync(dsn=dsn).sync_memory("preference:old", graph_id=graph_id)
        MemorySearchIndexSync(dsn=dsn).sync_memory("preference:new", graph_id=graph_id)

        fetched = query.fetch_nodes(["preference:old", "preference:new"], graph_id=graph_id)
        ...
        assert (memory.SUPERSEDES.name, "preference:old") in by_id["preference:new"].outgoing
