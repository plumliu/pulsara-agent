...
LONG_SKILL_SCRIPT = Path(
    ".pulsara/skills/test-failure-diagnoser/scripts/run_failure_check.py"
)
    ...
    async def next_action(
        ...
        *,
        ...
        context = test_llm_context(
            ...
            messages=(
                LLMMessage.user(json.dumps(state, ensure_ascii=False, indent=2)),
            ),
            tools=(),
        )
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            options = LLMOptions()
            target = self._runtime.resolve_target(
                role=ModelRole.FLASH,
                requested_options=options,
            )
            call = self._runtime.resolve_call(
                target=target,
            ...
            if errors:
                ...
                raise AssertionError(
                    f"user simulator LLM failed after {max_attempts} attempts: {attempt_diagnostics}"
                )
            try:
...
def test_real_pr4_dogfood_llm_user_small_loop(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert result["ok"], (
        f"{result.get('error', 'dogfood failed')}\n{result['evidence']}"
    )


def test_real_pr4_dogfood_llm_user_long_session(
    ...
    result = asyncio.run(
        asyncio.wait_for(
            ...
            # Keep an explicit override for slow-provider diagnosis, but retain
            # the 15-minute default so publisher/runtime deadlocks fail bounded.
            timeout=float(os.getenv("PULSARA_DOGFOOD_LONG_TIMEOUT_SECONDS", "900")),
        )
    )
...
async def _run_real_pr4_dogfood_llm_user_small_loop(
    ...
    try:
        ...
        last_text = await _run_round2_write_summary(
            session, simulator, evidence, last_text
        )
...
async def _run_real_pr4_dogfood_llm_user_long_session(
    ...
    try:
        ...
        last_text = await _run_long_round2_create_skill(
            session, simulator, evidence, last_text
        )
        _verify_skill_catalog_contains(
            session, LONG_SKILL_NAME, "round2_created_skill_visible", evidence
        )
        last_text = await _run_long_round3_use_skill(
            session, simulator, evidence, last_text
        )
        await _run_long_round4_active_stop(session, simulator, evidence, last_text)
        last_text = await _run_long_round5_continue_after_stop(
            session, simulator, evidence
        )
        ...
        await _inject_controlled_failed_run(
            session, "A controlled provider failure happened during dogfood."
        )
        evidence.turns.append(
            {
        ...
        last_text = await _run_long_round9_failure_note(
            session, simulator, evidence, last_text
        )
        evidence.turns.append(
            {
...
def _verify_round0_inspect_baseline(session, evidence: DogfoodEvidence) -> None:
    ...
    _require(
        "write_file" in registry_names,
        "round0 expected write_file in registry",
        evidence,
    )
    _require(
        "terminal" in registry_names, "round0 expected terminal in registry", evidence
    )
    _require(
        "terminal_process" in registry_names,
        "round0 expected terminal_process in registry",
        evidence,
    )
    _require(
    ...
    _require(
        session.get_pending_approval() is None,
        "round0 expected no pending approval",
        evidence,
    )
    _require(
        session.replay_events() == [],
        "round0 inspect baseline should not write runtime events",
        evidence,
    )
...
async def _run_round1_orientation(
    ...
    _require(
        ...
        "round1 simulator did not produce user_message",
        evidence,
    )
    _require(
    ...
    _require(
        not _confirm_events(run_events),
        "round1 read-only orientation triggered approval",
        evidence,
    )
    _require(
    ...
    _require(
        "pulsara" in final_text.lower(),
        "round1 final text did not identify Pulsara",
        evidence,
    )
    _require(
        session.get_pending_approval() is None,
        "round1 left a pending approval",
        evidence,
    )
    return final_text
...
async def _run_round2_write_summary(
    ...
    _require(
        action.type == "user_message" and action.text,
        "round2 simulator did not produce user_message",
        evidence,
    )
    _require(
    ...
    _require(
        first.status.value == "waiting_user", "round2 expected waiting_user", evidence
    )
    ...
    _require(
        approval_action.type == "approve",
        "round2 simulator did not approve safe write request",
        evidence,
    )
    _require(
    ...
    _require(
        resolved.status.value == "finished",
        "round2 approval did not finish run",
        evidence,
    )
    _require(artifact.exists(), "round2 summary artifact was not written", evidence)
    text = artifact.read_text(encoding="utf-8")
    _require(
        "pulsara" in text.lower(),
        "round2 summary artifact did not mention Pulsara",
        evidence,
    )
    _require(
        "agent" in text.lower(),
        "round2 summary artifact did not mention agent runtime",
        evidence,
    )
    _require(
        len(_confirm_events(resolved_events)) == 1,
        "round2 expected one RequireUserConfirmEvent",
        evidence,
    )
    _require(
        len(
    ...
    _require(
        session.get_pending_approval() is None,
        "round2 left a pending approval",
        evidence,
    )
    return final_text
...
async def _run_round3_terminal_ask(
    ...
    _require(
        action.type == "user_message" and action.text,
        "round3 simulator did not produce user_message",
        evidence,
    )
    _require(
    ...
    _require(
        first.status.value == "waiting_user", "round3 expected waiting_user", evidence
    )
    ...
    _require(
        approval_action.type == "approve",
        "round3 simulator did not approve safe terminal request",
        evidence,
    )
    _require(
        approval_action.approval_id == pending.approval_id,
    ...
    terminal_payload = next(
        (
            ...
            and SMALL_LOOP_SENTINEL in str(payload.get("output", ""))
        ),
        None,
    )
    evidence.turns.append(
    ...
    _require(
        resolved.status.value == "finished",
        "round3 approval did not finish run",
        evidence,
    )
    _require(
        terminal_payload is not None,
        "round3 terminal result did not include sentinel",
        evidence,
    )
    _require(
        SMALL_LOOP_SENTINEL in final_text,
        "round3 final text did not include terminal sentinel",
        evidence,
    )
    _require(
    ...
    _require(
        session.get_pending_approval() is None,
        "round3 left a pending approval",
        evidence,
    )
...
async def _run_long_round1_orientation(
    ...
    _require(
        action.type == "user_message" and action.text,
        "long round1 simulator did not produce user_message",
        evidence,
    )
    result = await _run_turn_with_trace(
        session, action.text or "", evidence, "long_round1_orientation"
    )
    run_events = _events_for_run(session.replay_events(), result.state.run_id)
    evidence.turns.append(
        _turn_evidence(
    ...
    _require(
        not _confirm_events(run_events), "long round1 triggered approval", evidence
    )
    _require(
    ...
    _require(
        "pulsara" in result.final_text.lower(),
        "long round1 did not identify Pulsara",
        evidence,
    )
...
async def _run_long_round2_create_skill(
    ...
    _require(
        action.type == "user_message" and action.text,
        "long round2 simulator did not produce user_message",
        evidence,
    )
    _require(
        LONG_SKILL_NAME in (action.text or ""),
        "long round2 user message omitted skill name",
        evidence,
    )

    result = await _run_turn_with_trace(
        session, action.text or "", evidence, "long_round2_create_skill"
    )
    ...
    while result.status.value == "waiting_user":
        ...
        approval_count += 1
        _require(
            approval_count <= 4, "long round2 exceeded approval loop budget", evidence
        )
    ...
    _require(
        result.status.value == "finished",
        "long round2 did not finish after approvals",
        evidence,
    )
    _require(
        approval_count >= 2,
        "long round2 expected multiple suspend/resume approvals",
        evidence,
    )
    _require(
        (session.workspace.workspace_root / LONG_SKILL_MD).exists(),
        "long round2 did not write SKILL.md",
        evidence,
    )
    _require(
        (session.workspace.workspace_root / LONG_SKILL_SCRIPT).exists(),
        "long round2 did not write skill script",
        evidence,
    )
    _require(
        session.get_pending_approval() is None,
        "long round2 left a pending approval",
        evidence,
    )
    return result.final_text.strip()
...
async def _run_long_round3_use_skill(
    ...
    _require(
        action.type == "user_message" and action.text,
        "long round3 simulator did not produce user_message",
        evidence,
    )
    _require(
        LONG_SKILL_NAME in (action.text or ""),
        "long round3 user message omitted skill name",
        evidence,
    )

    first = await _run_turn_with_trace(
        session, action.text or "", evidence, "long_round3_use_skill"
    )
    pending = session.get_pending_approval()
    run_events = _events_for_run(session.replay_events(), first.state.run_id)
    evidence.turns.append(
    ...
    _require(
        first.status.value == "waiting_user",
        "long round3 expected terminal approval",
        evidence,
    )
    _require(pending is not None, "long round3 expected pending approval", evidence)
    _validate_exact_terminal_pending(
        pending, LONG_FAILURE_COMMAND, evidence, "long round3"
    )
    ...
    _require(
        resolved.status.value == "finished", "long round3 did not finish", evidence
    )
    _require(
        LONG_FAILURE_TEXT in output,
        "long round3 terminal output did not include failure text",
        evidence,
    )
    _require(
        LONG_FAILURE_TEXT in resolved.final_text
        or "ZeroDivisionError" in resolved.final_text,
        "long round3 final text did not summarize failure",
        evidence,
    )
    _require(
        "terminal_process" not in _tool_names(resolved_events),
        "long round3 should not use terminal_process",
        evidence,
    )
...
async def _run_long_round4_active_stop(
    ...
    _require(
        action.type == "user_message" and action.text,
        "long round4 simulator did not produce user_message",
        evidence,
    )
    ...
    _require(
        stop_result is not None and stop_result.status.value == "aborted",
        "long round4 stop did not abort",
        evidence,
    )
    _require(
        result.status.value == "aborted",
        "long round4 run result was not aborted",
        evidence,
    )
    _require(
        sum(
            ...
            if isinstance(event, RunEndEvent) and event.status == "aborted"
        )
        == 1,
    ...
    _require(
        not session._run_lock.locked(), "long round4 left the run lock locked", evidence
    )
    _require(
        session.get_pending_approval() is None,
        "long round4 left a pending approval",
        evidence,
    )
...
async def _run_long_round5_continue_after_stop(
    ...
    _require(
        action.type == "user_message" and action.text,
        "long round5 simulator did not produce user_message",
        evidence,
    )
    result = await _run_turn_with_trace(
        session, action.text or "", evidence, "long_round5_continue_after_stop"
    )
    run_events = _events_for_run(session.replay_events(), result.state.run_id)
    evidence.turns.append(
        _turn_evidence(
    ...
    _require(
        LONG_INTERRUPTED_SENTINEL in result.final_text,
        "long round5 did not see interrupted note",
        evidence,
    )
    return result.final_text.strip()
...
async def _run_long_round6_stop_pending(
    ...
    _require(
        action.type == "user_message" and action.text,
        "long round6 simulator did not produce user_message",
        evidence,
    )
    first = await _run_turn_with_trace(
        session, action.text or "", evidence, "long_round6_stop_pending"
    )
    pending = session.get_pending_approval()
    run_events = _events_for_run(session.replay_events(), first.state.run_id)
    evidence.turns.append(
    ...
    _require(
        first.status.value == "waiting_user",
        "long round6 expected pending approval",
        evidence,
    )
    _require(
        pending is not None, "long round6 expected pending approval object", evidence
    )
    _validate_exact_terminal_pending(
        pending, LONG_PENDING_COMMAND, evidence, "long round6"
    )
    stop_action = DogfoodUserAction(type="stop")
    evidence.simulator_actions.append(
        _harness_action_evidence("long_round6_stop", stop_action)
    )
    ...
    _require(
        stopped is not None and stopped.status.value == "aborted",
        "long round6 stop did not abort",
        evidence,
    )
    _require(
        session.get_pending_approval() is None,
...
async def _run_long_round7_deny_pending(
    ...
    _require(
        action.type == "user_message" and action.text,
        "long round7 simulator did not produce user_message",
        evidence,
    )
    first = await _run_turn_with_trace(
        session, action.text or "", evidence, "long_round7_deny_pending"
    )
    pending = session.get_pending_approval()
    run_events = _events_for_run(session.replay_events(), first.state.run_id)
    evidence.turns.append(
    ...
    _require(
        first.status.value == "waiting_user",
        "long round7 expected pending approval",
        evidence,
    )
    _require(
        pending is not None, "long round7 expected pending approval object", evidence
    )
    _validate_exact_terminal_pending(
        pending, LONG_PENDING_COMMAND, evidence, "long round7"
    )
    deny_action = _deny_action_from_pending(
        pending, reason="scenario denies this command"
    )
    evidence.simulator_actions.append(
        _harness_action_evidence("long_round7_denial", deny_action)
    )
    resolved = await _resolve_approval_with_trace(
        session,
    ...
    _require(
        resolved.status.value == "finished",
        "long round7 did not finish after denial",
        evidence,
    )
    _require(
        LONG_DENY_SENTINEL in resolved.final_text,
        "long round7 final text did not handle denial",
        evidence,
    )
    _require(
        any(
...
async def _run_long_round9_failure_note(
    ...
    _require(
        action.type == "user_message" and action.text,
        "long round9 simulator did not produce user_message",
        evidence,
    )
    result = await _run_turn_with_trace(
        session, action.text or "", evidence, "long_round9_failure_note"
    )
    run_events = _events_for_run(session.replay_events(), result.state.run_id)
    evidence.turns.append(
        _turn_evidence(
    ...
    _require(
        LONG_FAILURE_NOTE_SENTINEL in result.final_text,
        "long round9 did not see failure note",
        evidence,
    )
    return result.final_text.strip()


def _create_python_workspace_temp_root(
    prefix: str = "pulsara-dogfood-pr4-small-",
) -> Path:
...
def _pending_summary(pending: PendingApproval) -> DogfoodPendingApprovalSummary:
    ...
    return DogfoodPendingApprovalSummary(
        approval_id=pending.approval_id, tool_calls=tuple(calls)
    )
...
def _approval_resolution(
    ...
    if requested_ids != visible_ids:
        raise ValueError(
            f"approval ids mismatch: requested={sorted(requested_ids)} visible={sorted(visible_ids)}"
        )
...
def _denial_resolution(
    ...
    if requested_ids != visible_ids:
        raise ValueError(
            f"denial ids mismatch: requested={sorted(requested_ids)} visible={sorted(visible_ids)}"
        )
    return ApprovalResolution(
        approval_id=pending.approval_id,
        decisions=tuple(
...
def _validate_write_summary_pending(
    pending: PendingApproval, evidence: DogfoodEvidence
) -> None:
    _require(
        len(pending.tool_calls) == 1,
        "round2 expected exactly one pending tool call",
        evidence,
    )
    ...
    _require(
        ...
        "round2 expected write_file pending approval",
        evidence,
    )
    _require(
        args.get("path") == str(SUMMARY_ARTIFACT),
        "round2 write_file path was not allowlisted",
        evidence,
    )
    _require(
        isinstance(content, str), "round2 write_file content was not a string", evidence
    )
    lower_content = content.lower() if isinstance(content, str) else ""
    _require(
        "pulsara" in lower_content,
        "round2 write_file content did not mention Pulsara",
        evidence,
    )
    _require(
        "agent" in lower_content,
        "round2 write_file content did not mention agent runtime",
        evidence,
    )
    _require(
        ".env" not in lower_content,
        "round2 write_file content unexpectedly mentioned .env",
        evidence,
    )


def _validate_long_skill_pending(
    pending: PendingApproval, evidence: DogfoodEvidence
) -> None:
    allowed_paths = {str(LONG_SKILL_MD), str(LONG_SKILL_SCRIPT)}
    _require(
        pending.tool_calls,
        "long round2 expected at least one pending skill write",
        evidence,
    )
    for call in pending.tool_calls:
        args = _parse_json_object(call.input)
        ...
        _require(
            call.name == "write_file",
            "long round2 expected write_file pending approval",
            evidence,
        )
        _require(
            path in allowed_paths,
            f"long round2 write path was not allowlisted: {path}",
            evidence,
        )
        _require(
            isinstance(content, str),
            "long round2 write content was not a string",
            evidence,
        )
        lower_content = content.lower() if isinstance(content, str) else ""
        if path == str(LONG_SKILL_MD):
            _require(
                LONG_SKILL_NAME in lower_content,
                "long round2 SKILL.md omitted skill name",
                evidence,
            )
            _require(
                content.lstrip().startswith("---"),
                "long round2 SKILL.md omitted YAML frontmatter",
                evidence,
            )
            _require(
                "name: test-failure-diagnoser" in lower_content,
                "long round2 SKILL.md omitted name field",
                evidence,
            )
            _require(
                "description:" in lower_content,
                "long round2 SKILL.md omitted description field",
                evidence,
            )
            _require(
                LONG_FAILURE_COMMAND in content,
                "long round2 SKILL.md omitted exact command",
                evidence,
            )
        if path == str(LONG_SKILL_SCRIPT):
            _require(
                "pulsara" in lower_content
                or "pytest" in lower_content
                or "failure" in lower_content,
                "long round2 script content did not look like a failure helper",
                evidence,
            )
        _require(
            ".env" not in lower_content,
            "long round2 content unexpectedly mentioned .env",
            evidence,
        )


def _validate_terminal_pending(
    pending: PendingApproval, evidence: DogfoodEvidence
) -> None:
    _require(
        len(pending.tool_calls) == 1,
        "round3 expected exactly one pending tool call",
        evidence,
    )
    call = pending.tool_calls[0]
    args = _parse_json_object(call.input)
    _require(
        call.name == "terminal", "round3 expected terminal pending approval", evidence
    )
    _require(
...
def _validate_exact_terminal_pending(
    ...
    _require(
        len(pending.tool_calls) == 1,
        f"{label} expected exactly one pending tool call",
        evidence,
    )
    call = pending.tool_calls[0]
    args = _parse_json_object(call.input)
    _require(
        call.name == "terminal", f"{label} expected terminal pending approval", evidence
    )
    _require(
        args.get("command") == command,
        f"{label} terminal command was not allowlisted",
        evidence,
    )
...
async def _wait_for_active_run_start(session, task, evidence: DogfoodEvidence) -> str:
    ...
    while time.monotonic() < deadline:
        if task.done():
            raise AssertionError(
                f"active-stop run finished before it could be stopped\nEvidence:\n{evidence.to_json()}"
            )
...
def _verify_skill_catalog_contains(
    ...
    _require(
        skill_name in catalog_names,
        f"expected skill catalog to contain {skill_name}",
        evidence,
    )


async def _inject_controlled_failed_run(session, user_input: str) -> None:
    ctx = EventContext(
...
def _verify_latest_terminal_note(
    *,
    ...
    if kind == "failure":
        ...
        _require(
            interrupted_count == 0,
            "failure note should supersede older interrupted note",
            evidence,
        )
    else:
        _require(
            interrupted_count == 1, "expected exactly one interrupted note", evidence
        )
        _require(
            failure_count == 0,
            "interrupted note check unexpectedly saw a failure note",
            evidence,
        )
...
def _verify_long_round10_final_inspect(session, evidence: DogfoodEvidence) -> None:
    ...
    _verify_skill_catalog_contains(
        session, LONG_SKILL_NAME, "long_round10_final_skill_catalog", evidence
    )
    evidence.turns.append(
        {
    ...
    _require(
        summary["pending_approval"] is None,
        "long round10 expected no pending approval",
        evidence,
    )
    _require(
        summary["stopping_run_id"] is None,
        "long round10 expected no stopping run",
        evidence,
    )
    _require(
        summary["active_run_id"] is None,
        "long round10 expected no active run",
        evidence,
    )
...
def _trace(evidence: DogfoodEvidence, message: str, **fields: object) -> None:
    ...
    print(
        "[dogfood-trace] " + json.dumps(payload, ensure_ascii=False, sort_keys=True),
        flush=True,
    )
...
def _session_trace(session) -> dict[str, object]:
    ...
    return {
        ...
        "pending_tool_names": [call.name for call in pending.tool_calls]
...
def _publisher_trace(session) -> dict[str, object]:
    ...
    return {
        ...
        "drain_done": publisher._drain_task.done()
        if publisher._drain_task is not None
        else None,
        "errors": [
            f"{type(error).__name__}: {error}" for error in publisher.errors[-3:]
        ],
    }
...
def _harness_action_evidence(
    phase: str, action: DogfoodUserAction
) -> dict[str, object]:
    return {
        "phase": phase,
...
def _parse_user_action(raw_text: str) -> DogfoodUserAction:
    ...
    if action_type not in {"user_message", "approve", "deny", "stop", "finish"}:
        raise ValueError(
            f"invalid user simulator action type: {action_type!r}; raw={raw_text!r}"
        )
...
def _extract_json_object_text(raw_text: str) -> str:
    ...
    if start == -1 or end == -1 or end <= start:
        raise ValueError(
            f"could not find JSON object in simulator output: {raw_text!r}"
        )
