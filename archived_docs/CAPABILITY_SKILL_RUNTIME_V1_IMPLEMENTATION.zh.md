...
- Host / Runtime 可以解析本地 filesystem skill roots：`.pulsara/skills` 和 `.agents/skills`。
...
V1 不把 `allowed_scopes` / `blocked_scopes` 暴露为 skill 作者可写 frontmatter。项目级 skill 属于哪个 workspace 已由 `<workspace>/.pulsara/skills` 或 `<workspace>/.agents/skills` 文件位置表达；用户级 skill 属于 `${PULSARA_HOME}/skills` 或 `~/.agents/skills`。让作者手写 `ctx:workspace/<hash>` 只能 no-op 或把 skill 写到不可见状态。
...
`location` 第一轮必须使用非绝对 display path，例如 `.agents/skills/review-pr/SKILL.md`、`.pulsara/skills/review-pr/SKILL.md`、`~/.agents/skills/review-pr/SKILL.md` 或 `~/.pulsara/skills/review-pr/SKILL.md`。不要把 host absolute path 放进 prompt；这样既减少本机路径泄漏，也降低模型尝试 out-of-workspace 绝对路径读取的诱因。实际读取仍由现有 workspace/file gate 兜底。
...
V1 按 `PULSARA_DIRECTORY_CONTRACT_CODEX_COMPAT.zh.md` 的 Codex-current-compatible 目录契约扫描：
...
不扫描其他 agent 的 product home，不扫描 plugin。第一轮暂不做 parent directory traversal。原因：

- workspace product/shared skills 已足够验证契约。
- user `${PULSARA_HOME}/skills` 与 Codex 当前 `$CODEX_HOME/skills` 行为对齐；user `~/.agents/skills` 是明确的共享 skill root。
- admin/plugin/parent roots 会引入 precedence、trust、permission、distribution 问题。
...
dot 子目录跳过普通扫描；例如 `~/.pulsara/skills/.system` 未来如需作为 bundled/system root，应作为独立 root 注入，而不是被普通 user root 扫描。

### 4.2 Discovery

流程：

1. 检查 `<workspace_root>/.pulsara/skills`、`<workspace_root>/.agents/skills`、`${PULSARA_HOME}/skills`、`~/.agents/skills` 是否存在。
2. 按 root precedence 枚举子目录，读取 `SKILL.md`。
3. 使用 `PyYAML.safe_load` 解析 YAML frontmatter；只接受 mapping。
4. validate `name`、`description`、tool refs。
5. 根据 root kind 做 V1 可见性判断：workspace roots 仅对当前 workspace 可见，user roots 对该用户的所有 workspace 可见；`memory_domain` / `workspace_kind` 字段已穿到 resolver context，但 scope filter 在 V1 只作为 future provider 预留，不做 active filtering。
6. 生成非绝对 `ResolvedSkillCatalogEntry.location`。
...
- workspace `.pulsara/skills` / `.agents/skills` 只在当前 workspace resolver 中可见；user `${PULSARA_HOME}/skills` / `~/.agents/skills` 在该用户的所有 workspace resolver 中可见。
- project workspace 的 memory domain 仍然是 `ctx:user` + `ctx:workspace/<hash>`；transient workspace 仍然只有 `ctx:user`。
- `memory_domain` / `workspace_kind` 已进入 `CapabilityResolveContext`，但在 V1 local filesystem provider 中是 dormant 字段：它们用于 diagnostics/future providers，不参与当前 root-kind visibility。
...
<available_skills>
  <skill>
    ...
    <location>.agents/skills/review-pr/SKILL.md</location>
  </skill>
</available_skills>
...
Source: .agents/skills/review-pr/SKILL.md
...
Skill directory: .agents/skills/review-pr
...
- `local_skills.py`：扫描和解析 `.pulsara/skills` 与 `.agents/skills` roots。
...
- project workspace：读取 `<workspace_root>/.pulsara/skills`、`<workspace_root>/.agents/skills`、`${PULSARA_HOME}/skills`、`~/.agents/skills`
- transient workspace：也可读取该 transient root 下的 `.pulsara/skills` / `.agents/skills` 以及 user roots，但无 workspace memory scope。
...
- 有效 `.pulsara/skills/foo/SKILL.md` 和 `.agents/skills/foo/SKILL.md` 能解析 name/description/path/base_dir。
...
- workspace `.pulsara/skills` / `.agents/skills` 只从当前 workspace root 发现；user `${PULSARA_HOME}/skills` / `~/.agents/skills` 从用户 roots 发现。
...
- catalog location 是非绝对 display path，不包含 host absolute path。
...
- 仓库根 `.pulsara/skills/foo/SKILL.md` 和 `.agents/skills/foo/SKILL.md` 能被 host/runtime 发现。
...
- catalog location 使用非绝对 display path，catalog wrapper escaping 有测试覆盖。
