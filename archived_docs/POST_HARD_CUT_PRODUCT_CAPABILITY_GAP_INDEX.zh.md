...
Foreground execution仍由一个runner决定“下一步做什么”，但turn admission、provider dispatch、compaction、Plan control batch、tool execution、memory与steer settlement各由唯一窄coordinator拥有。该结构清理只收紧import与authority边界，不改变canonical transaction、prefix continuity、physical effect ordering或cross-restart replay语义，也不增加compatibility facade、fingerprint或durable recovery owner。机器证据见[`production_code_cleanup_and_runner_decomposition_activation.json`](benchmarks/suites/core/v1/production_code_cleanup_and_runner_decomposition_activation.json)。
...
### 7.4 Round 9与Round 9.1均已激活

Round 6之后暴露出的命名/规划缺口已经由Round 9收口：旧tool-only `CapabilityDescriptor/CapabilityExposurePlan`、skill-only `KernelCapabilityComposer/CAPABILITY_CATALOG`及旁路surface API已删除或诚实收窄；execution-backed Builtin、每server MCP与单一聚合`LOCAL_SKILL_CATALOG`由三个原owner分别签发immutable snapshot，再由唯一pure registry合并。三项snapshot在同一dispatch absolute deadline内顺序冻结，但不虚构跨owner原子capture瞬间，也不新增composition generation或current-seal handshake。

该能力族按两轮处理；Round 9与Round 9.1均已ACTIVATED：

- [Round 9 unified capability semantics](ROUND_9_UNIFIED_CAPABILITY_SEMANTICS_IMPLEMENTATION_SPEC.zh.md)：三类能力共享`source registration -> complete source snapshot -> pure frozen registry -> exposure planning`，但execution authority仍归Builtin binding owner与MCP supervisor，Skill没有executor。Exact target/profile-aware native preflight先于parent dispatch cut；Tool planner与Skill composer只消费该cut派生的sibling views。Cold native-eligible MCP cohort按canonical和actual-wire双界all-direct或all-meta；native-wire-incompatible与late-ready工具走`inspect_new_mcp_tool/use_new_mcp_tool`，descriptor overbound或physical/schema drift typed unavailable。Ref exact绑定scope/epoch/tool/policy/route，只有Round 7.1 FULL安装后可调用。Final wire plan与continuity CAS逐字节证明same-epoch SYSTEM/tools稳定、messages只追加。机器证据见[`round9_unified_capability_semantics_activation.json`](benchmarks/suites/core/v1/round9_unified_capability_semantics_activation.json)。
- [Round 9.1 Agent Skills Standard](ROUND_9_1_AGENT_SKILLS_STANDARD_IMPLEMENTATION_SPEC.zh.md)：以Agent Skills开放标准替换Pulsara legacy Skill manifest；只保留Round 9冻结的四个physical roots（workspace/user各自的`.pulsara/skills`与`.agents/skills`），由Skill owner执行一次bounded global precedence scan，并向registry提交一个聚合`LOCAL_SKILL_CATALOG` snapshot。Catalog只投影完整`name + description + location`，explicit/configured activation投影exact parsed Markdown body；两者都是append-only、untrusted observation。模型驱动的progressive disclosure只调用普通`read_file`，其default/max窗口均为2,000行且重复读取返回current bytes；Skill不拥有executor、permission、dependency/health route或loaded-state。机器证据见[`round9_1_agent_skills_standard_activation.json`](benchmarks/suites/core/v1/round9_1_agent_skills_standard_activation.json)。Round 5B只消费已安装normal source，不反向建立loaded-state。

[Round 9.2 independent Hook subsystem](ROUND_9_2_HOOK_SUBSYSTEM_IMPLEMENTATION_SPEC.zh.md)先以USER `${PULSARA_HOME}/hooks.json`与exact WORKSPACE `.pulsara/hooks.json`形成真实custom-Hook产品路径：唯一generic trust、11项`HookEventType`、matcher/dispatcher/executor/output parser与source-neutral `HOOK_CONTEXT`均在`hooks/`平级子系统中实现。它不复用Committed/Live event，不拥有repository append或event-replay authority；Round 5B/10只提供Pre/PostCompact、SessionStart(compact)、SubagentStart/Stop等existing exact lifecycle seam。固定ROOT `reload_hooks`只替换future-event view，不热改SYSTEM/tools，不建立watcher、durable Hook history或exactly-once recovery。Architecture oracle因此诚实扩展第七维`Hook event types = 11`：前三套event vocabulary各自closed、各自计数，而不是把Hook lifecycle enum隐藏在既有六维oracle之外。

Plugin不进入Round 9/9.1 capability leaf。[Round 9.3 Agent Plugin bundle与Hook adapter](ROUND_9_3_AGENT_PLUGIN_BUNDLE_AND_HOOK_ADAPTER_IMPLEMENTATION_SPEC.zh.md)把Plugin建模为Agent Plugins 1.0 portable core与Codex-first/Claude-overlap宿主适配层：启用后把Skill物化到Round 9.1现有四根目录之一，向Round 9唯一MCP config inventory贡献definitions，并把normalized Plugin Hook definitions/provenance/environment overlay交给Round 9.2，不增加Plugin-private trust、matcher、dispatcher或第三套tool executor。Host composition在每次complete Skill scan前按current Plugin state同步reconcile managed projection；existing `LocalSkillProvider`仍不感知Plugin。固定ROOT `reload_plugins`只让current Host采用CLI已提交Plugin state并复用Skill successor/MCP late-meta/generic Hook future-view replacement；它不安装、启用或trust package，也不热改tools[]。Claude simple Subagent preset通过opaque ref进入Round 10 existing worker cold seed，不保留dormant inventory。
...
[前后代码确认] hard-cut前这是一条已经接入真实Agent model loop的production路径；hard-cut曾只保留canonical transcript rematerialization、最终provider materialization和一个很薄的skill prompt composer。Round 3已在当前canonical Kernel上重新建立provider-neutral typed compiler，而没有恢复旧durable context-input audit、provider-input replay或execution recovery graph；机器证据见[`round3_structured_model_input_compiler_activation.json`](benchmarks/suites/core/v1/round3_structured_model_input_compiler_activation.json)。Round 3.1随后恢复同一Host、同一exact scope内的append-only source/tool-result lifecycle，机器证据见[`round3_1_provider_input_prefix_continuity_activation.json`](benchmarks/suites/core/v1/round3_1_provider_input_prefix_continuity_activation.json)。Round 5A.1又让continuity CAS同时安装semantic view与plan-first actual-wire proof，并在assistant canonical FULL后绑定same-epoch exact reasoning fragment；机器证据见[`round5a1_provider_neutral_model_output_termination_activation.json`](benchmarks/suites/core/v1/round5a1_provider_neutral_model_output_termination_activation.json)。Round 5A.2现已把completed、entry-bound native carrier与assistant同事务持久化，并为fresh Host对compatible target恢复exact Chat/Responses replacement；机器证据见[`round5a2_durable_provider_replay_and_cross_restart_thread_continuation_activation.json`](benchmarks/suites/core/v1/round5a2_durable_provider_replay_and_cross_restart_thread_continuation_activation.json)。它仍不恢复exact historical request audit、remote provider state或execution recovery graph。
...
| provider materialization与最终估算 | [direct_model.py](src/pulsara_agent/conversation_kernel/direct_model.py)保留transport-bearing `PreparedKernelModelCall`；exact join后先冻结唯一`FrozenProviderWireInputPlan`，continuity CAS与physical adapter消费同一materialization identity，不能二次重建 | **compiler estimate、wire replacement quote与pre-send validation闭合** |
...
结论：Context Compiler Input与ContextSource文档有明确历史产品证据，`5b7ad9f7`也存在真实production owner和tests，因而形成PHC-17；Round 3已在canonical Kernel上恢复单次typed compilation，Round 3.1继续恢复同Host同scope的append-only prefix lifecycle，Round 5A.2又恢复compatible target的entry-bound native replay跨Host/进程重启。long-horizon compaction仍形成PHC-07B；tool timing由PHC-14拥有且已恢复。Exact context-input audit、完整compiled prefix snapshot与旧process-local epoch identity跨Host恢复仍明确不承诺，不计缺口；provider cache accounting仍只是operational observation，不替代typed compiler或本地strict-prefix proof；Context Timing Header只有计划证据，未单列。
