...
>
> Extension治理补订：extension只能订阅closed core AgentEvent projection，不能定义或发布新的Committed/Live AgentEvent；Stage 2 base为49类，Round 2 active target为50类。普通hook无catch-up且不能以`reliable=true`升级，V1第三方durable extension action为0。唯一pre-dispatch policy kind冻结为`ToolDispatchAuthorizationPolicy`，只返回Allow/Deny/RequireConfirmation，不重写已canonical commit的tool arguments。
>
> Round 2 Terminal产品扩展补订：Stage 2 activation时的26 Committed / 23 Live / 49 formal是历史cutover oracle，不是永久类型上限。恢复same-Host `terminal_monitor`后，physical process completion、monitor registration/progress/close仍只在23类Live plane；只有current Host把bounded observation接受为`TERMINAL_OBSERVATION` canonical entry时，才同transaction追加新的`TerminalObservationAccepted`。active target因此为27 Committed / 23 Live / 50 formal，subject slots仍13、append guards仍2；下文未显式标注“Stage 2历史基线”的26/49口径均由本补订supersede。
>
...
- Host存活期间已经进入canonical tool-result/entry的terminal completion/termination事实；terminal process与monitor本身只保留process-local typed lifecycle，不为physical lifecycle另设committed event，也不保留跨Host rebind authority；若current Host把bounded monitor observation接受为独立`TERMINAL_OBSERVATION` conversation entry，则由同transaction的`TerminalObservationAccepted`记录该acceptance occurrence；
...
| **合计** | **39** | **25** | **16** | **71** | 这是旧type的主处置，不是最终registry数量；39个A候选经semantic dedup并删除可推导的unknown occurrence后形成Stage 2的26类core；Round 2为新恢复的canonical Terminal observation acceptance另增1类，不复活旧9类terminal event |
...
6. `SUBAGENT_RUN_*`不与task lifecycle并存。`SubagentTaskAccepted`只接受initial `PENDING`；后续`ACTIVE | COMPLETED | FAILED | CANCELLED | INTERRUPTED`走一个closed `SubagentTaskStatusAccepted`。dependency wait与phase只走`SubagentProgress` live event，dependency failure归`FAILED(reason=DEPENDENCY_FAILED)`。message/result各有exact canonical child subject，不能继续只引用task aggregate或在payload放自由ordinal。

因此“39个A候选”不能被理解为保留39个旧class，更不能把old payload复制到新event。决策7的26类表是Stage 2 base registry；Round 2只按相同admission test增加`TerminalObservationAccepted`形成active 27类。本节的A/B/C/D只是从旧151类到目标plane/row的migration evidence。
...
- **[设计建议]** 这项优势必须以一组可测contract兑现：same-transaction canonical+event、closed Host/job append guard、exact typed subject FK、ephemeral stored-event/exact-subject observation projection、provider/control bounded GAP/detach、redacted capability projection、ordinary-hook registration-cut/no-catch-up、active closed 50-type subscription-only extension、hook failure isolation、V1 generic reliable action为0及future reliable-action-as-named-job。缺任何关键项都不能把“统一协议”写成既成产品优势。
...
压缩为3段：stop ingress；bounded cancel/join foreground并把turn/task写interrupted，tool outcome unknown仅由immutable attempt + missing result + interrupted turn读取时派生；flush Host-owned transcript/tool/queue/job-control authorization commits后关闭session资源。background worker及其claim lease属于独立lifecycle，不由session close完成、释放或换代。
...
job进入`SUCCEEDED`与“结果已进入conversation”成为两个明确事实，可能有可见延迟；这是避免worker成为第二session writer所必须的产品边界，不需要receipt或reconciliation owner。
...
provider safe point只保护input构造、binding revision推进、下一次physical dispatch准入，以及会成为ROOT上下文输入的job/subagent external-result entry acceptance；它不是session-wide canonical write lock，也不得把数据库锁或排他的semantic-write lease从pre-dispatch read持有到provider stream结束。tool result、queue status、memory以及source job/task result自身可以在模型运行期间按各自authority正常commit，`provider_input_through_sequence`是accepted assistant判断这些mutation是否属于本次conversation cut的唯一因果边界。external-result ROOT entry是窄例外：它具有新ROOT input语义，不能在active prepared-input handle/model operation与其assistant commit之间插入。Host用一个短process-local admission lock线性化external acceptance transaction与下一handle freeze；accept先赢则新handle的cut必须覆盖该ROOT entry，freeze先赢则source继续留在job/task row直到下一safe point或显式new turn，不写ROOT acceptance command/entry/`UserMessageAccepted`、pending或receipt。这个lock不跨provider stream持有数据库锁，实现也不得借该例外假定“模型运行期间没有其他semantic write”或省略cut字段。
...
- 当前Host以writer generation显式接受job result后，结果才进入conversation；进入existing RUNNING ROOT还必须位于provider safe point；
...
1. stop ingress；
2. bounded cancel/join foreground及仍使用session资源的physical operation，把turn/task写interrupted；tool outcome unknown只在读取时派生；
...
| durable committed core vocabulary | 151类共用durable registry | 60–80 | **active 27类exact core（Stage 2 base 26 + Terminal acceptance 1）；extension另行namespaced/versioned** | ≤12 record type |
...
代码删除量是基于当前owner文件和调用面的inventory target，不是未经实施即可保证的精确LOC。产品表与净删`≥22k` LOC是暴露架构回弹、触发审查和衡量方向的预算；active committed core则是exact 27类结构gate。两者都不得靠删除live typing或合并无关payload取巧；correctness由单authority、same-transaction canonical+event、fencing、crash、side-effect、resume、observer isolation与reconnect行为gate决定。
...
- job result进入conversation必须由当前Host以writer generation显式接受；进入existing RUNNING ROOT时与prepared-input freeze走同一provider-safe admission；
...
**冻结的selective `CommittedAgentEvent` core vocabulary：active 27类（Stage 2 base 26 + Round 2 Terminal acceptance 1）。** Stage 2的26类由2.1.2的151类逐项生命周期审计得出；Round 2新增类型通过同一个admission test：transition必须已经被canonical owner接受、能exact引用同transaction中的canonical subject、其发生时间对用户/Inspector/audit/eval/post-commit hook有独立价值，并且即使完全禁止execution replay仍有意义。缺任一条件就只留row、进入Live/Operational plane或删除。
...
| conversation | `TerminalObservationAccepted` | current Host把bounded monitor observation接受为`TERMINAL_OBSERVATION` canonical entry；不复制output、cursor、monitor policy或process owner | `subject_entry_id` | `HostWriterGuard` |
| policy | `CapabilityDecisionAccepted` | machine policy的closed `ALLOW | DENY | REQUIRE_CONFIRMATION`已接受；最终human allow/deny只由`InteractionDecisionAccepted`表达；不复制raw context、rules或arguments，policy不得rewrite call | `subject_interaction_decision_id` | `HostWriterGuard` |
...
| coordination | `SubagentTaskAccepted` | Host接受task/objective及initial `PENDING`；不同时伪造active或dependency状态 | `subject_subagent_task_id` | `HostWriterGuard` |
| coordination | `SubagentTaskStatusAccepted` | task后续进入closed `ACTIVE | COMPLETED | FAILED | CANCELLED | INTERRUPTED`；dependency wait只发process-local `SubagentProgress`，dependency failure归`FAILED(reason=DEPENDENCY_FAILED)`；active不证明child process已启动 | `subject_subagent_task_id` | `HostWriterGuard` |
...
| work | `JobTerminalAccepted` | job aggregate进入closed `SUCCEEDED | FAILED | CANCELLED | OUTCOME_UNKNOWN`；中间retryable attempt terminal只留attempt row+C | `subject_job_id` | `JobAttemptClaimGuard` |
...
`PromptRejected`没有与`PromptCancelled`合并，因为前者是系统拒绝、后者是显式取消，actor、用户补救与hook语义不同。相反，job terminal、subagent task status和memory lifecycle使用closed enum，是因为它们分别属于同一aggregate transition family；这不是自由JSON或用一个`StateChanged`吞掉全域语义。上述closed enum由同一descriptor生成SQL CHECK、event payload与cross-language fixture：`CapabilityDecisionAccepted`只表示machine policy，human confirmation的最终decision不能复用它；subagent initial `PENDING`只随`SubagentTaskAccepted`接受，后续status event没有第二个pending alias；job terminal使用`SUCCEEDED`，不再同时出现`COMPLETED`。`MemoryCandidateAccepted`不进入core：candidate row与对应`ToolResultAccepted`/`JobQueued`已经闭合durable intake，governance直接claim canonical candidate而不是tail event。中间job-attempt terminal也不发A；attempt journal保存lineage，只有aggregate terminal才是selective extension occurrence。
...
live vocabulary独立冻结为exact 23类：Text/Thinking/Data/ToolCall各Start/Delta/End十二类，`ToolResultStart/Delta/End`三类，`InteractionOpened/Replaced/Closed`三类，`TerminalProcessCompleted`与`TerminalMonitorOpened/Observation/Closed`四类，以及`SubagentProgress`一类。ToolResult Delta是closed text/data variant；Terminal streaming Delta可为provisional text，End以同一block identity携带Round 1 final canonical preview作为authoritative replacement，但只有后续canonical transaction的`ToolResultAccepted`才表示durable接受。独立`RawProvider*`为0类；vendor SDK object不进入Runtime vocabulary，provider failure归typed terminal/usage结果或OperationalEvent。由此active正式AgentEvent vocabulary为exact 50类（27 committed + 23 live），OperationalEvent topics另行治理且不进入这个数量。
...
4. prompt queue item：pending / consumed / cancelled / rejected；
...
prompt queue也不保留旧`active/claimed` recovery state。每个item在session row lock下取得stable `queue_sequence`，并保存closed delivery-target union：`NEW_TURN`要求`target_turn_id IS NULL`，`STEER_ACTIVE_TURN`要求enqueue transaction锁定并保存当时exact、same-session且RUNNING的ROOT `target_turn_id`。item只允许`PENDING -> CONSUMED | CANCELLED | REJECTED`的单次CAS；consume与new turn或**被冻结target**上的steer entry及对应committed occurrences同transaction。延迟consume时target已经terminal，Host必须确定性写`REJECTED(reason=TARGET_TURN_TERMINAL)`并让FIFO前进，不能永久卡住、解析“当前active turn”或转投另一个turn。preparation、wake hint和reservation只在进程内；Host crash前未commit的item保持pending，新Host按最小queue sequence level-read继续，不恢复claim、checkpoint或delivery receipt。
...
V1 production catalog不把safety class留给registration caller临场选择：background compaction precompute、post-compaction memory extraction、memory governance与coalesced FTS/pgvector refresh均为`RETRY_SAFE`，`REMOTE_QUERYABLE` handler为0。前三种LLM-backed work只有在immutable input、attempt output在terminal winner前完全私有、winning transaction唯一安装snapshot/proposal/decision且禁止调用tool/remote mutation时才允许自动重调provider；重复provider计算/计费是这一显式选择的代价。若实现不能满足这些约束，该handler必须在activation时禁用，而不是改成隐式retry或由配置覆盖safety class。

`RETRY_SAFE`不是无限重试许可。每个job enqueue必须从closed handler catalog冻结`retry_policy_id/version`、finite `maximum_attempts`、`attempt_timeout`与deterministic backoff/`next_eligible_at`；LLM-backed handler还必须冻结每attempt的finite input/requested-output token cap。每个LLM attempt至多一次provider call：dispatch前current claim在attempt row以set-once admission保存该次request token数并校验cap，确认后才可调用provider。`maximum_attempts × 每attempt request上限`直接给出整个job的静态上界；同attempt ACK unknown读取marker且不得再次调用。attempt耗尽或request超cap时aggregate稳定进入`FAILED`并恰好产生一次`JobTerminalAccepted`。不保存跨attempt累计call/token counter，也不得为此恢复budget account、reservation/settlement、receipt或reducer graph。
...
background worker不属于rehydrate/open fencing domain：它只用job attempt的`claim_generation`提交progress/result/failure/lease settlement。Host takeover不应使合法worker result失败。worker也不得把result直接写进transcript；当前Host若要继续conversation，先读取immutable SUCCEEDED job/attempt result，再以当前writer generation创建显式accepted transcript item或新turn。accepted entry保存`source_job_id`并受session内唯一约束，或使用同等stable acceptance command id，确保accept ACK unknown不会把同一job result导入两次。若target是既有RUNNING ROOT turn，该accept还必须与prepared-input freeze走同一provider-safe admission；unsafe时source row保持不变，不创建entry/event/receipt。
...
- child已接受的conversation不伪造parent transcript，也不新建durable child session：它使用同一`session_id`下由exact `subagent_task_id`隔离的canonical conversation scope，复用turn、entry、assistant block、tool attempt/result与session entry/event allocator；每个task至多一个RUNNING task-scoped turn；root TUI默认只读ROOT scope，显式task view才读取该task scope；
- task result只有经当前Host显式接受为ROOT-scoped entry后才进入parent context：entry以exact `source_subagent_result_id` FK + session-local unique引用result child并追加`UserMessageAccepted(source=SUBAGENT_RESULT)`；compiler始终只读exact scope，不存在cross-scope binding table或JSON id list。既有RUNNING ROOT只允许在active prepared-input handle/model operation均为0且上一tool-request全部terminal的provider safe point接受，并与下一handle freeze短暂线性化；否则result仍留在child row。parent turn已经terminal时不得自动新建turn或转投其他active turn，result继续作为child可查询，直到显式new-turn accept command；
...
- background compaction/memory extraction按决策10冻结为retry-safe：输入由immutable source range/hash固定，attempt output在winning terminal transaction前不可见，按唯一snapshot/job identity提交；
...
这也意味着memory write不再需要跨storage publication语义。governance在一个PostgreSQL transaction中提交fact/relation/lifecycle并推进受影响FTS/vector channel的`desired_generation`后即成功；notification只作edge hint，worker启动及周期扫描以desired/applied watermark重发现丢失wake。automatic refresh intent稳定且数据库唯一绑定`(workspace, channel, target_generation, handler_contract id/version)`：同一key的finite attempts耗尽后只把该job aggregate置`FAILED`，scanner不得因desired仍领先而创建同key successor。index state只保存desired/applied generation与handler-contract watermark，不复制job的`EXHAUSTED`、reason或id；query以stable key bounded join唯一job并返回`PARTIAL_UNAVAILABLE(INDEX_REFRESH_EXHAUSTED)`。只有更高desired generation或真实的新handler contract形成新automatic key；V1不交付same-key operator retry API，未来若需要必须另立maintenance ADR而不能恢复通用repair owner。index rows与`applied_generation`在同一PostgreSQL transaction安装；FTS/pgvector维护失败只能使对应candidate channel stale/degraded，不能回滚canonical fact。query必须closed返回`COMPLETE | PARTIAL_STALE | PARTIAL_UNAVAILABLE`；partial可合并exact fact/direct-edge/bounded-two-hop结果，但不得用无界canonical scan掩盖索引失败或静默声称complete。不存在“PostgreSQL FULL、Oxigraph pending”的产品状态，也不存在mirror revision、rebind、negative-result freshness或全量RDF rebuild owner。
...
job row可以记录`created_by_writer_generation`作为审计，但worker commit predicate不得检查它。Host cancel只写`cancel_requested`授权事实；实际cancel/failure/settlement由current attempt + claim generation提交。SUCCEEDED job result进入conversation是一个新的Host-owned accept transaction，而不是worker transaction的副作用。
...
Round 1 对“physical tool outcome已知、独立完整输出留存为可选产品能力”做了更精确的特化：physical tool返回后先冻结完整process-local sanitized candidate，artifact publisher再尝试写入shared blob。成功时，`tool_results`在tool-result entry与`ToolResultAccepted`的同一Host-writer transaction中安装nullable blob FK与logical artifact handle；已知publication failure/unconfirmed/超过16 MiB时，同一transaction仍接受已知tool outcome、bounded inline preview与typed `UNAVAILABLE` disposition，不把artifact retention失败改写成side-effect unknown，也不自动重跑tool。identity/FK/content conflict仍是canonical corruption并fail closed。这一特化没有新增artifact relation、receipt、repair或durable retry job。

Tool-result preview始终是不超过65,536 UTF-8 bytes的`InlineContent`；小输出完整显示，需要归档但仍适合完整展示的输出附一个artifact reference，大输出使用UTF-8-safe head/omission/tail并明示`artifact_read`调用。`source_coverage`、`artifact_disposition`与`display_kind`是三个正交closed dimension；terminal rolling buffer已产生gap时只可归档`RETAINED_SNAPSHOT`，offset 0从retained body开始。`artifact_read`只经exact session/workspace/tool-result edge读取`AVAILABLE | INCOMPLETE`的UTF-8 text，支持info与bounded character slice，其结果不递归产生artifact。
...
7. canonical fact transaction成功后，exact fact与direct relation query立即可见，并在同transaction推进受影响FTS/vector channel的desired generation；worker以persistent desired/applied差异重发现丢失wake，index rows与applied generation同transaction安装。FTS/pgvector允许短暂stale/degraded并异步追平，失败不回滚fact；query以closed complete/stale/unavailable disposition显式标出partial。UI可以把candidate显示为“待治理”，但不得在decision commit前把它混入normal recall或渲染成accepted memory。
...
1. `subagent_tasks`持久化已接受的task id、objective/profile、parent task/turn、创建它的`execution_writer_generation`、closed status与terminal reason；已接受的message/result作为该task的immutable children保留。它们表达coordination/history，不构成可claim的work queue；同一task最多一个RUNNING task-scoped turn；
...
7. child内部已提交的conversation使用同一session中的`SUBAGENT_TASK(task_id)` scoped turn/entry；需要作为parent-child coordination公开的message/result child以exact FK引用该scoped entry而不复制正文。tool call自然拥有真实assistant block/call subject并继续服从统一attempt/result contract；若Host丢失时attempt存在而result缺失，unknown属于exact tool attempt，而不是通过subagent aggregate恢复或重做；
8. ROOT compiler永远只读ROOT scope。当前Host接受exact child result时，在同transaction创建ROOT entry、以`source_subagent_result_id` exact FK + session-local unique引用result child，并追加`UserMessageAccepted(source=SUBAGENT_RESULT)`；这复用entry/source模式，不增加cross-scope binding relation、JSON id list或第27种event。进入既有RUNNING ROOT时必须在provider safe point与下一prepared-input handle freeze线性化；active handle/model operation期间不写entry/event，source仍留在child row。parent turn已terminal时不自动新建turn或注入另一个active turn，只有显式new-turn accept command可以消费该result；
9. future product若真的要求delegation跨Host继续，必须另行定义durable work identity、result acceptance、effect safety、cancel与visibility contract并重新评审Stage 2 schema；不能仅增加`background=true`、复用旧subagent event/checkpoint或把child `RuntimeSession`塞进generic job payload。

这保留了Pulsara的subagent产品能力和完整已接受history，同时删除最昂贵、也最难正确承诺的跨Host child execution recovery。`AgentEvent`只观察`SubagentTaskAccepted`、closed status及必要message/result acceptance，不驱动child重新执行；message/result child occurrence只作coordination audit/notification，task transcript由同transaction的普通entry occurrence形成唯一`ImmutableEntryProjection`，不得为同一entry生成第二份immutable projection。
...
V1 formal AgentEvent namespace只有Runtime-owned `pulsara.core`：active决策7的exact 27类Committed与23类Live，共50类。extension只能订阅Host生成的typed projection，不能定义、发布、伪装或动态注册新的Committed/Live AgentEvent type；`publisher_id`不是extension API，core producer仍由closed Runtime/domain owner决定。extension自己的debug topic只能进入namespaced Operational diagnostics/logging，不进入50类registry、durable serializer或historical decoder。未来增加formal type必须重开architecture decision并同时修改type count、producer、schema/version、sensitivity、subject/guard与fixtures，不能靠manifest或自由JSON热插拔。
...
显式user/conversation/tool/queue/interaction、全部subagent task/message/result acceptance、foreground `remember_*` proposal/tool-result acceptance，以及Host存活期间把terminal result或bounded monitor observation接受为canonical entry，都使用`HostWriterGuard`；process-local terminal monitor与child executor本身没有append authority，physical completion/progress/close也没有committed type。proposal transaction只把candidate变成durable work fact，借`ToolResultAccepted`、`TerminalObservationAccepted`及必要`JobQueued`观察，不新增`MemoryCandidateAccepted`。自动memory extraction/governance等worker-owned canonical mutation使用`JobAttemptClaimGuard`；governance worker只允许写其job/attempt-owned decision、memory output或决策7允许的memory/job occurrence，不能写transcript或subagent coordination rows。job result进入conversation仍需当前Host另做accept transaction。没有immutable `origin_session_id`的global job/memory work不进入session-scoped `agent_events`。V1不增加第三个generic memory/governance guard：无法归入Host或durable job的writer没有committed-event append authority。
...
首次job claim不要求caller凭空持有尚不存在的`JobAttemptClaimGuard`。sealed `claim_attempt()`先无锁定位candidate；有origin session时先锁session allocator，再锁/revalidate job、创建attempt与lease generation，并只在该transaction内部mint guard、追加`JobAttemptAccepted`；commit确认后才把exact worker capability返回给handler。global job跳过session event。这个bootstrap是Job guard的封闭构造器，不是第三种guard；claim ACK unknown按job/current attempt查询winner，未确认前不得执行handler。
...
- formal AgentEvent registry必须exact等于active `pulsara.core`的27 Committed + 23 Live；plugin manifest、callback或Operational topic不得增加、替换或发布Committed/Live type，也不得提供`CustomAgentEvent(kind, payload)`逃生口；
- capability composition只能读取canonical human `USER_MESSAGE | USER_STEER`对应的read-time USER item；`TERMINAL_OBSERVATION`必须保持独立provider-input kind，即使provider adapter最终把它编码为带untrusted边界的user-role message，也不得触发skill/capability activation；
...
`sessions.latest_event_sequence`是selective committed journal的public observation high-water。active决策7列出的27类canonical transition——包括queue admission/consume/cancel/reject、accepted interaction decision、turn terminal、accepted Terminal observation、tool attempt/remote identity、message/tool/job/memory/coordination acceptance——都由该canonical owner在同一transaction通过closed `EventAppendGuard`写对应`CommittedAgentEvent`并推进high-water。Host domain使用writer guard；job/automatic memory worker使用exact attempt-claim guard。session detach/close、纯CAS revision、context compiler中间态、pending live interaction、Terminal physical lifecycle、background worker private progress、spinner、transport和UI observation不发committed event。这样不再需要独立`control_revision`或per-section cursor：entry sequence回答conversation ordering，event sequence回答所承诺的可观察transition ordering，canonical row仍回答current truth。
...
active 27类到wire branch的closed mapping同时由Protocol v3 fixture冻结：`UserMessageAccepted | AssistantMessageAccepted | AssistantToolRequestAccepted | ToolResultAccepted | UserSteerAccepted | TerminalObservationAccepted`进入`ImmutableEntryProjection`；`SubagentMessageAccepted | SubagentResultAccepted | JobAttemptAccepted | MemoryRelationAccepted`进入`EventOnly`；其余17类进入`CurrentControlProjection`。subagent child event只观察coordination child；同transaction的普通entry event才投影task transcript，因此同一entry恰好产生一份immutable projection，Go不得把child occurrence再次解释成entry。
...
live到committed的happy path也不能留给Go猜测，但不需要第三套cross-plane ordering。provider/tool-result Start/Delta/End携带process-local owner epoch、turn/conversation scope、channel/generation、预生成entry/block identity；successful canonical commit复用这些identity。Host可以best-effort发送typed `LiveGenerationSettlement(Committed | Aborted)` delivery-control hint，settlement不是第24类live event且不落库，也不把draft升级为canonical truth。live bus sequence与committed `event_sequence`各自独立；Go只按canonical entry/block identity做join，任何先到的canonical projection都立即替换并retire provisional generation，迟到delta/settlement因相同canonical identity被丢弃。settlement或End只触发bounded level-read，失败转fresh snapshot；Protocol不创建`attachment_sequence`、cross-plane reorder buffer、ACK或retired-delivery cursor。Host crash后attachment state全部消失，重连只读canonical snapshot，不合成历史settlement/Start/End。active 27个committed type到`EventOnly | ImmutableEntryProjection | CurrentControlProjection`的exhaustive mapping由Protocol v3 generated fixture冻结。
...
目标审查预算为24个产品表；下面是逻辑关系，不要求一项一表。Stage 2 dormant construction期间它们物理上全部位于独立`pulsara_v3` schema，repository/SQL/FK显式schema-qualified，legacy runtime role无`USAGE`；complete reset后new composition只获该schema权限，`public`同名legacy表保持空且不可达。超过预算需要逐项证明产品价值，但表数本身不判定正确性；也禁止用一个无约束巨型 JSON 表隐藏多个彼此独立的authority：

| 逻辑关系 | 作用 | authority |
|---|---|---|
| sessions | `workspace_id`/session metadata、lifecycle、writer generation/lease fencing、commit-ordered latest entry/event sequence与prompt queue sequence；session identity不可复用，且`UNIQUE(id, workspace_id)`供subject composite FK | canonical high-waters；没有transcript/event retention lower bound，event high-water不是row truth |
| turns | user turn、status、final entry、interruption、command/client submission id、current context binding revision pointer、closed `ROOT | SUBAGENT_TASK(task_id)` conversation scope；同session最多一个RUNNING ROOT、每task最多一个RUNNING task-scoped turn | canonical |
...
| transcript_entries | user/assistant message/tool result；commit-ordered append-only sequence并从turn继承conversation scope；assistant tool-request message是原子parent；provider-generated assistant entry绑定exact context revision与`provider_input_through_sequence`；ROOT external-result entry使用closed `source_job_id | source_subagent_result_id | no source` FK union及session-local unique，进入既有RUNNING ROOT时只在provider safe point接受；每个正文slot使用受约束的inline-bytes/blob-FK exactly-one union | canonical |
...
| tool_results | call唯一terminal result；exact call/attempt join；closed pre-dispatch terminal是唯一无attempt分支；late reconciliation保留实际entry sequence；可选primary output artifact以nullable shared-blob FK与logical handle表达，并独立保存source coverage、artifact disposition、display kind与coverage/unavailability reason | canonical conversation child；artifact不是独立authority，preview始终是bounded inline entry |
| durable_jobs | background compaction precompute/post-compaction memory extraction/governance等first-party具名、确需跨Host继续的intent及stable handler-specific intent identity、safety class、closed aggregate state、accepted result，以及immutable versioned retry policy、finite attempt/timeout和LLM每attempt input/requested-output token cap；V1不包含generic durable extension action，也不包含任何subagent execution、yielded terminal process或terminal monitor | canonical job aggregate |
| durable_job_attempts | attempt ordinal、claim generation/lease/deadline、set-once provider-call admission与该次bounded request量、remote identity、terminal/unknown、result/error与retry_of | canonical work journal |
| prompt_queue_items | stable session queue sequence、command/client submission id、closed `NEW_TURN(NULL target) | STEER_ACTIVE_TURN(exact same-session ROOT target)` union、`PENDING | CONSUMED | CANCELLED | REJECTED`与exact consumed entry；无claim/lease/checkpoint | canonical ordered inbox |
...
| subagent_task_children | task下immutable message/result child、closed child kind、ordinal与stable child id；exact FK引用同session task-scoped transcript entry，不复制正文 | canonical coordination child；可共用一张typed table，不是第二task aggregate/session |
...
| search_indexes | 可重建全文/vector索引及每workspace/channel desired/applied generation与handler-contract watermark；canonical memory mutation同tx推进desired，index rows同tx推进applied；automatic refresh intent对`workspace/channel/target/contract`唯一，job状态不复制进index state | derived、non-gating；query以stable key bounded join job并显式complete/stale/unavailable，同key exhausted后scanner不重建 |
| schema_migrations | physical schema version | infrastructure |

V1必须把`subagent_tasks`保留为Host-owned canonical coordination relation，不能把它建模为`durable_jobs` type，也不能同时拥有另一份execution state。child conversation使用同一session allocator与`SUBAGENT_TASK(task_id)` scoped turns/entries；它不是另一个durable session，也不能被root TUI误当parent transcript。message/result child必须拥有数据库可引用的stable identity并exact引用对应scoped entry：可以各自成表，也可以共用带closed child kind的`subagent_task_children`，但不能复制正文、藏在task JSON payload或伪造parent call subject。无论哪种布局，都不得出现background flag、subagent execution attempt/claim/lease/checkpoint列。

context compiler的FULL_HISTORY与post-snapshot delta按exact turn conversation scope过滤；session-wide entry sequence只提供commit order，不授权ROOT读取任意task scope。parent若需要child result，只能由当前Host把exact result child接受为ROOT-scoped entry：该entry以`source_subagent_result_id` typed FK + session-local unique引用source并产生普通`UserMessageAccepted(source=SUBAGENT_RESULT)`。进入既有RUNNING ROOT时，external acceptance与下一prepared-input freeze必须在同一短process-local provider-safe admission边界线性化；accept先赢则下一cut覆盖entry，freeze先赢则本次assistant前external entry/event为0。不存在cross-scope binding table、JSON id list、durable pending receipt或TUI projection隐式纳入；parent turn已经terminal时也不得自动把result转投另一个active turn。
...
|---|---|---|---|
| 0 | 产品语义与并发约束冻结 | single Host writer、两类closed append guard、Stage 2 base exact 26 committed core/13 subject slots、base exact 49 formal AgentEvent subscription-only extension、stored/observation/content DTO、registration/version/lease、V1 generic durable extension action为0、唯一无rewrite tool policy、live control、partial、subagent、terminal、audit、stream、memory physical store与candidate-first async governance决策不再漂移 | 决策可转成行为test；151 inventory及A39/B25/C16/D71逐项处置可重复，base 26类core mapping、49类formal registry、0 custom publisher/action与1 policy kind可生成结构guard；Round 2另以reviewed product extension推进为27/50 |
...
| 5 | universal EventLog退役 | 151类universal registry、execution replay/proof schema物理删除；selective `agent_events`与LiveAgentEvent package按Stage 2 base exact 49 core types定型 | production import graph无旧execution authority；custom AgentEvent publisher为0；committed/live/operational隔离guard全通过；后续具独立产品语义的type仍须逐项architecture review |
...
- active selective committed registry exact等于决策7的27类、subject union exact等于13种slot；`PromptRejected`不合并为cancel，subagent message/result必须引用exact child，memory candidate、derived tool outcome unknown、Terminal physical lifecycle与retryable job-attempt terminal不发committed event；
...
- 两条探针可稳定复现43/83 EventLog event量级，AST inventory稳定得到151类及14-family分布；五片逐项审计可重复得到A39/B25/C16/D71，并生成“旧151类→row/A/B/C/D→Stage 2 base exact 26 core”的closed migration manifest；Round 2新增type不是旧event换名复活；
...
- committed registry/subject/guard与live registry使用独立生成输入：active前者只能生成exact 27类、13种slot与两种guard，后者只能生成决策7的exact 23类；独立raw-provider registry/union数量必须为0，vendor SDK wire type不得进入Runtime生成输入。
...
1. 独立`pulsara_v3`中的fresh conversation schema、canonical content slot的inline/blob exactly-one union、Stage 2 base exact 26类selective `agent_events`、13-slot closed typed subject-FK union/event-sequence high-water、PostgreSQL `memory_candidates`/closed governance decisions/`memory_facts`/`memory_relations`与现有FTS/pgvector read models、minimal `durable_jobs`/`durable_job_attempts` schema、claim repository、job-result acceptance port、global blob contract与migration，默认dormant；旧runtime role无schema权限，repository显式schema-qualified，新schema没有Oxigraph surface/delivery relation；
...
1. `sessions/turns/turn_context_binding_revisions/transcript_entries/assistant message blocks/agent_events/tool_execution_attempts/tool_results/context_snapshots/blobs/prompt_queue_items/subagent_tasks/subagent_task_children/durable_jobs/durable_job_attempts/memory_candidates/memory_governance_decisions/memory_facts/memory_relations`直接schema、canonical content slot inline/blob exactly-one union、commit-ordered entry/event sequences与session high-waters、stable queue sequence/四态CAS/closed frozen-target union、Stage 2 base exact 26 event registry、13-slot exactly-one typed subject FK及type/slot mapping、assistant exact revision + `provider_input_through_sequence`、`(session_id, command_id)`、assistant-message/call/attempt/result pairing、same-session task-scoped conversation/每task单RUNNING约束/subagent child exact entry FK、ROOT result-source typed FK + unique及existing-ROOT provider-safe admission、subagent Host-generation/status约束、sealed first-job-claim、job fencing/final winner/finite retry与每attempt provider request约束，以及memory desired/applied generation、index-refresh stable intent/exhaustion约束；
...
3. Host open时原子 acquire/takeover + 旧running root/task-scoped turn和旧generation nonterminal subagent task→interrupted；不构造旧RuntimeSession/child RuntimeSession，不恢复provider/tool/subagent；
...
6. Protocol v3 repeatable-read snapshot/page/`event_sequence_cut`、26-type到三个observation branch的closed mapping（subagent child occurrence固定`EventOnly`，task entry只投影一次）、bounded `CommittedObservationProjection`/budget-or-schema GAP、session-lifetime stored-event audit query、统一`ObservationContent`与stateless `ReadCanonicalContent`逐请求鉴权/range/integrity contract、provider/tool-result exact live generation、identity-based live/committed handoff与best-effort settlement hint、first-party user live profile、`CompleteToolArguments | TruncatedToolArguments`、带epoch/revision的atomic session live-control snapshot/event，以及canonical-row command query/idempotency contract；
...
- job/subagent external result进入existing RUNNING ROOT与下一prepared-input freeze使用同一短process-local safe-point admission；acceptance先赢则cut覆盖ROOT entry，freeze先赢则本次assistant前ROOT source entry/`UserMessageAccepted`为0且source domain fact仍可提交；
...
- subagent task/message/result/status只由当前Host接受；task result进入existing ROOT还必须通过上述provider-safe admission。全部physical child state只在进程内，orderly close bounded cancel/join后把nonterminal task置interrupted，takeover完成同一幂等收口；reattach不resume/requeue，重新委派创建new task id；
...
- index lost-wake scanner只可创建/claim exact stable refresh intent；同一target generation/handler contract exhausted后不自动创建successor，current channel由query bounded join唯一failed job而显式unavailable；只有更高desired或新contract形成新automatic key，V1 same-key operator retry surface为0；
...
- queued steer始终投递到enqueue时冻结的exact ROOT turn；target terminal后只确定性reject并推进FIFO，不转投另一个active turn；
- ROOT external-source acceptance与prepared-input freeze的两个调度次序都线性化：不得出现H=100冻结后ROOT external entry=101、assistant=102但cut仍为100；freeze winner期间source job/task fact仍可提交，但ROOT source entry/`UserMessageAccepted`为0；
...
- Stage 2 fresh schema的committed registry逐项exact匹配当时决策7的base 26类，数据库type→subject slot/guard矩阵逐项匹配；Round 2 migration在同一closed input上增加`TerminalObservationAccepted`形成active 27类；`PromptRejected`与`PromptCancelled`分别由系统拒绝和显式取消transaction产生，不能共享一个模糊terminal type；
...
- 新assistant/tool-result stored event只保存occurrence与exact subject FK；Gateway在同一bounded read transaction形成带ordered `ObservationContent`的`ImmutableEntryProjection`，Go对inline直接渲染、对blob reference经唯一bounded read port hydrate；subagent message/result分别引用exact child slot并固定为`EventOnly`，task-scoped普通entry occurrence是唯一immutable entry projection。错误event subject slot/child kind、跨session/workspace subject、missing/deleted subject全部由数据库拒绝；
...
- retry-safe job只有在deterministic due与attempt deadline/maximum允许时才能新claim；LLM attempt在provider call前原子安装唯一admission并校验该次input/requested-output cap，每attempt至多调用一次，attempt耗尽或request超cap时稳定FAILED且aggregate terminal occurrence恰好一次，不恢复累计budget account或receipt graph；
- index refresh job exhausted后，scanner重复运行不能为同一`workspace/channel/target_generation/handler contract`创建第二个automatic aggregate；query明确`PARTIAL_UNAVAILABLE(INDEX_REFRESH_EXHAUSTED)`，更高target或新contract才形成新的automatic key；
- task-scoped result只有经当前Host创建带exact `source_subagent_result_id` FK + unique的ROOT entry后才进入parent context；每task最多一个RUNNING scoped turn，terminal parent不自动接收或转投result，cross-scope binding relation/JSON列表为0；
- job/subagent result进入existing RUNNING ROOT只在active prepared-input/model operation为0且上一tool-request全部terminal的safe point接受；不得为unsafe等待新增durable pending/receipt owner；
...
- durable prompt queue以stable sequence保留顺序；未commit consume在restart后仍为pending，不存在claim recovery；
...
- 当前Host以writer generation显式接受SUCCEEDED job result后，结果才进入conversation；
...
`durable_jobs` row最少保存：job id/type、payload/blob ref、stable handler-specific intent identity/contract version、closed aggregate status、safety class、current attempt id、accepted result/error ref、cancel request、immutable retry policy id/version、finite maximum attempts/attempt timeout、next eligible time，以及LLM handler的finite per-attempt input/requested-output token cap、created/updated time。`created_by_writer_generation`可作审计，但worker conditional commit不得检查它。

`durable_job_attempts` row最少保存：job id、attempt id/ordinal、claim generation、lease owner/expiry、attempt deadline、started/terminal time、stable remote/idempotency identity、LLM provider admission与该次bounded request量的set-once字段、status、immutable result/error ref、retry_of_attempt_id。一个job最多一个active attempt；job aggregate与terminal attempt在同transaction更新。唯一性与generation conditional update直接由这两张窄表和PostgreSQL约束完成，不增设累计budget account、receipt/confirmation/target-head表，也不允许`attempt_summary JSON`代替历史rows。

handler规则：

1. `RETRY_SAFE`：lease expiry/loss后terminalize旧attempt；只有deterministic due、attempt ordinal/maximum与deadline仍允许时，才能创建带`retry_of_attempt_id`的下一attempt；LLM attempt还必须逐次满足冻结的request cap，不得覆盖旧row或把0/`None`解释为无限；
...
memory index-refresh的automatic intent另外以`workspace/channel/target_generation/handler contract`受数据库唯一约束。同key耗尽只更新该job aggregate，scanner不得创建successor，query直接bounded join它；index state不复制job health。更高target或新contract形成新automatic key；V1不交付same-key operator retry action。

LLM handler每次实际provider call前必须先由current claim对attempt执行唯一admission CAS，保存该次input/requested-output token数并校验per-attempt cap；ACK unknown读取该attempt winner，未确认不得dispatch，同attempt marker已存在不得再次调用。maximum attempts耗尽或request超cap时aggregate稳定FAILED，只有该aggregate nonterminal→terminal CAS可追加唯一`JobTerminalAccepted`；中间attempt failure不得追加。

Host cancel只提交`cancel_requested`授权事实；current attempt claim owner观察后以attempt id + claim generation提交cancelled/failure/settlement。worker完成只提交immutable attempt result并在同transaction推进job aggregate。若conversation需要该结果，当前Host创建独立accepted transcript entry/turn mutation；entry直接保存`source_job_id`并保证session内唯一，或复用stable acceptance command id，因此这个accept可以在Host takeover后发生、ACK unknown后幂等重试，但不能被worker transaction隐式完成。若写入existing RUNNING ROOT，accept还必须与下一prepared-input freeze在provider safe point线性化；unsafe时job result row保持唯一durable source。
...
- result commit后不重复执行；
- index scanner反复运行不会为同一exhausted target/handler contract创建第二个automatic job；
- Host close不等待job完成；
...
- committed core vocabulary必须exact等于active决策7的27类，live vocabulary必须exact等于同一决策的23类，正式AgentEvent总数exact为50；独立`RawProvider*`/raw-provider registry/逐delta semantic-draft protocol必须为0。新增或删除任一formal type、改变committed type→subject/guard映射都需要architecture review；不得靠巨型JSON、通用receipt graph、第二transcript source或重复transport DTO伪造收敛。
...
1. user/turn、assistant/final、完整assistant tool-request message及ordered blocks、逐call tool execution attempt与terminal result、subagent task/message/result/status及close/takeover interruption、context snapshot、binding revision install/current-pointer advance、accepted interaction decision、open/takeover、job enqueue/cancel、job/subagent-result进入ROOT acceptance的SQL transaction boundary；provider-generated assistant transaction还必须消费唯一prepared-input handle并保存其exact revision/cut，进入existing ROOT的external-source acceptance必须与handle freeze走同一短provider-safe admission；
2. session-wide command id物理唯一形状、same-input comparison与canonical query SQL；
3. per-session entry sequence与selective event sequence各自在owner transaction内按commit order分配，`latest_sequence`/`latest_event_sequence`、统一session event allocator lock order、rollback/Host-worker并发commit、append-only history/event cut、session-lifetime event retention与committed budget/schema GAP规则；Stage 2规格当时把base exact 26 type、13 subject slot、type→slot与type→guard矩阵生成到SQL constraint、serializer registry和cross-language fixture；event sequence只排序accepted occurrence，不替代entry ordering或canonical truth；schema不得出现event retention lower bound或prune transaction；
...
5. durable job aggregate/attempt claim/result/retry lineage/cancel request、closed `SUCCEEDED | FAILED | CANCELLED | OUTCOME_UNKNOWN` terminal vocabulary、per-handler immutable retry-policy version、finite maximum attempts/attempt timeout/deterministic due、LLM one-call-per-attempt与per-attempt request cap/pre-dispatch admission、source-job acceptance唯一约束，以及index-refresh `(workspace, channel, target_generation, handler contract)` automatic intent唯一/exhausted不重建规则；
6. Protocol v3 canonical snapshot/page、`StoredCommittedEvent`与exactly-one typed nullable subject-FK union、`CommittedObservationProjection`三分支及bounded hydrate/GAP、Stage 2 base 26类到三分支的exact mapping（subagent child occurrence固定`EventOnly`且同entry只产生一次immutable projection）、snapshot/history/observation共用的`ObservationContent`、provider live stream/snapshot、authenticated first-party user live profile、`CompleteToolArguments | TruncatedToolArguments`、ordinary/privileged extension projection、Host-owned capability issuer/revoke、带`owner_epoch/live_revision`的session live-control `snapshot_and_subscribe()`与Opened/Replaced/Closed DTO、ordinary post-commit registration cut/no-catch-up、read-only repeatable-read transaction和Go cache transition；同时冻结`CommittedAgentEventBase`、`LiveAgentEventBase`、`OperationalEventBase`的物理依赖方向；
...
10. complete-reset store manifest、old-process/remote-effect quiesce、无import/cold-archive/reverse-projection guard、cross-language fixtures、ACK unknown、concurrent entry/event/exact-subject read cut、observation budget GAP、blob content多range/跨codec边界/scope撤销/corruption、typed subject negative constraints、Host/job append race、lost notification、provider/tool-result/live-control overflow与epoch change、raw-thinking user projection原样性、tool-argument short/oversize/UTF-8截断与dispatch isolation、user/ordinary/privileged/revoked/S3 projection matrix、hook exception/timeout/detach/capability leak/no-catch-up、pending interaction replace/takeover、frozen steer target terminal rejection、PromptRejected-vs-cancel、ToolResult live-End-before-commit、job retryable-attempt/due/deadline/per-attempt-provider-bound-vs-aggregate-terminal、index-refresh exhausted repeated-scan、memory candidate-no-event、compaction-complete-without-adoption、subagent exact-child/单RUNNING task turn/唯一ROOT source acceptance/provider-cut双向100→101→102 race/terminal-parent不自动注入与orderly-close/takeover interruption/no-resume/new-task identity、mixed/multi-tool crash、stale generation和physical resource shutdown fault matrix；
...
| committed event vocabulary | 当前151类universal `EventType` | active exact 27个`pulsara.core`类型（Stage 2 base 26 + `TerminalObservationAccepted`）；type/subject/guard逐项固定，extension-defined/published type为0 | 结构gate |
| formal AgentEvent vocabulary | 当前151类universal registry | active exact 50：27 Committed + 23 Live；custom/free-form extension type为0 | 结构gate |
...
对象/fact百分比使用当前下界，因此实际降幅可能更高。产品表`≤24`、Host close`≤12 awaits`与净删`≥22k` production LOC仍只是architecture review budget和减法信号；active committed core exact 27类、type/subject/guard mapping以及required live grammar则已升级为结构gate。两者都不能通过巨型JSON、合并无关类型、生成代码迁移、删除typed可观测性或机械合并coroutine来取巧，也不能让一个行为错误的cut通过。live与operational类型使用独立registry/预算，不能混入committed数量，也不能被“process-local”解释成无需schema/version。
...
5. **client mutation幂等**：turn/queue/accepted-interaction-decision canonical row持有session-wide command id；user acceptance ACK unknown后同id/same input返回原target，同id/different input稳定conflict；query不依赖receipt row。queue steer同时冻结exact same-session ROOT target，terminal target只转REJECTED而不能投递给新的active turn；
...
9. **job retry安全与lineage**：stale attempt claim generation不能commit；retry-safe重执行只在versioned deterministic due与finite maximum attempts/timeout内创建新attempt并保留retry_of；LLM每attempt至多一次provider call，调用前先原子安装admission并校验该次input/requested-output cap；remote-queryable只observe旧remote identity，旧attempt永不被覆盖；attempt耗尽或request超cap稳定FAILED且aggregate terminal occurrence恰好一次；
...
14. **selective occurrence保持最小但非零**：active committed registry exact等于决策7的27类；只有具独立用户可观察/audit/hook语义的accepted transition进入`agent_events`。tool attempt、public remote-identity publication、turn interrupted及accepted Terminal observation等transition必须有committed occurrence；physical Terminal completion/progress/close仍只在Live plane。`PromptRejected`与explicit cancel分型，job只在enqueue、attempt acceptance与aggregate terminal发A，memory candidate/skip/batch progress不发A，compaction只在binding revision真实adopt时发A。`outcome_unknown`只由attempt/result/turn在read time推导，不发独立occurrence。每个stored event以13-slot exactly-one typed nullable FK引用subject，type/slot、type/guard、same-session/workspace和deletion由数据库约束；subagent message/result引用exact child而非task aggregate并固定为`EventOnly`，普通entry occurrence是同一task entry唯一的`ImmutableEntryProjection`。纯CAS/context compiler中间态、retryable job-attempt terminal、background private progress、transport与UI observation不进入journal；没有session lifecycle core、独立control revision/history、per-section cursor、receipt、checkpoint或consumer ACK；
...
23. **memory proposal、governance与index retry解耦**：现有`remember_*`的`proposed`只在candidate row与对应tool result原子commit后成立；automatic extraction同样先提交candidate。governance只claim已commit candidate并在durable job中异步完成，foreground provider/reply/turn/close不等待；decision前candidate不进入normal recall。index lost-wake scanner使用对`workspace/channel/target_generation/handler contract`唯一的automatic intent；同key finite attempts耗尽后保持exhausted并返回partial unavailable，scanner不能创建无限job链。V1没有delete/forget tool、candidate、decision、event或pending-delete状态，supersede/stale不改名为删除；
...
25. **subagent execution lifetime绑定Host**：accepted task/objective/parent-child/message/result与terminal status保留；child conversation使用同session的`SUBAGENT_TASK(task_id)` scoped turn/entry，每task最多一个RUNNING scoped turn，message/result各有stable exact child id并FK对应entry，内部tool attempt引用真实scoped assistant block；同一result的explicit/inferred路径只接受一次。ROOT只在当前Host以exact `source_subagent_result_id` FK + session-local unique创建ROOT entry后看到result；进入existing ROOT还必须在active prepared-input/model operation为0、上一tool-request全部terminal的safe point与下一handle freeze线性化。terminal parent不自动接收、创建turn或转投其他active turn，cross-scope binding relation/JSON列表为0。activation task、physical run、child `RuntimeSession`、partial live output和capacity/MCP owner只在当前进程。orderly close有界cancel/join并把nonterminal task置`interrupted`且同transaction写`SubagentTaskStatusAccepted`；不得复用`cancelled`或`SUBAGENT_RUN_*`。crash后的takeover按旧`execution_writer_generation`完成同一幂等收口。production job catalog/schema没有durable child session、subagent handler、execution attempt/claim/lease/checkpoint；reattach不resume/requeue，显式重新委派使用new task id；child effect unknown只落exact tool attempt。
...
- committed registry不与active决策7的exact 27类逐项相等，出现`CUSTOM`/generic `StateChanged`逃生口、`ToolOutcomeUnknown`、`subject_session_id`或第14种subject slot，type→slot/type→guard没有生成数据库constraint，或serializer/cross-language fixture与SQL registry各自维护而可漂移；
...
- prompt queue的`STEER_ACTIVE_TURN`没有exact same-session `target_turn_id`、consume时重新解析“当前active turn”、terminal target永久阻塞FIFO或被转投其他turn；
- machine `CapabilityDecisionAccepted`承载最终human allow/deny、subagent status重新出现dependency专用durable状态，或job terminal同时接受`COMPLETED`与`SUCCEEDED`；SQL CHECK、event payload与generated fixture没有共用closed descriptor；
...
- subagent child occurrence被映射为第二个`ImmutableEntryProjection`、每task可存在多个RUNNING scoped turn、ROOT compiler直接跨scope读取child result，或result acceptance没有exact source FK + unique而依赖binding table/JSON id list；
- job/subagent external source在active prepared-input handle/model operation期间仍可进入existing ROOT、accept transaction与下一handle freeze没有同一短process-local linearization、freeze winner后出现`H < external entry sequence < assistant sequence`，或为等待safe point新增durable pending/receipt/recovery owner；
...
- retry-safe job缺少finite immutable maximum attempts/timeout/deterministic due，LLM handler没有one-call-per-attempt或finite per-attempt request cap，provider在set-once admission transaction确认前被调用，或attempt/request上限耗尽后仍可claim/dispatch；
- index scanner在同一`workspace/channel/target_generation/handler contract` automatic job exhausted后创建新的automatic aggregate、stable key不受数据库唯一约束、exhausted current target仍静默返回stale/complete，index state复制job terminal truth，或Stage 2新增same-key operator retry/repair owner；
...
- committed/live/operational registry的类型数量与payload byte预算分别报告；active exact 27 committed、exact 23 live及独立raw-provider registry为0是前述结构gate，不在这里降级为数值审查带；
- 产品表超过24；
...
review必须逐项检查语义；产品表等审查预算不能因一个数字自动判对或判错，active exact 27 committed registry则按closed set直接判定。
...
| async memory governance | candidate commit后让governance model、claim、decision、fact或index maintenance慢/失败/worker crash，并丢失index wake | reply与turn照常完成；pending candidate由新job attempt按canonical query reclaim；accepted decision/fact/relation/occurrence与desired generation原子；worker扫描desired/applied target不一致重发现；query显式complete/stale/unavailable | foreground等待治理、从event replay恢复governance、decision前进入normal recall、silent stale、增加delete/forget或pending-delete路径 |
...
| exact committed registry | 对SQL constraint、serializer、Python/Go fixture分别注入一个缺失type、额外generic type、`ToolOutcomeUnknown`、错误subject slot或错误guard | 三者都只接受active决策7的exact 27 types、13 slots与closed guard mapping；任一漂移在build/migration test失败 | 任意浮动数量预算、`CUSTOM`/`StateChanged`、`subject_session_id`、运行时临场路由 |
| closed formal extension vocabulary | plugin manifest尝试注册custom Committed/Live type、伪造publisher或发送`Unknown + raw JSON`；Operational debug topic与core registry同名 | formal registry始终为`pulsara.core` active exact 50；extension只能订阅协商后的typed projection；Operational topic不进入serializer/type count | plugin event publisher、`CustomAgentEvent`、free-form metadata或manifest热插拔type |
...
| queued steer target becomes terminal | enqueue时冻结ROOT turn A，消费前A terminal并启动ROOT turn B | item确定性`REJECTED(TARGET_TURN_TERMINAL)`并发`PromptRejected`，FIFO继续；B不收到旧steer | consume时解析current active turn、转投B、永久卡住head、恢复claim/receipt |
...
| job retry/provider request bound | 让attempt timeout连续发生，并在provider admission ACK unknown或per-attempt input/output cap边界kill | 只在deterministic due且仍有attempt时claim；每attempt至多一次provider call且有先行set-once admission/request量；maximum耗尽或request超cap后aggregate稳定FAILED且terminal occurrence恰好一次 | 无限retry、同attempt重复call、未确认admission就call、跨attempt累计counter/charge或budget receipt graph |
| subagent result accepted into ROOT | task result child已接受；分别让parent仍RUNNING、已terminal、ACK unknown并并发重复accept | RUNNING parent只产生一个带exact source FK的ROOT entry；terminal parent不自动注入或新建turn；显式new-turn command可幂等接受；child event固定EventOnly | compiler跨scope读child、JSON binding list、转投另一个active turn、同entry双immutable projection |
| ROOT external source vs provider cut | 对job与subagent source分别竞争accept与prepared-input freeze；构造H=100并尝试在assistant commit前插入ROOT entry | accept先赢时ROOT entry=101且下一handle H≥101；freeze先赢时本次assistant可按cut100 commit，source job/task fact可正常落地，但ROOT entry/`UserMessageAccepted`为0，下一safe point或显式new turn再接受 | assistant sequence=102/cut100却已有ROOT external-input entry=101、阻塞source domain commit、长持有DB锁、durable pending/receipt |
...
| live/committed happy-path乱序 | 同一generation分别让committed projection先于settlement、settlement先于projection，并在retire后注入迟到Delta/End | canonical projection总是按entry/block identity替换provisional draft且generation恰好retire一次；settlement先到只触发bounded level-read；迟到frame丢弃；Host crash后fresh snapshot | Go猜subject/content、迟到delta复活draft、settlement持久化/成为第24类live event、增加cross-plane sequencer/ACK |
...
| first job claim bootstrap | candidate尚无attempt时在session lock、job revalidate、attempt insert、event append、commit ACK各点kill | commit前worker capability不可得；commit后exact attempt/claim/event可查询；origin-session allocator先于job lock；guard种类仍为2 | caller伪造guard、先执行handler后补attempt、第三种bootstrap guard、先锁job再回锁session |
...
| subagent exact child observation/effect | 同一task-scoped conversation依次接受两条message、内部tool call/attempt/result与一个explicit result，再让result inferred/explicit路径竞争 | child使用同session task scope；message/result child exact FK scoped entry；tool attempt引用真实scoped assistant block；两条message与唯一result occurrence精确，task completion不重复result | durable child session、伪造parent transcript call、三条event只引用task、payload ordinal定位、explicit result重复接受 |
| terminal monitor append isolation | monitor callback与Host observation acceptance并发，令monitor持有旧handle或抛错 | monitor coordinator只能发process-local typed live/operational event与bounded wake；只有current Host在provider safe point把observation接受为`TERMINAL_OBSERVATION` entry时，由canonical repository同transaction追加`TerminalObservationAccepted`；callback失败不否定commit | monitor持有appender/Host guard、为physical completion/progress/close发committed type、receipt/notification job恢复 |
| terminal observation capability isolation | Terminal output包含`$skill`、`skill:name`或其他human control syntax后接受为canonical observation并启动下一provider call | canonical reader生成独立`TERMINAL_OBSERVATION` provider-input kind；provider adapter可编码为带untrusted boundary的user role，但capability composer只读取human `USER_MESSAGE | USER_STEER`，skill/capability activation为0 | observation复用USER kind、stdout改变skill或tool capability projection、落入assistant branch |
| prompt queue restart | item pending或consume transaction中途kill | 未commit consume保持pending并按stable queue sequence重取；已commit consume与turn/steer entry/event同可见；claim row为0 | checkpoint缺失阻断queue、恢复active claim或重复consume |
| memory index failure | vector/index write失败或notification丢失 | canonical memory fact与desired generation仍accepted；scanner重发现desired/applied target不一致；query返回`PARTIAL_STALE`/`PARTIAL_UNAVAILABLE`，成功重建时index rows与applied同tx | memory acceptance回滚、永久silent stale、无界全表fallback或Runtime latch |
| memory index refresh exhausted scanner loop | exact target/handler-contract job耗尽全部attempt后反复启动/周期scan，再推进或不推进desired | 不推进desired时automatic job总数保持1，index state不复制job failure，query bounded join该job并返回`PARTIAL_UNAVAILABLE(INDEX_REFRESH_EXHAUSTED)`；更高desired或新handler contract才形成新automatic key；V1 operator-retry surface为0 | 每次scan创建successor、index health复制job truth、同key无限attempt/job链、静默complete/stale |
...
- complete-reset empty database/blob namespace端到端通过；dormant product relations位于独立`pulsara_v3`、repository全限定且legacy role无权限，cutover后new role/composition不能解析empty `public` legacy壳；cutover前旧Host/worker/monitor/subagent均已quiesce/fence，外部effect不会被新Runtime导入、继续或重做；
...
- TUI只用canonical conversation snapshot/page、read-time committed observation projection与current provider/tool-result/session-control typed live stream完成fresh attach、history/committed/content-live/control-live四类GAP与reconnect重建；active exact 27 type到三个projection branch由generated fixture闭合，`TerminalObservationAccepted`与其他entry occurrence进入`ImmutableEntryProjection`，subagent child event固定`EventOnly`且同entry恰好一份immutable projection；live generation与committed projection只按复用的entry/block identity交接，两种乱序都由canonical projection胜出且迟到delta不能复活draft；Go不直接解释stored event或subject id，没有cross-plane attachment sequence、presentation root、UI ACK或derived delivery state反向成为Runtime authority，也不从raw canonical row自行猜测实时Start/Delta/End；
...
- committed registry在SQL、Python serializer与Python/Go fixtures中exact等于active决策7的27 types、13 subject slots与closed type→slot/guard mapping；不存在generic/custom/session-subject逃生口。`ToolOutcomeUnknown`、job retryable-attempt terminal、memory candidate/batch/skip、compaction completion未adopt、terminal physical lifecycle与subagent physical run/progress均不能产生A；只有accepted `TERMINAL_OBSERVATION` entry产生`TerminalObservationAccepted`；`PromptRejected`与cancel继续分型；
...
- prompt queue拥有stable session order、closed `NEW_TURN(NULL target) | STEER_ACTIVE_TURN(exact same-session ROOT target)` union与exact`PENDING | CONSUMED | CANCELLED | REJECTED`，consume/cancel/reject只竞争一次CAS，consume与new-turn/frozen-target steer entry及occurrence原子；target terminal只确定性reject并推进FIFO，不转投其他turn；crash后只恢复pending row，不存在claim/lease/checkpoint/receipt；
- subagent已接受task/objective/parent-child/message/result/status可跨attach查询，child conversation使用同session task-scoped turns/entries且每task最多一个RUNNING turn，message/result各有stable exact child identity并FK对应entry，内部tool effect引用真实scoped assistant block，explicit/inferred result恰好接受一次；ROOT只通过current Host接受的exact source FK + unique entry消费result，进入existing ROOT时与下一prepared-input freeze在provider safe point线性化，terminal parent不自动注入或新建turn，cross-scope binding relation/JSON列表为0。全部physical child execution绑定当前Host。orderly close bounded cancel/join并把nonterminal task置interrupted且发唯一status occurrence，takeover按旧generation幂等收口，reattach不resume/requeue且重新委派使用new task id；production schema/registry/job catalog/import graph没有durable child session、`SUBAGENT_RUN_*`、subagent background flag、job handler、execution attempt/claim/lease/checkpoint或child recovery owner；
...
- single Host writer与job-attempt claim两个独立fencing domain、sealed first-claim bootstrap、job aggregate/attempt lineage与三类safety规则通过跨进程故障注入；V1四种具名handler safety class固定为retry-safe且满足winner隔离，remote-queryable handler为0，不能由registration覆盖；每个handler拥有immutable finite retry/timeout/deterministic due，LLM handler的one-call-per-attempt、per-attempt request cap与pre-dispatch admission在restart/ACK-unknown下仍不超，attempt耗尽或request超cap只产生唯一aggregate terminal occurrence；index scanner不能用新job绕过同一target/contract的exhausted attempts；
...
- initial context revision与user/turn原子安装；mid-turn只在provider safe point新增revision并推进pointer；每次provider dispatch从固定MVCC cut建立唯一prepared-input handle，每条accepted provider-generated assistant原样保存exact revision与`provider_input_through_sequence`；tool late-result的100→101→102按cut保留late observation，而job/subagent external-source的100→101→102必须通过safe-point admission使accept或freeze恰有一个先行winner，不能在assistant前插入未见输入；并发commit-order sequence测试通过；compaction不修改transcript/epoch，unreferenced snapshot可GC，被revision引用的snapshot exact保留并通过current-turn-delta rematerialization；
...
- Oxigraph代码、依赖、配置、surface worker/delivery、Inspector/CLI、contracts、tests与部署说明全部物理删除；PostgreSQL-only memory tools、governance、FTS/vector/direct-edge和现有bounded两跳recall在无Oxigraph环境全绿；canonical memory mutation同tx推进desired、index rows同tx推进applied、lost wake可由scanner重发现，query不会隐瞒stale/unavailable；同一index target/handler contract exhausted后scanner automatic successor为0，index state不复制job terminal truth，higher target/new contract边界闭合且V1 same-key operator retry surface为0；
